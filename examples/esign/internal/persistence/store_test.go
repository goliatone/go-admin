...
func newSQLiteBootstrapForStoreAdapterTests(t *testing.T) *BootstrapResult {
	...
	cfg.Persistence.SQLite.DSN = "file:" + filepath.Join(t.TempDir(), "phase4-store-adapter.db") + "?_fk=1&_busy_timeout=5000"
...
func TestStoreAdapterPreservesOptimisticLockVersionConflicts(t *testing.T) {
	...
	_, createErr := adapter.Create(ctx, scope, stores.DocumentRecord{
		ID:                 "doc-opt-lock",
