...
func TestSigningServiceGetSessionSequentialState(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase4-session"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}

	_, err = store.UpsertFieldValue(ctx, scope, stores.FieldValueRecord{
...
func TestSigningServiceGetSessionStageParallelState(t *testing.T) {
	...
	signerOne, err := agreementSvc.UpsertRecipientDraft(ctx, scope, agreement.ID, stores.RecipientDraftPatch{
		Email:        new("stage1-signer-1@example.com"),
	...
	signerTwo, err := agreementSvc.UpsertRecipientDraft(ctx, scope, agreement.ID, stores.RecipientDraftPatch{
		Email:        new("stage1-signer-2@example.com"),
	...
	signerThree, err := agreementSvc.UpsertRecipientDraft(ctx, scope, agreement.ID, stores.RecipientDraftPatch{
		Email:        new("stage2-signer@example.com"),
		Name:         new("Stage Two"),
		Role:         new(stores.RecipientRoleSigner),
		SigningOrder: new(2),
	}, 0)
	...
	for _, signer := range []stores.RecipientRecord{signerOne, signerTwo, signerThree} {
		_, err = agreementSvc.UpsertFieldDraft(ctx, scope, agreement.ID, stores.FieldDraftPatch{
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase24-session-stage-parallel"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceSubmitProgressesBySigningStage(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase24-submit-stage-progression"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	signStage := func(recipient stores.RecipientRecord, objectKeySuffix, digest, submitKey string) SignerSubmitResult {
		...
		_, err = signingSvc.CaptureConsent(ctx, scope, token, SignerConsentInput{Accepted: true})
		if err != nil {
			t.Fatalf("CaptureConsent %s: %v", recipient.ID, err)
		}
		_, err = signingSvc.AttachSignatureArtifact(ctx, scope, token, SignerSignatureInput{
...
func TestSigningServiceSubmitTriggersStageWorkflowOnStageAdvance(t *testing.T) {
	...
	signerOne, err := agreementSvc.UpsertRecipientDraft(ctx, scope, agreement.ID, stores.RecipientDraftPatch{
		Email:        new("stage1-signer@example.com"),
		Role:         new(stores.RecipientRoleSigner),
		SigningOrder: new(1),
	}, 0)
	if err != nil {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase24-stage-workflow-send"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceDeclineMixedStageTerminalBehavior(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase24-decline-mixed-stage"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = signingSvc.Decline(ctx, scope, waitingToken, SignerDeclineInput{Reason: "too early"})
	if err == nil {
...
func TestSigningServiceGetSessionIncludesUnifiedGeometryAndBootstrapMetadata(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase18-session-bootstrap"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceGetSessionSupportsPreviewFallbackKillSwitch(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase-slice1-preview-fallback"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = manager.UploadFile(ctx, document.SourceObjectKey, samplePDF(2), uploader.WithContentType("application/pdf"))
	if err != nil {
...
func TestSigningServiceGetSessionDegradesWhenSourceUnavailableWithoutObjectStore(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase-slice7-source-unavailable-block"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceGetSessionReportsFullCompatibilityWhenSourceImportSucceeds(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase-slice1-full-compat"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceGetSessionPrefersNormalizedSourceWhenOriginalUnavailable(t *testing.T) {
	...
	_, err = manager.UploadFile(ctx, document.SourceObjectKey, []byte("not-a-pdf"), uploader.WithContentType("application/pdf"))
	if err != nil {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase-slice4-prefer-normalized"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceGetSessionStrictModeReturnsUnsupportedWhenNormalizedUnavailable(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase-slice4-strict-no-fallback"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceGetSessionNormalizesFieldGeometryToCanonicalPageSpace(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase19-coordinate-normalization"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceTracksRecipientViewLifecycle(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase4-view"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceCaptureConsentAndUpsertFieldValue(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase4-consent-upsert"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = signingSvc.UpsertFieldValue(ctx, scope, token, SignerFieldValueInput{
...
func TestSigningServiceRejectsWaitingSignerConsentAndFieldUpsert(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase4-waiting"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceAttachSignatureArtifact(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase4-signature-attach"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceIssueSignatureUploadBootstrap(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase19-signature-upload-bootstrap"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceIssueSignatureUploadRespectsConfiguredTTLPolicy(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase19-signature-upload-ttl-policy"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceIssueAndAttachDrawnInitials(t *testing.T) {
	...
	initialsField, err := agreementSvc.UpsertFieldDraft(ctx, scope, agreement.ID, stores.FieldDraftPatch{
		RecipientID: &signer.ID,
		Type:        new(stores.FieldTypeInitials),
		PageNumber:  new(1),
		Required:    new(true),
	})
	if err != nil {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase19-initials-drawn"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = signingSvc.ConfirmSignatureUpload(ctx, scope, SignerSignatureUploadCommitInput{
...
func TestSigningServiceAttachSignatureArtifactDrawnVerifiesBootstrapAndRetries(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase19-signature-upload-verify"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceConfirmSignatureUploadPersistsPayloadToObjectStore(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase19-signature-upload-persist"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceAttachSignatureArtifactDrawnRecoversReceiptFromAudit(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase19-signature-upload-audit-recovery"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = signingSvcA.ConfirmSignatureUpload(ctx, scope, SignerSignatureUploadCommitInput{
		UploadToken: contract.UploadToken,
		ObjectKey:   contract.ObjectKey,
		SHA256:      contract.SHA256,
		ContentType: contract.ContentType,
...
func TestSigningServiceAttachSignatureArtifactDrawnRecoversReceiptFromObjectStore(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase19-signature-upload-object-recovery"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceSubmitWithIdempotencyAndCAS(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase4-submit"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceSubmitAllowsConsentCapturedBeforeServiceRestart(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase-v2-consent-restart"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = firstService.CaptureConsent(ctx, scope, token, SignerConsentInput{Accepted: true})
	if err != nil {
		t.Fatalf("CaptureConsent: %v", err)
	}
	_, err = firstService.AttachSignatureArtifact(ctx, scope, token, SignerSignatureInput{
		FieldID:   signatureField.ID,
		Type:      "typed",
	...
	_, err = firstService.UpsertFieldValue(ctx, scope, token, SignerFieldValueInput{
		FieldID:   textField.ID,
		ValueText: "Signer Name",
	})
	if err != nil {
...
func TestSigningServiceSubmitReplayPersistsAcrossServiceRestart(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase-v2-submit-replay"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceSubmitEnqueuesCompletionWorkflowOutboxWhenConfigured(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase5-submit-outbox-completion"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceSubmitEnqueuesStageActivationWorkflowOutboxWhenConfigured(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase5-submit-outbox-stage"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceSubmitPersistsWhenCompletionWorkflowFails(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase5-submit-failing-workflow"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceDeclineFlow(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase4-decline"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = signingSvc.Decline(ctx, scope, token, SignerDeclineInput{})
	if err == nil {
...
func TestSigningServiceSignerAuditEventsIncludeActorMetadata(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "phase4-audit"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = signingSvc.GetSession(ctx, scope, token)
	if err != nil {
		t.Fatalf("GetSession for view snapshot: %v", err)
	}
	_, err = signingSvc.CaptureConsent(ctx, scope, token, SignerConsentInput{Accepted: true, IPAddress: actorIP, UserAgent: actorUA})
	if err != nil {
		t.Fatalf("CaptureConsent: %v", err)
	}
	_, err = signingSvc.AttachSignatureArtifact(ctx, scope, token, SignerSignatureInput{
	...
	_, err = signingSvc.Submit(ctx, scope, token, SignerSubmitInput{
...
func TestSigningServiceGetSessionIncludesReviewContext(t *testing.T) {
	...
	signer, err := agreementSvc.UpsertRecipientDraft(ctx, scope, agreement.ID, stores.RecipientDraftPatch{
		Email:        new("review.session@example.com"),
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "review-session-send"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceGetReviewSessionSupportsExternalReviewerToken(t *testing.T) {
	...
	signer, err := agreementSvc.UpsertRecipientDraft(ctx, scope, agreement.ID, stores.RecipientDraftPatch{
		Email:        new("external.review.signer@example.com"),
		Name:         new("External Review Signer"),
		Role:         new(stores.RecipientRoleSigner),
		SigningOrder: new(1),
	}, 0)
	if err != nil {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "external-review-send"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSigningServiceSessionIncludesReviewApprovalCounts(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "review-count-send"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	_, err = agreementSvc.ApproveReview(ctx, scope, agreement.ID, ReviewDecisionInput{
...
func TestSigningServiceSessionIncludesReviewOverrideState(t *testing.T) {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "override-session-send"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	_, err = agreementSvc.ForceApproveReview(ctx, scope, agreement.ID, ReviewOverrideInput{
		ActorType: "user",
		ActorID:   "ops-admin",
		ActorName: "Ops Admin",
...
func TestSigningServicePublicReviewSessionFiltersInternalThreadsAndCarriesActorMap(t *testing.T) {
	...
	_, err = agreementSvc.CreateCommentThread(ctx, scope, agreement.ID, ReviewCommentThreadInput{
...
func TestSigningServiceSubmitBlockedUntilApproveBeforeSignReviewCompletes(t *testing.T) {
	...
	signer, err := agreementSvc.UpsertRecipientDraft(ctx, scope, agreement.ID, stores.RecipientDraftPatch{
		Email:        new("review.submit@example.com"),
		Name:         new("Review Submit"),
		Role:         new(stores.RecipientRoleSigner),
		SigningOrder: new(1),
	}, 0)
	if err != nil {
	...
	_, err = agreementSvc.Send(ctx, scope, agreement.ID, SendInput{IdempotencyKey: "review-submit-send"})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
