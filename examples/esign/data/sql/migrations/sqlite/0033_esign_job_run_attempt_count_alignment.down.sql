...
SELECT
    id, tenant_id, org_id, job_name, dedupe_key, agreement_id, recipient_id, correlation_id,
    status, CASE WHEN attempt_count < 1 THEN 1 ELSE attempt_count END, max_attempts, last_error,
    next_retry_at, created_at, updated_at, payload_json, available_at, started_at, completed_at,
    claimed_at, lease_expires_at, worker_id, resource_kind, resource_id, last_error_code
FROM job_runs_old;
