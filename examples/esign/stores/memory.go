...
type InMemoryStore struct {
	...
	publicSignerSessionTokens    map[string]PublicSignerSessionTokenRecord
	publicSignerSessionHashIndex map[string]string
...
func (s *InMemoryStore) snapshot() (inMemoryStoreSnapshot, error) {
	...
	payload := inMemoryStoreSnapshot{
		...
		PublicSignerSessionTokens:    s.publicSignerSessionTokens,
		PublicSignerSessionHashIndex: s.publicSignerSessionHashIndex,
...
func (s *InMemoryStore) applySigningSnapshot(snapshot inMemoryStoreSnapshot) {
	...
	s.publicSignerSessionTokens = ensurePublicSignerSessionTokenMap(snapshot.PublicSignerSessionTokens)
	s.publicSignerSessionHashIndex = ensureStringMap(snapshot.PublicSignerSessionHashIndex)
...
func clonePublicSignerSessionTokenRecord(record PublicSignerSessionTokenRecord) PublicSignerSessionTokenRecord {
	record.RevokedAt = cloneTimePtr(record.RevokedAt)
	record.ExpiresAt = normalizeRecordTime(record.ExpiresAt)
	record.CreatedAt = normalizeRecordTime(record.CreatedAt)
	return record
...
func (s *InMemoryStore) CreatePublicSignerSessionToken(ctx context.Context, scope Scope, record PublicSignerSessionTokenRecord) (PublicSignerSessionTokenRecord, error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, exists := s.publicSignerSessionTokens[key]; exists {
		return PublicSignerSessionTokenRecord{}, invalidRecordError("public_signer_session_tokens", "id", "already exists")
	}
	if _, exists := s.publicSignerSessionHashIndex[record.TokenHash]; exists {
...
func (s *InMemoryStore) GetPublicSignerSessionToken(ctx context.Context, scope Scope, id string) (PublicSignerSessionTokenRecord, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return PublicSignerSessionTokenRecord{}, err
	...
	id = normalizeID(id)
	if id == "" {
		return PublicSignerSessionTokenRecord{}, invalidRecordError("public_signer_session_tokens", "id", "required")
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
	record, ok := s.publicSignerSessionTokens[scopedKey(scope, id)]
	if !ok {
		return PublicSignerSessionTokenRecord{}, notFoundError("public_signer_session_tokens", id)
	}
	return clonePublicSignerSessionTokenRecord(record), nil
}

func (s *InMemoryStore) GetPublicSignerSessionTokenByHash(ctx context.Context, scope Scope, tokenHash string) (PublicSignerSessionTokenRecord, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return PublicSignerSessionTokenRecord{}, err
...
func (s *InMemoryStore) ListPublicSignerSessionTokens(ctx context.Context, scope Scope, agreementID, recipientID, participantID string) ([]PublicSignerSessionTokenRecord, error) {
	...
	participantID = normalizeID(participantID)
	if agreementID == "" {
		return nil, invalidRecordError("public_signer_session_tokens", "agreement_id", "required")
	}
	if recipientID == "" && participantID == "" {
		return nil, invalidRecordError("public_signer_session_tokens", "recipient_id|participant_id", "required")
	}
	out := make([]PublicSignerSessionTokenRecord, 0)
	s.mu.RLock()
	defer s.mu.RUnlock()
	for _, record := range s.publicSignerSessionTokens {
		if record.TenantID != scope.TenantID || record.OrgID != scope.OrgID {
			continue
		}
		if record.AgreementID != agreementID {
		...
		if recipientID != "" && record.RecipientID != recipientID {
			continue
		}
		if participantID != "" && record.ParticipantID != participantID {
			continue
		}
		out = append(out, clonePublicSignerSessionTokenRecord(record))
	}
	sort.Slice(out, func(i, j int) bool {
		if out[i].CreatedAt.Equal(out[j].CreatedAt) {
			return out[i].ID < out[j].ID
...
func (s *InMemoryStore) SavePublicSignerSessionToken(ctx context.Context, scope Scope, record PublicSignerSessionTokenRecord) (PublicSignerSessionTokenRecord, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return PublicSignerSessionTokenRecord{}, err
...
func (s *InMemoryStore) RevokeActivePublicSignerSessionTokens(ctx context.Context, scope Scope, agreementID, recipientID, participantID string, revokedAt time.Time) (int, error) {
	...
	participantID = normalizeID(participantID)
	if agreementID == "" {
		return 0, invalidRecordError("public_signer_session_tokens", "agreement_id", "required")
	}
	if recipientID == "" && participantID == "" {
		return 0, invalidRecordError("public_signer_session_tokens", "recipient_id|participant_id", "required")
	}
	if revokedAt.IsZero() {
		revokedAt = time.Now().UTC()
	}
	...
	defer s.mu.Unlock()
	revoked := 0
	for key, token := range s.publicSignerSessionTokens {
		if token.TenantID != scope.TenantID || token.OrgID != scope.OrgID {
			continue
		}
		if token.AgreementID != agreementID {
			continue
		}
		if recipientID != "" && token.RecipientID != recipientID {
			continue
		}
		if participantID != "" && token.ParticipantID != participantID {
			continue
		}
		if token.Status != PublicSignerSessionTokenStatusActive {
			continue
		}
		token.Status = PublicSignerSessionTokenStatusRevoked
		token.RevokedAt = cloneTimePtr(&revokedAt)
		s.publicSignerSessionTokens[key] = token
		revoked++
	}
	return revoked, nil
}
...
func cloneJobRunRecord(record JobRunRecord) JobRunRecord {
	record.AvailableAt = cloneTimePtr(record.AvailableAt)
	...
	record.LeaseExpiresAt = cloneTimePtr(record.LeaseExpiresAt)
	record.NextRetryAt = cloneTimePtr(record.NextRetryAt)
	record.CreatedAt = normalizeRecordTime(record.CreatedAt)
	record.UpdatedAt = normalizeRecordTime(record.UpdatedAt)
	return record
...
func (s *InMemoryStore) ClaimDueJobs(ctx context.Context, scope Scope, input JobRunClaimInput) ([]JobRunRecord, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return nil, err
	...
	sort.SliceStable(candidates, func(i, j int) bool {
		...
		case !candidates[i].UpdatedAt.Equal(candidates[j].UpdatedAt):
...
func (s *InMemoryStore) RenewJobLease(ctx context.Context, scope Scope, id string, input JobRunLeaseRenewInput) (JobRunRecord, error) {
	...
	workerID := strings.TrimSpace(input.WorkerID)
	if workerID == "" {
		return JobRunRecord{}, invalidRecordError("job_runs", "worker_id", "required")
	}
	...
	if record.Status != JobRunStatusRunning {
		return JobRunRecord{}, invalidRecordError("job_runs", "status", "lease renewal requires running job")
	}
	if strings.TrimSpace(record.WorkerID) != "" && strings.TrimSpace(record.WorkerID) != workerID {
		return JobRunRecord{}, invalidRecordError("job_runs", "worker_id", "lease held by another worker")
	}
	record.WorkerID = workerID
	record.ClaimedAt = cloneTimePtr(&renewedAt)
...
func (s *InMemoryStore) MarkJobSucceeded(ctx context.Context, scope Scope, id string, completedAt time.Time) (JobRunRecord, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return JobRunRecord{}, err
...
func (s *InMemoryStore) MarkJobFailed(ctx context.Context, scope Scope, id string, input JobRunFailureInput) (JobRunRecord, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return JobRunRecord{}, err
	...
	if input.NextAvailableAt != nil && record.AttemptCount < record.MaxAttempts {
		...
		record.NextRetryAt = nil
		record.AvailableAt = nil
		record.CompletedAt = cloneTimePtr(&failedAt)
	}
	record.UpdatedAt = failedAt
	record = cloneJobRunRecord(record)
	s.jobRuns[key] = record
...
func (s *InMemoryStore) MarkJobStale(ctx context.Context, scope Scope, id, reason string, staleAt time.Time) (JobRunRecord, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return JobRunRecord{}, err
	...
	record.NextRetryAt = nil
	record.AvailableAt = nil
	record.CompletedAt = cloneTimePtr(&staleAt)
...
func (s *InMemoryStore) RequeueStaleJobs(ctx context.Context, scope Scope, input JobRunRequeueInput) (int, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return 0, err
...
func (s *InMemoryStore) ListJobRunsByResource(ctx context.Context, scope Scope, resourceKind, resourceID string) ([]JobRunRecord, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return nil, err
	...
	if resourceKind == "" || resourceID == "" {
		return nil, invalidRecordError("job_runs", "resource_kind|resource_id", "required")
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	for _, record := range s.jobRuns {
		...
		if strings.TrimSpace(record.ResourceKind) != resourceKind || strings.TrimSpace(record.ResourceID) != resourceID {
			continue
		}
		out = append(out, cloneJobRunRecord(record))
	}
...
func jobRunDueAt(record JobRunRecord) *time.Time {
	if record.AvailableAt != nil {
		return cloneTimePtr(record.AvailableAt)
	}
	if record.NextRetryAt != nil {
		return cloneTimePtr(record.NextRetryAt)
	}
	return nil
}
...
func normalizeAgreementReminderStatus(status string) string {
	...
	case AgreementReminderStatusTerminal:
		return AgreementReminderStatusTerminal
	default:
		return ""
	}
}

func normalizeReminderPolicyVersion(version string) string {
	version = strings.TrimSpace(version)
	if version == "" {
		return defaultReminderPolicyVersion
	}
	return version
}

func normalizeReminderReasonCode(reasonCode string) string {
...
func reminderLeaseHeartbeat(record AgreementReminderStateRecord) *time.Time {
...
func reminderLeaseIsActive(record AgreementReminderStateRecord, now time.Time, leaseSeconds int) bool {
	if normalizeID(record.WorkerID) == "" || record.LeaseSeq <= 0 {
		return false
	}
	...
	if leaseSeconds <= 0 {
		leaseSeconds = defaultReminderLeaseSeconds
	}
	return hb.Add(time.Duration(leaseSeconds) * time.Second).After(now)
}

func normalizeReminderLeaseToken(token AgreementReminderLeaseToken) AgreementReminderLeaseToken {
	token.WorkerID = normalizeID(token.WorkerID)
...
func validateReminderLease(record AgreementReminderStateRecord, token AgreementReminderLeaseToken, asOf time.Time, leaseSeconds int) error {
	token = normalizeReminderLeaseToken(token)
	if token.WorkerID == "" {
		return invalidRecordError("agreement_reminder_states", "worker_id", "required")
	}
	if token.SweepID == "" {
		return invalidRecordError("agreement_reminder_states", "sweep_id", "required")
	}
	if token.LeaseSeq <= 0 {
		return invalidRecordError("agreement_reminder_states", "lease_seq", "required")
	}
	if normalizeID(record.WorkerID) == "" || record.LeaseSeq <= 0 {
		return reminderLeaseLostError(record.AgreementID, record.RecipientID, token.WorkerID)
	}
	if normalizeID(record.WorkerID) != token.WorkerID || strings.TrimSpace(record.SweepID) != token.SweepID {
		return reminderLeaseLostError(record.AgreementID, record.RecipientID, token.WorkerID)
	}
	if record.LeaseSeq != token.LeaseSeq {
		return reminderLeaseConflictError(record.AgreementID, record.RecipientID, token.WorkerID, token.LeaseSeq, record.LeaseSeq)
	}
	if !reminderLeaseIsActive(record, asOf, leaseSeconds) {
		return reminderLeaseLostError(record.AgreementID, record.RecipientID, token.WorkerID)
	}
	return nil
}

func clearReminderLease(record *AgreementReminderStateRecord) {
	if record == nil {
		return
	}
	record.WorkerID = ""
...
func reminderClaimFromRecord(record AgreementReminderStateRecord) AgreementReminderClaim {
	record = cloneAgreementReminderStateRecord(record)
	return AgreementReminderClaim{
		State: record,
...
func (s *InMemoryStore) UpsertAgreementReminderState(ctx context.Context, scope Scope, record AgreementReminderStateRecord) (AgreementReminderStateRecord, error) {
	...
	record.Status = status
	record.PolicyVersion = normalizeReminderPolicyVersion(record.PolicyVersion)
	record.LastReasonCode = normalizeReminderReasonCode(record.LastReasonCode)
	record.LastErrorCode = normalizeReminderReasonCode(record.LastErrorCode)
	record.TerminalReason = normalizeReminderReasonCode(record.TerminalReason)
	record.SweepID = strings.TrimSpace(record.SweepID)
	record.WorkerID = normalizeID(record.WorkerID)
	record.LeaseSeq = max(record.LeaseSeq, 0)
	record.LastErrorInternalEncrypted = strings.TrimSpace(record.LastErrorInternalEncrypted)
	record.LastErrorInternalExpiresAt = cloneTimePtr(record.LastErrorInternalExpiresAt)
	record.ClaimedAt = cloneTimePtr(record.ClaimedAt)
	...
	if record.Status != AgreementReminderStatusTerminal {
		record.TerminalReason = ""
	}
	...
	if exists && record.LeaseSeq == 0 {
		record.LeaseSeq = max(existing.LeaseSeq, 0)
	}
	if record.CreatedAt.IsZero() {
...
func (s *InMemoryStore) ClaimDueAgreementReminders(ctx context.Context, scope Scope, input AgreementReminderClaimInput) ([]AgreementReminderClaim, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return nil, err
...
func (s *InMemoryStore) RenewAgreementReminderLease(
	...
	agreementID,
	recipientID string,
	input AgreementReminderLeaseRenewInput,
) (AgreementReminderClaim, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
	...
	recipientID = normalizeID(recipientID)
	if agreementID == "" {
		return AgreementReminderClaim{}, invalidRecordError("agreement_reminder_states", "agreement_id", "required")
	}
	if recipientID == "" {
		return AgreementReminderClaim{}, invalidRecordError("agreement_reminder_states", "recipient_id", "required")
	}
	if input.Now.IsZero() {
		input.Now = time.Now().UTC()
	}
	...
	record, ok := s.agreementReminderStates[key]
	if !ok {
		return AgreementReminderClaim{}, notFoundError("agreement_reminder_states", agreementID+"|"+recipientID)
	}
...
func (s *InMemoryStore) PauseAgreementReminder(ctx context.Context, scope Scope, agreementID, recipientID string, pausedAt time.Time) (AgreementReminderStateRecord, error) {
	...
	if record.Status == AgreementReminderStatusTerminal {
		return AgreementReminderStateRecord{}, reminderStateInvariantError("status", "cannot_pause_terminal")
	}
...
func (s *InMemoryStore) ResumeAgreementReminder(ctx context.Context, scope Scope, agreementID, recipientID string, resumedAt time.Time, nextDueAt *time.Time) (AgreementReminderStateRecord, error) {
	...
	if record.Status == AgreementReminderStatusTerminal {
		return AgreementReminderStateRecord{}, reminderStateInvariantError("status", "cannot_resume_terminal")
	}
	nextDueAt = cloneTimePtr(nextDueAt)
	if nextDueAt == nil {
		return AgreementReminderStateRecord{}, reminderStateInvariantError("next_due_at", "active_requires_next_due_at")
	}
	record.Status = AgreementReminderStatusActive
...
func (s *InMemoryStore) CleanupAgreementReminderInternalErrors(ctx context.Context, scope Scope, now time.Time, limit int) (int, error) {
	_ = ctx
	scope, err := validateScope(scope)
	if err != nil {
		return 0, err
	...
	cleared := 0
	for key, record := range s.agreementReminderStates {
		if record.TenantID != scope.TenantID || record.OrgID != scope.OrgID {
			continue
		}
		if strings.TrimSpace(record.LastErrorInternalEncrypted) == "" {
			continue
		}
		if record.LastErrorInternalExpiresAt == nil || record.LastErrorInternalExpiresAt.After(now) {
			continue
		}
		record.LastErrorInternalEncrypted = ""
		record.LastErrorInternalExpiresAt = nil
