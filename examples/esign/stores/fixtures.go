...
func SeedCoreFixtures(ctx context.Context, db bun.IDB, scope Scope) (FixtureSet, error) {
	...
	if err := seedCoreFixtureAgreementRecords(ctx, db, scope, fx, now); err != nil {
		return FixtureSet{}, err
	}
	if err := seedCoreFixtureFieldRecords(ctx, db, scope, fx, now); err != nil {
		return FixtureSet{}, err
	}
	if err := seedCoreFixtureAuditRecords(ctx, db, scope, fx, now); err != nil {
		return FixtureSet{}, err
	}
...
func seedCoreFixtureAgreementRecords(ctx context.Context, db bun.IDB, scope Scope, fx FixtureSet, now time.Time) error {
...
func seedCoreFixtureFieldRecords(ctx context.Context, db bun.IDB, scope Scope, fx FixtureSet, now time.Time) error {
	if _, err := db.ExecContext(ctx, `
...
func seedCoreFixtureAuditRecords(ctx context.Context, db bun.IDB, scope Scope, fx FixtureSet, now time.Time) error {
	if _, err := db.ExecContext(ctx, `
...
func SeedLineageFixtures(ctx context.Context, db bun.IDB, scope Scope) (LineageFixtureSet, error) {
	...
	if err := seedLineagePrimaryRecords(ctx, db, scope, fx, now, secondNow); err != nil {
		return LineageFixtureSet{}, err
	}
	if err := seedLineageRevisionRecords(ctx, db, scope, fx, now, secondNow); err != nil {
		return LineageFixtureSet{}, err
	}
	if err := seedLineageCandidateRecords(ctx, db, scope, fx, secondNow); err != nil {
		return LineageFixtureSet{}, err
	}
...
func seedLineagePrimaryRecords(ctx context.Context, db bun.IDB, scope Scope, fx LineageFixtureSet, now, secondNow time.Time) error {
	if _, err := db.ExecContext(ctx, `
...
func seedLineageRevisionRecords(ctx context.Context, db bun.IDB, scope Scope, fx LineageFixtureSet, now, secondNow time.Time) error {
	if _, err := db.ExecContext(ctx, `
	...
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, fx.FirstSourceRevisionID, scope.TenantID, scope.OrgID, fx.SourceDocumentID, fx.LegacySourceHandleID, "v1", now, now, "fixture-user", "application/vnd.google-apps.document", `{"origin":"native_google_import","revision_signature":"sig-v1"}`, now, now); err != nil {
		return err
	}
	if _, err := db.ExecContext(ctx, `
...
`, fx.FirstSourceArtifactID, scope.TenantID, scope.OrgID, fx.FirstSourceRevisionID, "fixtures/google-v1.pdf", strings.Repeat("2", 64), 3, 4096, "full", "", "completed", now, now); err != nil {
	...
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'google_drive', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, fx.ImportedDocumentID, scope.TenantID, scope.OrgID, "Imported Fixture Source", "Imported Fixture Source.pdf", "fixtures/google-v1.pdf", "fixtures/google-v1.normalized.pdf", strings.Repeat("2", 64), 4096, 3, "google-file-legacy", "https://docs.google.com/document/d/google-file-legacy/edit", "application/vnd.google-apps.document", "google_export_pdf", fx.SourceDocumentID, fx.FirstSourceRevisionID, fx.FirstSourceArtifactID, "fixture-user", now, now); err != nil {
		return err
	}
	if _, err := db.ExecContext(ctx, `
	...
	`, fx.ImportedAgreementID, scope.TenantID, scope.OrgID, fx.ImportedDocumentID, "Imported Fixture Agreement", "Fixture", "google-file-legacy", "https://docs.google.com/document/d/google-file-legacy/edit", "application/vnd.google-apps.document", "google_export_pdf", fx.FirstSourceRevisionID, "fixture-user", "fixture-user", now, now); err != nil {
		return err
	}
	if _, err := db.ExecContext(ctx, `
INSERT INTO source_revisions (id, tenant_id, org_id, source_document_id, source_handle_id, provider_revision_hint, modified_time, exported_at, exported_by_user_id, source_mime_type, metadata_json, created_at, updated_at)
...
func seedLineageCandidateRecords(ctx context.Context, db bun.IDB, scope Scope, fx LineageFixtureSet, secondNow time.Time) error {
	if _, err := db.ExecContext(ctx, `
INSERT INTO source_documents (id, tenant_id, org_id, provider_kind, canonical_title, status, lineage_confidence, created_at, updated_at)
