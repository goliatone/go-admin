...
function setPreview(elements, src) {
    if (!elements.previewImg || !elements.placeholder)
        return;
    if (src) {
...
function ensureInlineElement(root, selector, className) {
...
function resolveElements(root) {
    const urlInput = root.querySelector('input[data-file-uploader-url]');
    const fileInput = root.querySelector('input[data-file-uploader-file]');
    ...
    const previewImg = root.querySelector('[data-file-uploader-preview]');
    const placeholder = root.querySelector('[data-file-uploader-placeholder]');
    return {
        root,
        urlInput,
        fileInput,
...
async function uploadFile(elements, endpoint, file) {
...
export function initFileUploaders(scope = document) {
    const roots = Array.from(scope.querySelectorAll('[data-component="file_uploader"], [data-file-uploader]'));
