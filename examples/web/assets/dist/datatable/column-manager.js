...
export class ColumnManager {
    ...
    handleReset() {
        ...
        if (typeof this.grid.resetColumnsToDefault === 'function') {
     ...
     */
    createColumnItem(field, label, isVisible) {
    ...
    setupDragAndDrop() {
        ...
        this.sortable = Sortable.create(columnList, {
            animation: 150,
            handle: '.drag-handle',
     ...
     */
    bindSwitchToggles() {
        const switches = this.container.querySelectorAll('input[type="checkbox"]');
     ...
     */
    updateSwitchState(field, isVisible) {
        const checkbox = this.container.querySelector(`input[type="checkbox"][data-column="${field}"]`);
        if (checkbox) {
            checkbox.checked = isVisible;
            checkbox.setAttribute('aria-checked', String(isVisible));
            // Update parent item aria-selected
