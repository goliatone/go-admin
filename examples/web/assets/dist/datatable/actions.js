...
export class ActionRenderer {
     ...
     */
    renderRowActions(record, actions) {
        if (this.mode === 'dropdown') {
            return this.renderRowActionsDropdown(record, actions);
        }
        // Inline mode (existing behavior)
        const visibleActions = actions.filter(action => !action.condition || action.condition(record));
     ...
     */
    renderRowActionsDropdown(record, actions) {
        const visibleActions = actions.filter(action => !action.condition || action.condition(record));
        if (visibleActions.length === 0) {
     ...
     */
    buildDropdownItems(record, actions) {
     ...
     */
    shouldShowDivider(action, index, actions) {
        if (index === 0)
            return false;
        ...
        return dividerBefore.some(label => action.label.toLowerCase().includes(label));
    }
    /**
     ...
     */
    renderDefaultActions(record, basePath) {
     ...
     */
    attachRowActionListeners(container, actions, records) {
        actions.forEach(action => {
            const buttons = container.querySelectorAll(`[data-action-id="${this.sanitize(action.label)}"]`);
     ...
     */
    renderBulkActionsToolbar(bulkActions) {
     ...
     * Execute bulk action
     */
    async executeBulkAction(config, selectedIds) {
