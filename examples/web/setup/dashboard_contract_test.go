package setup

import (
	"testing"

	"github.com/goliatone/go-admin/pkg/admin"
)

func TestValidateCanonicalWidgetPayload_Definitions(t *testing.T) {
	cases := []struct {
		name       string
		definition string
		model      any
	}{
		{
			name:       "legacy chart sample",
			definition: admin.WidgetChartSample,
			model:      LegacyChartSampleWidgetData{Disabled: true},
		},
		{
			name:       "user stats",
			definition: admin.WidgetUserStats,
			model: UserStatsWidgetData{
				Type:     "stat_card",
				StatType: "users",
				Total:    10,
				Active:   6,
				NewToday: 2,
				Trend:    "+12%",
				TrendUp:  true,
			},
		},
		{
			name:       "content stats",
			definition: admin.WidgetContentStats,
			model: ContentStatsWidgetData{
				Type:      "stat_card",
				StatType:  "content",
				Published: 7,
				Draft:     2,
				Scheduled: 1,
			},
		},
		{
			name:       "storage stats",
			definition: admin.WidgetStorageStats,
			model: StorageStatsWidgetData{
				Type:       "stat_card",
				StatType:   "storage",
				Used:       "21.4 GB",
				Total:      "100 GB",
				Percentage: 21,
			},
		},
		{
			name:       "quick actions",
			definition: admin.WidgetQuickActions,
			model: QuickActionsWidgetData{
				Actions: []QuickActionWidgetItem{{Label: "Invite", URL: "/admin/api/onboarding/invite", Method: "POST"}},
			},
		},
		{
			name:       "activity feed",
			definition: admin.WidgetActivityFeed,
			model:      ActivityFeedWidgetData{Entries: []admin.ActivityEntry{}},
		},
		{
			name:       "user profile",
			definition: admin.WidgetUserProfileOverview,
			model:      UserProfileOverviewWidgetData{Values: map[string]string{"Username": "superadmin"}},
		},
		{
			name:       "system health",
			definition: admin.WidgetSystemHealth,
			model: SystemHealthWidgetData{
				Status:     "healthy",
				Uptime:     "7d 12h",
				APILatency: "45ms",
				DBStatus:   "connected",
			},
		},
		{
			name:       "chart",
			definition: admin.WidgetBarChart,
			model: ChartWidgetData{
				ChartType:       "bar",
				Title:           "Monthly Content Creation",
				Theme:           "westeros",
				ChartAssetsHost: "/dashboard/assets/echarts/",
				ChartOptions: buildChartOptions("bar", map[string]any{
					"title":  "Monthly Content Creation",
					"x_axis": []string{"Jan", "Feb"},
					"series": []map[string]any{{"name": "Posts", "data": []float64{1, 2}}},
				}),
			},
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if err := validateCanonicalWidgetModel(tc.definition, tc.model); err != nil {
				t.Fatalf("expected canonical payload to validate for %s: %v", tc.definition, err)
			}
		})
	}
}

func TestValidateCanonicalWidgetPayload_RejectsLegacyMarkup(t *testing.T) {
	type legacyChartPayload struct {
		ChartType       string         `json:"chart_type"`
		Title           string         `json:"title"`
		Theme           string         `json:"theme"`
		ChartAssetsHost string         `json:"chart_assets_host"`
		ChartOptions    map[string]any `json:"chart_options"`
		ChartHTML       string         `json:"chart_html"`
	}
	payload := legacyChartPayload{
		ChartType:       "bar",
		Title:           "Bad payload",
		Theme:           "westeros",
		ChartAssetsHost: "/dashboard/assets/echarts/",
		ChartOptions:    map[string]any{"series": []any{}},
		ChartHTML:       "<html><body><script>alert(1)</script></body></html>",
	}

	if err := validateCanonicalWidgetModel(admin.WidgetBarChart, payload); err == nil {
		t.Fatalf("expected canonical validator to reject legacy chart_html payload")
	}
}
