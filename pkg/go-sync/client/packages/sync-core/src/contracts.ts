...
export type SyncResourceStatus =
  | "idle"
  | "loading"
  | "ready"
  | "refreshing"
...
export type BoundMutationRequest<P = unknown> =
  Omit<MutationRequest<P>, "ref"> & {
    ref?: ResourceRef;
  };
...
export interface SyncResource<T, P = unknown> {
  ...
  refresh(options?: { force?: boolean }): Promise<ResourceSnapshot<T>>;
}
