...
import { createLogger as L } from "../shared/logger.js";
...
import { t as j } from "./modal-nXs4C8ko.js";
var le = { async prompt(e) {
...
}, ce = Object.fromEntries(Object.entries(C).map(([e, n]) => [e, n.tone])), P = {
  healthy: "success",
  ok: "success",
  warning: "warning",
  critical: "error",
...
function ue(e, n = "translation") {
  const t = x(e);
  return n === "alert" ? P[t] || "neutral" : C[t]?.tone || "neutral";
}
function R(e) {
  return x(e).split("_").filter(Boolean).map((n) => n.charAt(0).toUpperCase() + n.slice(1)).join(" ");
}
function U(e, n = "badge") {
...
function Y(e) {
  ...
  const n = e;
  if (!F(n)) return null;
  const t = k({ reason_code: n.reason_code }), r = { enabled: typeof n.enabled == "boolean" ? n.enabled : !1 }, a = l(n.reason), s = l(n.severity), d = l(n.kind), g = l(n.permission), b = n.metadata && typeof n.metadata == "object" && !Array.isArray(n.metadata) ? n.metadata : null, f = V(n.remediation), M = H(n.available_transitions);
...
function G(e) {
  if (!u(e)) return null;
  const n = e.selection_sensitive === !0, t = l(e.selection_state_endpoint), r = q(e.debounce_ms);
...
function ge(e) {
  if (!u(e)) return null;
  const n = Array.isArray(e.data) ? e.data : Array.isArray(e.records) ? e.records : null, t = n && n.map((s) => I(s) ?? s), r = J(e.$meta), a = { ...e };
  if (t && (Array.isArray(e.data) && (a.data = t), Array.isArray(e.records) && (a.records = t)), r && (a.$meta = r), u(e.schema)) {
    const s = G(e.schema.bulk_action_state_config);
    s && (a.schema = {
      ...e.schema,
...
function i(e, n = {}) {
  const t = B(e), r = t?.tone ?? "neutral";
  return {
    label: t?.label ?? R(e),
    shortLabel: n.shortLabel,
    colorClass: U(r, "badge"),
    bgClass: X[r],
    textClass: W[r],
    borderClass: K[r],
...
function he(e) {
  switch (e) {
    ...
    case "exchange":
      return [...Object.keys(w), ...Object.keys(A)];
    default:
      return [];
  }
}
...
function Z(e, n = {}) {
  ...
  const { size: r = "default", showIcon: a = !0, showLabel: s = !0, extraClass: d = "" } = n, g = {
    ...
    sm: "px-2 py-0.5",
    default: ""
  }, b = a ? ee(t, r) : "", f = s ? `<span>${o(t.label)}</span>` : "";
...
function Ae(e = {}) {
  return (n) => typeof n != "string" || !n ? '<span class="text-gray-400">-</span>' : Z(n, e);
}
function Ce(e = {}) {
  return (n) => typeof n != "string" || !n ? "" : ne(n, e);
}
function xe(e) {
  e.schema_version !== 1 && Q.warn("[TranslationStatusVocabulary] Unknown schema version:", e.schema_version);
}
function ke() {
  return `
...
async function re(e) {
  ...
  return e.notifier?.confirm ? e.notifier.confirm(n, {
    title: t,
    confirmText: "Delete",
    cancelText: "Cancel"
  }) : j.confirm(n, {
...
async function Te(e) {
  ...
  const t = String(e.fallbackMessage || "Delete failed").trim() || "Delete failed", r = n.error || te(t), a = {
    ...r,
    message: $(r, t)
  };
...
//# sourceMappingURL=action-execution-DXtVRyWi.js.map