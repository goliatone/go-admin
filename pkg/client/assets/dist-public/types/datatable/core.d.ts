import type { DataGridConfig, DataGridState, DataGridCapabilities } from './core-types.js';
export type { DataGridCapabilities, DataGridConfig, DataGridPaginationLabels, DataGridPaginationMode, DataGridPaginationPresentation, DataGridPaginationPluralCategory, DataGridPaginationSummaryLabels, DataGridRenderState, } from './core-types.js';
export declare class DataGrid {
    private static readonly URL_KEY_SEARCH;
    private static readonly URL_KEY_PAGE;
    private static readonly URL_KEY_PER_PAGE;
    private static readonly URL_KEY_FILTERS;
    private static readonly URL_KEY_SORT;
    private static readonly URL_KEY_STATE;
    private static readonly URL_KEY_HIDDEN_COLUMNS;
    private static readonly URL_KEY_VIEW_MODE;
    private static readonly URL_KEY_EXPANDED_GROUPS;
    private static readonly MANAGED_URL_KEYS;
    private static readonly DEFAULT_MAX_URL_LENGTH;
    private static readonly DEFAULT_MAX_FILTERS_LENGTH;
    config: DataGridConfig;
    state: DataGridState;
    private selectors;
    private tableEl;
    private searchTimeout;
    private abortController;
    private dropdownAbortController;
    private actionMenuController;
    private selectionAbortController;
    private didRestoreColumnOrder;
    private shouldReorderDOMOnRestore;
    private actionRenderer;
    private cellRendererRegistry;
    private recordsById;
    private notifier;
    private columnManager;
    private defaultColumns;
    private lastSchema;
    private lastForm;
    private bulkActionState;
    private bulkActionStateConfig;
    private bulkActionStateDebounce;
    private bulkActionStateAbortController;
    private bulkActionStateRequestSeq;
    private refreshDrainPromise;
    private refreshInFlight;
    private refreshQueued;
    private refreshRequestSeq;
    private activeRefreshSeq;
    private stateStore;
    private hasURLStateOverrides;
    private hasPersistedHiddenColumnState;
    private hasPersistedColumnOrderState;
    constructor(config: DataGridConfig);
    init(): void;
    isCapabilityEnabled(capability: keyof DataGridCapabilities): boolean;
    private setRenderState;
    private refreshAfterStateHydration;
    private getURLStateConfig;
    private parseJSONArray;
    private applyPersistedStateSnapshot;
    private applyShareStateSnapshot;
    private buildPersistedStateSnapshot;
    private buildShareStateSnapshot;
    private persistStateSnapshot;
    private restoreStateFromURL;
    private applyRestoredState;
    private pushStateToURL;
    syncURL(): void;
    refresh(): Promise<void>;
    requestRefreshAfterCurrent(): void;
    isCurrentRefresh(requestSeq: number): boolean;
    private drainRefreshQueue;
    buildApiUrl(): string;
    buildQueryString(): string;
    private buildQueryParams;
    private getResponseTotal;
    private normalizePagination;
    resetPagination(): void;
    updateColumnVisibility(visibleColumns: string[], skipURLUpdate?: boolean): void;
    private syncColumnVisibilityCheckboxes;
    private renderData;
    private renderLoadingState;
    private renderErrorState;
    private renderFlatData;
    private renderGroupedData;
    private isGroupedViewActive;
    private fallbackGroupedMode;
    private handleGroupedModeStatusFallback;
    private handleGroupedModePayloadFallback;
    toggleGroup(groupId: string): void;
    setExpandedGroups(groupIDs: string[]): void;
    expandAllGroups(): void;
    collapseAllGroups(): void;
    private updateGroupVisibility;
    private updateGroupedRowsFromState;
    private isGroupExpandedByState;
    setViewMode(mode: any): void;
    getViewMode(): any;
    getGroupedData(): any;
    fetchDetail(id: string): Promise<{
        data: any;
        schema?: Record<string, any>;
        form?: Record<string, any>;
        tabs?: any[];
    }>;
    getSchema(): Record<string, any> | null;
    getForm(): Record<string, any> | null;
    getTabs(): any[];
    private normalizeDetailResponse;
    private resolveRendererOptions;
    private createTableRow;
    private sanitizeActionId;
    private handleDelete;
    private updatePaginationUI;
    private renderPaginationButtons;
    private bindSearchInput;
    private bindPerPageSelect;
    private bindFilterInputs;
    private bindColumnVisibility;
    private bindExportButtons;
    private bindSorting;
    private updateSortIndicators;
    private bindSelection;
    private updateSelectionBindings;
    private bindBulkActions;
    private bindOverflowMenu;
    private updateBulkActionsBar;
    private setBulkActionState;
    private applyBulkActionState;
    private syncBulkActionState;
    private bindBulkClearButton;
    private clearSelection;
    private positionDropdownMenu;
    private bindDropdownToggles;
    private showError;
    private notify;
    private confirmAction;
    private extractError;
    private parseDatasetStringArray;
    private parseDatasetObject;
    reorderColumns(newOrder: string[]): void;
    resetColumnsToDefault(): void;
    private mergeColumnOrder;
    private reorderTableColumns;
    private reorderRowCells;
    destroy(): void;
}
//# sourceMappingURL=core.d.ts.map