...
import { C as ds, S as us, T as hs, _ as ps, a as Y, b as fs, c as gs, d as bs, f as ms, g as vs, h as ys, i as M, l as ws, m as xs, n as Ss, o as $s, p as ks, r as Cs, s as Es, t as Ls, u as As, v as _s, w as Ts, x as Is, y as Ds } from "./chunks/translation-context--nekPn-6.js";
import { n as Ps, r as Ms, t as Bs } from "./chunks/schema-actions-CeNQvt5t.js";
import { n as zs, r as qs, t as js } from "./chunks/detail-actions-rQMb3jJU.js";
...
var m = v("DataGrid"), j = {
  ...
  constructor(e) {
    this.criteria = [], this.modal = null, this.container = null, this.searchInput = null, this.clearBtn = null, this.config = e, this.notifier = e.notifier || new K();
  }
  init() {
  ...
  normalizeCriteria(e) {
    ...
    return e.map((r) => {
      ...
      const i = String(r?.operator || "ilike").trim() || "ilike", a = r?.logic === "or" ? "or" : "and";
      return {
        field: s,
        operator: i,
  ...
  pushCriteriaToURL() {
    ...
    this.criteria.length > 0 ? e.set(E, JSON.stringify(this.criteria)) : (e.delete(E), e.delete(q)), he.forEach((r) => e.delete(r));
  ...
  createCriterionRow(e, t) {
    ...
    `, r.querySelectorAll("select, input").forEach((i) => {
      i.addEventListener("change", (a) => this.updateCriterion(a.target));
  ...
  createLogicConnector(e) {
    ...
    `, t.querySelectorAll("[data-logic-index]").forEach((s) => {
      s.addEventListener("click", (i) => {
        const a = i.target, o = parseInt(a.dataset.logicIndex || "0", 10), l = a.dataset.logicValue;
  ...
  render() {
    if (!this.container) {
      be.warn("[StatusLegend] Container not found");
      return;
    }
    this.container.innerHTML = this.buildHTML();
  }
  buildHTML() {
    const { title: e, orientation: t, size: r, items: s, containerClass: i } = this.config, a = t === "vertical", o = r === "sm", l = a ? "flex-col" : "flex-row flex-wrap", c = o ? "gap-2" : "gap-4", d = o ? "text-xs" : "text-sm", h = o ? "text-sm" : "text-base";
    return `
      <div class="status-legend inline-flex items-center ${l} ${c} ${i}"
           role="list"
           aria-label="Translation status legend">
        ${e ? `<span class="font-medium text-gray-600 dark:text-gray-400 mr-2 ${d}">${n(e)}</span>` : ""}
  ...
  renderItem(e, t, r) {
    return `
      <div class="status-legend-item inline-flex items-center gap-1"
           role="listitem"
           title="${n(e.description)}"
           aria-label="${n(e.label)}: ${n(e.description)}">
        <span class="${e.colorClass} ${t}" aria-hidden="true">${e.icon}</span>
        <span class="text-gray-600 dark:text-gray-400 ${r}">${n(e.label)}</span>
      </div>
    `;
  }
  setItems(e) {
...
async function ye(e, t, r = {}) {
  const { apiEndpoint: s, notifier: i = new K(), maxFailuresToShow: a = 5 } = e, o = `${s}/bulk/create-missing-translations`;
  try {
    const l = await x(o, {
      method: "POST",
      headers: {
    ...
    if (!l.ok) throw new Error(await de(l, `Request failed: ${l.status}`, { appendStatusToFallback: !1 }));
    const c = we(await l.json(), a);
    return xe(c, i), e.onSuccess && e.onSuccess(c), c;
  } catch (l) {
...
function we(e, t) {
  const r = e.data || [], s = e.created_count ?? r.filter((o) => o.success).length, i = e.failed_count ?? r.filter((o) => !o.success).length, a = e.skipped_count ?? 0;
  return {
...
function xe(e, t) {
  const { created: r, failed: s, skipped: i, total: a } = e;
  if (a === 0) {
...
function ei(e) {
  const { created: t, failed: r, skipped: s, total: i, failures: a } = e, o = `
        ...
        <div class="border rounded overflow-hidden">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ${a.map((c) => `
        <tr>
          <td class="px-3 py-2 text-sm text-gray-700">${n(c.id)}</td>
          <td class="px-3 py-2 text-sm text-gray-700">${n(c.locale)}</td>
          <td class="px-3 py-2 text-sm text-red-600">${n(c.error)}</td>
        </tr>
      `).join("")}
            </tbody>
          </table>
...
var $ = class {
  ...
  render() {
    const { locale: e, size: t, mode: r, localeExists: s } = this.config, { loading: i, created: a, error: o } = this.state, l = g(e), c = t === "sm" ? "text-xs px-2 py-1" : "text-sm px-3 py-1.5", d = r === "button" ? "rounded-lg" : "rounded-full";
    let h, p = "";
    i ? (h = "bg-gray-100 text-gray-600 border-gray-300", p = this.renderSpinner()) : a ? (h = "bg-green-100 text-green-700 border-green-300", p = this.renderCheckIcon()) : o ? (h = "bg-red-100 text-red-700 border-red-300", p = this.renderErrorIcon()) : s ? h = "bg-blue-100 text-blue-700 border-blue-300" : h = "bg-amber-100 text-amber-700 border-amber-300";
    const f = this.renderActions();
    return `
      <div class="inline-flex items-center gap-1.5 ${c} ${d} border ${h}"
           data-locale-action="${n(e)}"
        ...
        ${p}
        <span class="font-medium uppercase tracking-wide" aria-hidden="true">${n(e)}</span>
  ...
  renderActions() {
    const { locale: e, localeExists: t, size: r } = this.config, { loading: s, created: i } = this.state, a = r === "sm" ? "p-0.5" : "p-1", o = r === "sm" ? "w-3 h-3" : "w-4 h-4", l = [];
    if (!t && !i && !s && l.push(`
        <button type="button"
  ...
  async handleCreate() {
    if (!(this.state.loading || this.state.created)) {
      ...
      try {
        ...
        const r = `${this.config.apiEndpoint}/actions/create_translation`, s = await ue(r, e);
        if (s.success) {
  ...
  handleOpen() {
    const { locale: e, navigationBasePath: t, recordId: r } = this.config, { newRecordId: s } = this.state, i = s || r, a = new URLSearchParams();
    a.set("locale", e);
...
function ii(e, t) {
  ...
  return e.querySelectorAll("[data-locale-action]").forEach((s) => {
    ...
    if (!i) return;
    const a = s.getAttribute("data-locale-exists") === "true", o = {
      ...t,
      locale: i,
      localeExists: a
...
function H(e, t, r, s) {
  ...
  return a && i.set("channel", a), `${e}/${t}/edit?${i.toString()}`;
}
var B = class {
  constructor(e) {
  ...
  render() {
    ...
    const { context: e, showFormLockMessage: t } = this.config, r = e.requestedLocale || "requested", s = e.resolvedLocale || "default", i = g(r), a = g(s), o = this.renderPrimaryCta(), l = this.renderSecondaryCta(), c = t ? this.renderFormLockMessage() : "";
    return `
      <div class="fallback-banner bg-amber-50 border border-amber-200 rounded-lg shadow-sm"
           role="alert"
           aria-live="polite"
           data-fallback-banner="true"
           data-requested-locale="${n(r)}"
           data-resolved-locale="${n(s)}">
        <div class="p-4">
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="mt-1 text-sm text-amber-700">
                The <strong class="font-medium">${n(i)}</strong> (${n(r.toUpperCase())})
                translation doesn't exist yet. You're viewing content from
                <strong class="font-medium">${n(a)}</strong> (${n(s.toUpperCase())}).
              </p>
  ...
  renderPrimaryCta() {
    const { context: e, apiEndpoint: t, navigationBasePath: r, panelName: s, channel: i } = this.config, a = e.requestedLocale, o = String(i ?? "").trim();
    return !a || !e.recordId ? "" : `
      <button type="button"
              ...
              data-locale="${n(a)}"
              data-record-id="${n(e.recordId)}"
              data-api-endpoint="${n(t)}"
              data-panel="${n(s || "")}"
              data-channel="${n(o)}"
              aria-label="Create ${g(a)} translation">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
        </svg>
        Create ${n(a.toUpperCase())} translation
      </button>
    `;
  }
  renderSecondaryCta() {
  ...
  async handleCreate() {
    const { context: e, apiEndpoint: t, panelName: r, channel: s, navigationBasePath: i } = this.config, a = e.requestedLocale, o = e.recordId, l = String(s ?? "").trim() || void 0;
    !a || !o || await new $({
      locale: a,
      recordId: o,
      apiEndpoint: t,
...
var Q = class {
  ...
  getDisabledReason() {
    ...
    const e = ge({ reason_code: this.actionState?.reason_code });
  ...
  render() {
    ...
    const t = this.isCreateActionEnabled(), r = this.getDisabledReason(), s = this.getOverflowCount(), i = e.map((o) => this.renderChip(o, t, r)).join(""), a = s > 0 ? this.renderOverflow(s) : "";
    return `
      <div class="${t ? "inline-flex items-center gap-1.5 flex-wrap" : "inline-flex items-center gap-1.5 flex-wrap opacity-60"}"
           ...
           data-record-id="${n(this.config.recordId)}"
  ...
  renderChip(e, t, r) {
    const { recordId: s, apiEndpoint: i, navigationBasePath: a, panelName: o, channel: l, size: c } = this.config, d = String(l ?? "").trim() || void 0;
    return t ? X({
      locale: e,
      recordId: s,
      apiEndpoint: i,
  ...
  renderDisabledChip(e, t, r) {
    const s = r === "md" ? "text-sm px-3 py-1.5" : "text-xs px-2 py-1", i = t || "Translation creation unavailable", a = g(e);
    return `
      <div class="inline-flex items-center gap-1 ${s} rounded-full border border-gray-300 bg-gray-100 text-gray-500 cursor-not-allowed"
        ...
        <span class="font-medium uppercase tracking-wide">${n(e)}</span>
      </div>
    `;
  }
  renderOverflow(e) {
...
function Z(e) {
  const t = e.modifiers.map(Le), r = Ae(e.key);
...
var ee = class {
  ...
  matchesEvent(e, t) {
    ...
    const i = k(), a = new Set(e.modifiers), o = a.has("ctrl"), l = a.has("meta"), c = a.has("alt"), d = a.has("shift");
...
function fi(e) {
  ...
  for (const a of e) {
    ...
    const o = t.get(a.category) || [];
    o.push(a), t.set(a.category, o);
  }
  const r = {
    save: "Save & Submit",
      ...
      for (const l of o) {
        ...
        i += `
          <div class="flex justify-between items-center py-1">
            <dt class="text-sm text-gray-600">${n(l.description)}</dt>
            <dd class="flex-shrink-0 ml-4">
              <kbd class="px-2 py-1 bg-gray-100 border border-gray-200 rounded text-xs font-mono text-gray-700">${n(c)}</kbd>
            </dd>
          </div>
      `;
      }
...
var te = "admin_keyboard_shortcuts_settings", re = "admin_keyboard_shortcuts_hint_dismissed", S = {
  enabled: !0,
  shortcuts: {},
...
function gi(e) {
  ...
  if (t)
    try {
      ...
      t.setItem(te, JSON.stringify(r));
    } catch {
    }
}
function Ie() {
  const e = C();
  return e ? e.getItem(re) === "true" : !1;
}
function De() {
  const e = C();
  if (e)
...
function Re(e) {
  if (Ie()) return null;
  const { container: t, position: r = "bottom", onDismiss: s, onShowHelp: i, autoDismissMs: a = 1e4 } = e, o = document.createElement("div");
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            to view all shortcuts, or use <kbd class="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded text-xs font-mono">${Ee()}+S</kbd> to save.
          </p>
...
function bi(e) {
  const { container: t, shortcuts: r, settings: s, onSettingsChange: i } = e, a = {
    save: "Save & Submit",
    navigation: "Navigation",
    locale: "Locale Switching",
    actions: "Actions",
  ...
  for (const d of l) {
    ...
    `;
      for (const p of h) {
        const f = s.shortcuts[p.id] !== !1, b = Z(p);
        c += `
        <div class="flex items-center justify-between py-2 px-3 rounded hover:bg-gray-50 dark:hover:bg-gray-700/50">
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-700 dark:text-gray-300">${n(p.description)}</span>
          </div>
          <input type="checkbox"
                 id="shortcut-${n(p.id)}"
                 data-settings-action="toggle-shortcut"
                 data-shortcut-id="${n(p.id)}"
                 ${f ? "checked" : ""}
                 class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
                 aria-label="Enable ${n(p.description)} shortcut">
        </div>
      `;
      }
      c += `
...
function Me(e, t) {
  ...
  for (const a of i) s.register(a);
  return Pe(s, r), s.bind(), s;
}
function vi(e, t) {
  const r = Me(e, t);
  return t.hintContainer && Re({
...
var Be = 1500, Fe = 2e3, F = "autosave", y = {
  idle: "",
  saving: "Saving...",
  saved: "Saved",
  error: "Save failed",
  ...
  constructor(e = {}) {
    this.state = "idle", this.conflictInfo = null, this.pendingData = null, this.lastError = null, this.debounceTimer = null, this.savedTimer = null, this.listeners = [], this.isDirty = !1, this.config = {
      ...
      onSave: e.onSave,
      debounceMs: e.debounceMs ?? Be,
      savedDurationMs: e.savedDurationMs ?? Fe,
      notifier: e.notifier,
      showToasts: e.showToasts ?? !1,
      classPrefix: e.classPrefix ?? F,
      labels: {
        ...y,
        ...e.labels
      },
...
function Si(e, t) {
  ...
  let a = i.container;
  !a && s && (a = e.querySelector(s) ?? void 0);
  const o = new se({
    ...i,
    container: a
  }), l = () => {
    const f = new FormData(e), b = {};
    return f.forEach((w, ce) => {
      b[ce] = w;
    }), b;
  }, c = (f) => {
    const b = f.target;
...
var ie = "char-counter", qe = "interpolation-preview", ne = "dir-toggle", ae = [
  {
    pattern: /\{\{(\w+(?:\.\w+)*)\}\}/g,
    name: "Mustache",
    example: "{{name}}"
  ...
  constructor(e) {
    this.previewEl = null, this.config = {
      input: e.input,
      container: e.container,
      sampleValues: e.sampleValues ?? je,
      patterns: [...ae, ...e.customPatterns ?? []],
  ...
  getPreviewText() {
    ...
    for (const t of this.config.patterns)
      t.pattern.lastIndex = 0, e = e.replace(t.pattern, (r, s) => {
        ...
        for (const [a, o] of Object.entries(this.config.sampleValues)) if (a.toLowerCase() === i) return o;
        return r;
      });
    return e;
  }
  ...
  renderHighlightedPreview() {
    ...
    t.sort((a, o) => a.start - o.start);
    let s = "", i = 0;
    for (const a of t) {
      s += n(e.slice(i, a.start));
      const o = this.getSampleValue(a.variable), l = e.slice(a.start, a.end);
      s += `<span class="${r}__variable" title="${n(l)}">${n(o ?? l)}</span>`, i = a.end;
    }
    return s += n(e.slice(i)), s;
  }
  render() {
    const e = this.config.classPrefix;
    return `<div class="${e}${this.getMatches().length === 0 ? ` ${e}--empty` : ""}">
      <span class="${e}__label">Preview:</span>
      <span class="${e}__content">${this.config.highlightVariables ? this.renderHighlightedPreview() : n(this.getPreviewText())}</span>
    </div>`;
  }
  destroy() {
...
function $i(e, t = {}) {
  ...
  return {
    ...
    toggles: i,
    destroy: () => {
      r.forEach((a) => a.destroy()), s.forEach((a) => a.destroy()), i.forEach((a) => a.destroy());
    }
  };
}
function ki(e, t, r, s = ie) {
  const i = [s];
  r && i.push(`${s}--${r}`);
  const a = t ? `${e} / ${t}` : `${e}`;
  return `<span class="${i.join(" ")}" aria-live="polite">${a}</span>`;
}
function Ci(e, t = ne) {
...
function O(e) {
  ...
  const t = e, r = t.enabled === !0, s = R(t.entry), i = typeof t.visible == "boolean" ? t.visible : r && (s ? s.enabled : !0), a = t.actions && typeof t.actions == "object" ? t.actions : {}, o = {};
...
function Ne(e) {
  const t = e && typeof e == "object" ? e : {}, r = R(t) ?? { enabled: !1 };
  return {
    ...r,
...
function Ge(e) {
  ...
  const t = e, r = typeof t.modules == "object" && t.modules ? t.modules : {}, s = typeof t.features == "object" && t.features ? t.features : {};
  return {
    profile: _(t.profile ?? t.capability_mode),
    capability_mode: _(t.capability_mode ?? t.profile),
    supported_profiles: Array.isArray(t.supported_profiles) ? t.supported_profiles.map(_).filter((i, a, o) => o.indexOf(i) === a) : [...D.supported_profiles],
    schema_version: typeof t.schema_version == "number" ? t.schema_version : 1,
    modules: {
      exchange: O(r.exchange),
      queue: O(r.queue)
    },
    features: {
      cms: typeof s.cms == "boolean" ? s.cms : !1,
      dashboard: typeof s.dashboard == "boolean" ? s.dashboard : !1,
      suggestions: Ne(s.suggestions)
    },
    routes: Ve(t.routes),
...
function We(e) {
  return !e || typeof e != "object" ? null : Ge(e);
}
var oe = class {
  constructor(e) {
    this.capabilities = e;
  ...
  e.querySelectorAll("[data-capability-gate]").forEach((r) => {
    ...
    if (s)
      try {
        ...
        Qe(r, t.gateNavItem(i));
      } catch {
        Ke.warn("Invalid capability gate config:", s);
      }
  });
}
var Ze = v("DataGrid");
...
var tt = {
  ...
  constructor(e) {
    super("loading"), this.container = null, this.gateResult = null, this.data = null, this.error = null, this.activePreset = "all", this.refreshTimer = null, this.config = {
      ...
      filterPresets: e.filterPresets || rt,
  ...
  async loadData() {
    ...
    try {
      ...
      this.data = await et(s), this.state = this.data.assignments.length === 0 ? "empty" : "loaded", this.error = null;
    } catch (e) {
  ...
  render() {
    ...
    this.container.innerHTML = `
      <div class="translator-dashboard" role="region" aria-label="${n(e.title)}">
        ${this.renderHeader()}
  ...
  renderHeader() {
    ...
    return `
      <div class="dashboard-header">
        <h2 class="dashboard-title">${n(e.title)}</h2>
  ...
  renderSummaryCard(e, t, r, s) {
    return `
      <div class="summary-card ${s}" role="listitem" data-summary="${e}">
        ...
        <div class="summary-label">${n(t)}</div>
      </div>
    `;
  }
  ...
  renderFilterPreset(e) {
    ...
    return `
      <button type="button"
              ...
              data-preset="${e.id}">
        ${e.icon || ""}
        <span class="filter-label">${n(e.label)}</span>
        ${s}
      </button>
    `;
  }
  ...
  renderError() {
    ...
    return `
      <div class="dashboard-error" role="alert">
        ...
        <p class="error-message">${n(e.error)}</p>
        ${this.error ? `<p class="error-detail">${n(this.error.message)}</p>` : ""}
        <button type="button" class="retry-btn">${n(e.retry)}</button>
      </div>
    `;
  }
  renderEmpty() {
    ...
    return `
      <div class="dashboard-empty" role="status">
        ...
        <p class="empty-title">${n(e.noAssignments)}</p>
        <p class="empty-description">${n(e.noAssignmentsDescription)}</p>
      </div>
    `;
  }
  renderDisabled() {
    const e = this.gateResult?.reason || "Access to this feature is not available.", t = this.gateResult ? Xe(this.gateResult) : "";
    return `
      <div class="dashboard-disabled" role="alert" aria-live="polite">
        ...
        <p class="disabled-description">${n(e)}</p>
        ${t}
    ...
    return this.activePreset === "due_soon" && (t = t.filter((r) => r.due_state === "due_soon" || r.due_state === "overdue")), t.length === 0 ? this.renderEmpty() : `
      <div class="dashboard-assignment-list">
        <table class="assignment-table" role="grid" aria-label="Translation assignments">
          <thead>
            <tr>
              <th scope="col">${n(e.sourceTitle)}</th>
              <th scope="col">${n(e.targetLocale)}</th>
              <th scope="col">${n(e.status)}</th>
              <th scope="col">${n(e.dueDate)}</th>
              <th scope="col">${n(e.priority)}</th>
  ...
  renderAssignmentRow(e) {
    this.config.labels;
    const t = it(e.due_state), r = nt(e.priority), s = P(e.queue_state, {
      domain: "queue",
      size: "sm"
    }), i = e.due_date ? ot(new Date(e.due_date)) : "-";
    return `
      <tr class="assignment-row" data-assignment-id="${u(e.id)}">
        <td class="title-cell">
          <div class="title-content">
            <span class="source-title">${n(e.source_title || e.source_path || e.id)}</span>
            <span class="entity-type">${n(e.entity_type)}</span>
          </div>
        </td>
        <td class="locale-cell">
          <span class="locale-badge">${n(e.target_locale.toUpperCase())}</span>
          <span class="locale-arrow">←</span>
          <span class="locale-badge source">${n(e.source_locale.toUpperCase())}</span>
        </td>
        ...
        <td class="priority-cell">
          <span class="priority-indicator ${r}">${n(at(e.priority))}</span>
        </td>
...
function ct(e, t = {}) {
  ...
  if (!r)
    return Ze.warn("TranslatorDashboard: Missing data-my-work-endpoint attribute"), null;
  const s = dt(t);
  return lt(e, {
    myWorkEndpoint: r,
...
function dt(e) {
  ...
  if (e.capabilitiesPayload !== void 0) return U(e.capabilitiesPayload);
  const t = ut();
...
var pt = {
  ...
  render() {
    ...
    const e = this.config.labels;
    this.container.innerHTML = `
      <div class="exchange-import" role="region" aria-label="${n(e.title)}">
        ${this.renderHeader()}
        ${this.renderContent()}
        ${this.renderFooter()}
      </div>
  ...
  renderHeader() {
    ...
    return `
      <div class="import-header">
        <h3 class="import-title">${n(e.title)}</h3>
  ...
  renderSummaryBadges() {
    ...
    return `
      <div class="import-summary-badges">
        <span class="summary-badge success">${e.succeeded} ${n(t.success)}</span>
        <span class="summary-badge error">${e.failed} ${n(t.error)}</span>
        <span class="summary-badge conflict">${e.conflicts} ${n(t.conflict)}</span>
        <span class="summary-badge skipped">${e.skipped} ${n(t.skipped)}</span>
      </div>
    `;
  }
  renderContent() {
    ...
    return `
      <div class="import-file-input">
        <label class="file-dropzone">
          ...
          <span class="dropzone-text">${n(e.selectFile)}</span>
        </label>
      ...
      <div class="import-preview">
        <div class="preview-toolbar">
          <div class="selection-controls">
            <button type="button" class="select-all-btn">${n(e.selectAll)}</button>
            <button type="button" class="deselect-all-btn">${n(e.deselectAll)}</button>
            <span class="selection-count">${t} / ${r} ${n(e.selectedCount)}</span>
          </div>
          <div class="import-options">
            <label class="option-checkbox">
              <input type="checkbox" name="allowCreateMissing" ${this.applyOptions.allowCreateMissing ? "checked" : ""} />
              ${n(e.allowCreateMissing)}
            </label>
            <label class="option-checkbox">
          ...
          <table class="preview-grid" role="grid">
            <thead>
              <tr>
                ...
                <th scope="col">${n(e.resource)}</th>
                <th scope="col">${n(e.field)}</th>
                <th scope="col">${n(e.status)}</th>
                <th scope="col">${n(e.translatedText)}</th>
                <th scope="col">${n(e.conflictResolution)}</th>
              </tr>
            </thead>
            <tbody>
  ...
  renderPreviewRow(e) {
    this.config.labels;
    const t = J(e.status, "exchange"), r = e.status === "error", s = P(e.status, {
      domain: "exchange",
      size: "sm"
    });
    return `
      <tr class="preview-row ${t} ${e.isSelected ? "selected" : ""}" data-index="${e.index}">
        ...
        <td class="resource-cell">
          <span class="resource-type">${n(e.resource)}</span>
          <span class="entity-id">${n(e.entityId)}</span>
        </td>
        <td class="field-cell">${n(e.fieldPath)}</td>
        <td class="status-cell">
          ${s}
          ${e.error ? `<span class="error-message" title="${u(e.error)}">${n(gt(e.error, 30))}</span>` : ""}
        </td>
        <td class="translation-cell">
          <span class="translation-text" title="${u(e.targetLocale)}">${n(e.targetLocale)}</span>
        </td>
  ...
  renderConflictResolution(e) {
    ...
    return `
      <select class="resolution-select" data-index="${e.index}">
        <option value="skip" ${r === "skip" ? "selected" : ""}>${n(t.skip)}</option>
        <option value="keep_current" ${r === "keep_current" ? "selected" : ""}>${n(t.keepCurrent)}</option>
        <option value="accept_incoming" ${r === "accept_incoming" ? "selected" : ""}>${n(t.acceptIncoming)}</option>
        <option value="force" ${r === "force" ? "selected" : ""}>${n(t.force)}</option>
      </select>
  ...
  renderError() {
    ...
    return `
      <div class="import-error" role="alert">
        ...
        <p class="error-message">${n(this.error?.message || e.error)}</p>
        <button type="button" class="reset-btn">${n(e.cancelButton)}</button>
      </div>
    `;
  }
  renderFooter() {
    ...
    return `
      <div class="import-footer">
        <button type="button" class="cancel-btn">${n(e.cancelButton)}</button>
        ${this.state === "idle" ? `
...
function zi(e) {
  ...
  return !t || !r ? (ht.warn("ExchangeImport: Missing required data attributes"), null) : bt(e, {
    validateEndpoint: t,
    applyEndpoint: r
  });
}
var mt = {
  ...
  elapsed: "Elapsed",
  conflicts: "Conflicts"
}, vt = 2e3, yt = 300, wt = "async_job_", le = class {
  constructor(e = {}) {
    ...
    this.config = {
      storageKeyPrefix: e.storageKeyPrefix || wt,
      pollInterval: e.pollInterval || vt,
      maxPollAttempts: e.maxPollAttempts || yt,
  ...
  render() {
    ...
    const e = this.config.labels;
    this.container.innerHTML = `
      <div class="async-progress" role="region" aria-label="${n(e.title)}">
        ${this.renderHeader()}
        ${this.renderContent()}
        ${this.renderFooter()}
      </div>
  ...
  renderHeader() {
    ...
    if (!this.job) return `
        <div class="progress-header idle">
          <h4 class="progress-title">${n(e.title)}</h4>
          <span class="progress-status">${n(e.noActiveJob)}</span>
        </div>
      `;
    const t = J(this.job.status, "exchange"), r = this.getStatusLabel(), s = this.pollingState === "paused" ? `<span class="progress-status ${t}">${n(r)}</span>` : P(this.job.status, {
      domain: "exchange",
      size: "sm"
    });
    return `
    ...
    return `
      <div class="progress-info">
        <span class="info-item">
          <span class="info-label">${n(e.jobId)}:</span>
          <code class="info-value">${n(this.job.id)}</code>
        </span>
        ${t ? `
          <span class="info-item">
            <span class="info-label">${n(e.elapsed)}:</span>
            <span class="info-value">${n(t)}</span>
          </span>
        ` : ""}
      </div>
    `;
    ...
    return `
      <div class="progress-conflicts">
        <span class="conflicts-header">
          <span class="conflicts-label">${n(e.conflicts)}:</span>
  ...
  renderError() {
    ...
    return e ? `
      <div class="progress-error" role="alert">
        <span class="error-message">${n(e)}</span>
      </div>
    ` : "";
  }
  renderFooter() {
    const e = this.config.labels, t = [];
    return this.pollingState === "paused" && t.push(`<button type="button" class="resume-btn">${n(e.resume)}</button>`), this.pollingState === "polling" && t.push(`<button type="button" class="cancel-btn">${n(e.cancel)}</button>`), (this.error || this.job?.status === "failed") && t.push(`<button type="button" class="retry-btn">${n(e.retry)}</button>`), (this.job?.status === "completed" || this.job?.status === "failed") && t.push(`<button type="button" class="dismiss-btn">${n(e.dismiss)}</button>`), t.length === 0 ? "" : `
...
var Ct = class {
  ...
  render() {
    if (!this.container) {
      St.warn("[SideBySideEditor] Container not found");
      return;
    }
    this.container.innerHTML = this.buildHTML(), this.attachEventListeners();
  }
  buildHTML() {
    const { drift: e, labels: t, sourceLocale: r, targetLocale: s, fields: i } = this.config, a = this.shouldShowDriftBanner() ? this.renderDriftBanner(e, t) : "", o = i.map((l) => this.renderFieldRow(l, t)).join("");
    return `
        ...
        <div class="sbs-columns">
          <div class="sbs-header">
            <div class="sbs-column-header sbs-source-header">
              <span class="sbs-column-title">${n(t.sourceColumn)}</span>
              <span class="sbs-locale-badge">${r.toUpperCase()}</span>
            </div>
            <div class="sbs-column-header sbs-target-header">
              <span class="sbs-column-title">${n(t.targetColumn)}</span>
  ...
  renderDriftBanner(e, t) {
    const r = {
      ...I,
      ...t
    }, s = e.changedFieldsSummary.count, i = e.changedFieldsSummary.fields, a = i.length > 0 ? `<ul class="sbs-drift-fields-list">${i.map((o) => `<li>${n(o)}</li>`).join("")}</ul>` : "";
    return `
      <div class="sbs-drift-banner" role="alert" aria-live="polite" data-drift-banner="true">
        ...
        <div class="sbs-drift-content">
          <h3 class="sbs-drift-title">${n(r.driftBannerTitle)}</h3>
          <p class="sbs-drift-description">
            ${n(r.driftBannerDescription)}
  ...
  renderFieldRow(e, t) {
    const r = {
      ...I,
      ...t
    }, s = e.hasSourceChanged ? `<span class="sbs-field-changed" title="${n(r.fieldChangedIndicator)}">
          <svg class="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clip-rule="evenodd" />
          </svg>
        </span>` : "", i = this.renderSourceField(e), a = this.renderTargetField(e), o = `
      <button type="button"
  ...
  renderTargetField(e) {
    const t = n(e.targetValue || ""), r = e.placeholder ? `placeholder="${u(e.placeholder)}"` : "", s = e.required ? "required" : "", i = e.maxLength ? `maxlength="${e.maxLength}"` : "";
    return e.type === "textarea" || e.type === "richtext" || e.type === "html" ? `
...
export {
  ...
  Xs as AdvancedSearch,
  le as AsyncProgress,
  se as AutosaveIndicator,
  es as CORE_READINESS_DISPLAY,
  oe as CapabilityGate,
  lr as CellRendererRegistry,
  He as CharacterCounter,
  Cr as ColumnManager,
  Jt as CommonRenderers,
  rt as DEFAULT_FILTER_PRESETS,
  ae as DEFAULT_INTERPOLATION_PATTERNS,
  je as DEFAULT_SAMPLE_VALUES,
  I as DEFAULT_SIDE_BY_SIDE_LABELS,
  me as DEFAULT_STATUS_LEGEND_ITEMS,
  Js as DEFAULT_TRANSLATION_QUICK_FILTERS,
  ss as DISABLED_REASON_DISPLAY,
  mr as DataGrid,
  kr as DefaultColumnVisibilityBehavior,
  js as DetailActionsController,
  Ue as DirectionToggle,
  Yr as EXCHANGE_JOB_STATUS_DISPLAY,
  Nr as EXCHANGE_ROW_STATUS_DISPLAY,
  ft as ExchangeImport,
  B as FallbackBanner,
  At as FilterBuilder,
  Sr as GoCrudBulkActionBehavior,
  fr as GoCrudExportBehavior,
  $r as GoCrudFilterBehavior,
  wr as GoCrudPaginationBehavior,
  ur as GoCrudSearchBehavior,
  cr as GoCrudSortBehavior,
  Q as InlineLocaleChips,
  Oe as InterpolationPreview,
  ee as KeyboardShortcutRegistry,
  or as LocalDataGridStateStore,
  $ as LocaleActionChip,
  Hr as PayloadInputModal,
  Yt as PreferencesDataGridStateStore,
  ts as QUEUE_CONTENT_STATE_DISPLAY,
  is as QUEUE_DUE_STATE_DISPLAY,
  Gr as QUEUE_STATE_DISPLAY,
  Ns as QuickFilters,
  Bs as SchemaActionBuilder,
  yr as ServerColumnVisibilityBehavior,
  Ct as SideBySideEditor,
  W as StatusLegend,
  Us as TranslationBlockerModal,
  Ws as TranslationPanel,
  st as TranslatorDashboard,
  $e as applyFormLock,
  Qe as applyGateToElement,
  Pe as applyShortcutSettings,
  H as buildLocaleEditUrl,
  Ps as buildSchemaRowActions,
  Hi as checkForPersistedJob,
  Er as collapseAllGroups,
  xt as createAsyncProgress,
  ri as createBulkCreateMissingHandler,
  U as createCapabilityGate,
  ar as createDataGridStateStore,
  Ii as createEmptyCapabilityGate,
  bt as createExchangeImport,
  pi as createInlineLocaleChipsRenderer,
  Ls as createLocaleBadgeRenderer,
  Qr as createReasonCodeCellRenderer,
  Et as createSideBySideEditor,
  ns as createStatusCellRenderer,
  ve as createStatusLegend,
  yi as createTranslationAutosave,
  Ss as createTranslationMatrixRenderer,
  Gs as createTranslationPanel,
  Os as createTranslationQuickFilters,
  _e as createTranslationShortcuts,
  Cs as createTranslationStatusRenderer,
  lt as createTranslatorDashboard,
  _r as decodeExpandedGroupsToken,
  Li as detectInterpolations,
  De as dismissShortcutHint,
  dr as encodeExpandedGroupsToken,
  ye as executeBulkCreateMissing,
  Ar as expandAllGroups,
  ir as extractBackendSummaries,
  We as extractCapabilities,
  Pt as extractExchangeError,
  Ms as extractSchemaActions,
  $t as extractSourceTargetDrift,
  M as extractTranslationContext,
  Y as extractTranslationReadiness,
  hr as formatPaginationNumber,
  Z as formatShortcutDisplay,
  Mt as generateExchangeReport,
  ge as getActionBlockDisplay,
  Kr as getAllReasonCodes,
  qi as getAsyncProgressStyles,
  xi as getAutosaveIndicatorStyles,
  Ri as getCapabilityGateStyles,
  Oi as getChangedFields,
  Ai as getCharCountSeverity,
  mi as getDefaultShortcutRegistry,
  rs as getDisabledReasonDisplay,
  Fi as getExchangeImportStyles,
  Nt as getExpandedGroupIds,
  Ei as getFieldHelperStyles,
  ai as getFormLockReason,
  g as getLocaleLabel,
  $s as getMissingTranslationsCount,
  Le as getModifierSymbol,
  Lr as getPersistedExpandState,
  nr as getPersistedViewMode,
  Ee as getPrimaryModifierLabel,
  Zr as getSeverityCssClass,
  Ni as getSideBySideEditorStyles,
  J as getStatusCssClass,
  Jr as getStatusDisplay,
  Ur as getStatusVocabularyStyles,
  as as getStatusesForDomain,
  Mi as getTranslatorDashboardStyles,
  Gt as getViewModeForViewport,
  Bt as groupRowResultsByStatus,
  pr as handleDelete,
  Vt as hasBackendGroupedRows,
  kt as hasFieldDrift,
  Es as hasMissingTranslations,
  gs as hasTranslationContext,
  ws as hasTranslationReadiness,
  ji as initAsyncProgress,
  Pi as initCapabilityGating,
  zi as initExchangeImport,
  ci as initFallbackBanner,
  $i as initFieldHelpers,
  Si as initFormAutosave,
  di as initFormLock,
  hi as initInlineLocaleChips,
  Me as initKeyboardShortcuts,
  vi as initKeyboardShortcutsWithDiscovery,
  ii as initLocaleActionChips,
  zs as initPanelDetailActions,
  Ks as initQuickFilters,
  Ui as initSideBySideEditorFromRecord,
  Qs as initStatusLegends,
  Bi as initTranslatorDashboard,
  ct as initTranslatorDashboardWithOptions,
  ls as initializeVocabularyFromPayload,
  Ti as isCoreEnabled,
  Je as isExchangeEnabled,
  Ft as isExchangeError,
  ni as isFormLocked,
  As as isInFallbackMode,
  k as isMacPlatform,
  tr as isNarrowViewport,
  Ye as isQueueEnabled,
  bs as isReadyForTransition,
  Ie as isShortcutHintDismissed,
  Vr as isValidReasonCode,
  os as isValidStatus,
  Te as loadShortcutSettings,
  br as mergeBackendSummaries,
  Mr as normalizeActionBlockCode,
  Pr as normalizeActionState,
  qr as normalizeActionStateMap,
  Xr as normalizeActionStateMeta,
  Dr as normalizeActionStateRecord,
  Ot as normalizeBackendGroupedRows,
  Wr as normalizeBulkActionStateConfig,
  Fr as normalizeBulkActionStateMap,
  zr as normalizeBulkActionStateResponse,
  jr as normalizeDetailActionStatePayload,
  Br as normalizeListActionStatePayload,
  xr as paginationWindow,
  _i as parseCapabilityMode,
  zt as parseImportResult,
  gr as parseViewMode,
  Zt as persistExpandState,
  er as persistViewMode,
  ke as removeFormLock,
  wi as renderAutosaveIndicator,
  ms as renderAvailableLocalesIndicator,
  ti as renderBulkResultInline,
  ei as renderBulkResultSummary,
  ki as renderCharacterCounter,
  qs as renderDetailActions,
  Ci as renderDirectionToggle,
  Xe as renderDisabledReasonBadge,
  Re as renderDiscoveryHint,
  oi as renderFallbackBannerFromRecord,
  ks as renderFallbackWarning,
  Di as renderGateAriaAttributes,
  rr as renderGroupHeaderRow,
  Kt as renderGroupHeaderSummary,
  Wt as renderGroupedEmptyState,
  Qt as renderGroupedErrorState,
  sr as renderGroupedLoadingState,
  Ce as renderInlineLocaleChips,
  X as renderLocaleActionChip,
  si as renderLocaleActionList,
  xs as renderLocaleBadge,
  ys as renderLocaleCompleteness,
  vs as renderMissingTranslationsBadge,
  vr as renderPaginationButtons,
  ps as renderPublishReadinessBadge,
  Ys as renderQuickFiltersHTML,
  _s as renderReadinessIndicator,
  fe as renderReasonCodeBadge,
  Rr as renderReasonCodeIndicator,
  bi as renderShortcutSettingsUI,
  fi as renderShortcutsHelpContent,
  Ds as renderStatusBadge,
  Zs as renderStatusLegendHTML,
  fs as renderTranslationAssignmentSummary,
  Is as renderTranslationExchangeSummary,
  us as renderTranslationFamilyLink,
  ds as renderTranslationFamilyMemberCount,
  Ts as renderTranslationMatrixCell,
  hs as renderTranslationStatusCell,
  P as renderVocabularyStatusBadge,
  Or as renderVocabularyStatusIcon,
  pe as resolveActionState,
  gi as saveShortcutSettings,
  li as shouldShowFallbackBanner,
  ui as shouldShowInlineLocaleChips,
  Vs as showTranslationBlocker,
  Tr as toggleGroupExpand,
