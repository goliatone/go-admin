        ...
        let E = 1, x = r.DomRenderer = class extends t.Disposable {
          ...
          _setCellUnderline(b, k, A, H, P, M) {
            ...
            for (let $ = A; $ <= H; ++$) {
              ...
              if (!C || !D) break;
              C.replaceChildren(...this._rowFactory.createRow(D, X, X === w, F, N, L, I, this.dimensions.css.cell.width, this._widthCache, M ? $ === A ? b : 0 : -1, M ? ($ === H ? k : P) - 1 : -1));
            }
          }
        };
        r.DomRenderer = x = c([
        ...
        let o = r.DomRendererRowFactory = class {
          ...
          createRow(v, d, h, m, E, x, b, k, A, H, P) {
            ...
            for (let G = 0; G < L; G++) {
              ...
              if (le === " " && (K.isUnderline() || K.isOverline()) && (le = " "), B = Y * k - A.get(le, K.isBold(), K.isItalic()), w) {
...
`);
          ...
          _getWordAt(d, h, m = !0, E = !0) {
            ...
            if (k.charAt(A) === " ") {
              ...
              for ($ > 1 && (w += $ - 1, H += $ - 1); F > 0 && A > 0 && !this._isCharWordSeparator(b.loadCell(F - 1, this._workCell)); ) {
...
`, t);
  ...
  (function(j, q) {
    typeof R == "object" && typeof U == "object" ? U.exports = q() : typeof define == "function" && define.amd ? define([], q) : typeof R == "object" ? R.FitAddon = q() : j.FitAddon = q();
  })(self, (() => (() => {
    "use strict";
    var j = {};
    return (() => {
  ...
  return () => {
    ...
    U.setAttribute("data-debug-repl-xterm", "true"), U.textContent = be, document.head.appendChild(U), R = !0;
  };
})(), Be = class {
  constructor(R) {
    this.socket = null, this.status = "disconnected", this.reconnectAttempts = 0, this.reconnectTimer = null, this.manualClose = !1, this.resetOnOpen = !1, this.resizeObserver = null, this.lineBuffer = "", this.skipEscape = !1, this.prompt = ">>> ", this.awaitingPrompt = !0, this.options = R, Ae(), this.terminal = new ye.Terminal({
    ...
    }), this.fitAddon = new we.FitAddon(), this.terminal.loadAddon(this.fitAddon), this.terminal.open(R.container), this.fitAddon.fit(), this.terminal.focus(), this.bindTerminal(), this.observeResize(R.container), R.autoConnect !== !1 && this.connect();
  }
  connect() {
    if (this.socket && (this.socket.readyState === WebSocket.OPEN || this.socket.readyState === WebSocket.CONNECTING)) return;
    this.manualClose = !1, this.setStatus("connecting");
    const R = Re(this.options.debugPath, this.options.kind);
    this.socket = new WebSocket(R), this.socket.onopen = () => {
  ...
  handleKeyEvent(R) {
    ...
    return U && R.shiftKey && R.code === "KeyV" ? (navigator.clipboard?.readText().then((j) => {
...
`);
  ...
  scheduleReconnect() {
    ...
    const R = this.reconnectAttempts, U = Math.min(Ee * Math.pow(2, R), ke), j = U * (0.2 + Math.random() * 0.3);
  ...
  error: "error"
}, Pe = '<i class="iconoir-terminal debug-repl__overlay-icon"></i>', Ie = '<span class="debug-repl__overlay-text">Session not connected. Click the button below to start a terminal session.</span>', He = '<button class="debug-repl__overlay-btn" data-overlay-connect><i class="iconoir-play"></i> Connect</button>', ve = (R) => String(R ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;"), We = class {
  constructor(R) {
    ...
    this.root.innerHTML = `
      <div class="debug-repl__header">
        <div class="debug-repl__title">
          <span class="debug-repl__label">${Te[R.kind]}</span>
      ...
      <div class="debug-repl__footer">
        <span class="debug-repl__hint">${Me[R.kind]}</span>
      </div>
  ...
  updateStatus(R) {
    ...
    const j = R === "disconnected" || R === "error";
  ...
  renderCommands() {
    ...
    const R = this.commands, U = R.length, j = R.map((q) => {
  ...
  requireElement(R, U) {
    const j = U.querySelector(R);
    if (!j) throw new Error(`Missing debug repl element: ${R}`);
    return j;
  }
};
export {
...
//# sourceMappingURL=repl-panel-Cxtz4VCo.js.map