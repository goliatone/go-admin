  ...
  renderResults() {
    ...
    const s = e.map((r, i) => {
      ...
      return `
        <div
          ...
          aria-selected="${n}"
        >
          ${this.config.renderer.render(r, n)}
        </div>
      `;
    });
    this.dropdown.innerHTML = s.join(""), this.dropdown.querySelectorAll(".searchbox-item").forEach((r) => {
      r.addEventListener("click", (i) => {
        i.stopPropagation();
        const n = e[parseInt(r.dataset.index || "0", 10)];
        n && this.selectResult(n);
      });
    });
  }
  scrollToSelected() {
...
var g = {
  ...
  render(e, t) {
    const s = `${this.config.itemClass || ""} ${t && this.config.selectedClass || ""}`.trim(), r = e.metadata || {}, i = this.getMetadataValue(r, this.config.avatarField || "avatar"), n = this.getMetadataValue(r, this.config.emailField || "email"), a = this.getMetadataValue(r, this.config.roleField || "role"), o = this.renderAvatar(i, e.label), l = a ? this.renderRole(a) : "", c = n ? `<p class="text-xs text-gray-500 truncate">${this.escapeHtml(n)}</p>` : "";
    return `
      <div class="${s}">
  ...
  renderAvatar(e, t) {
    if (e) return `<img src="${this.escapeHtml(e)}" class="w-10 h-10 rounded-full object-cover flex-shrink-0" alt="" />`;
  ...
  render(e, t) {
    const s = `${this.config.itemClass || ""} ${t && this.config.selectedClass || ""}`.trim(), r = e.metadata || {}, i = this.config.badgeField ? this.getMetadataValue(r, this.config.badgeField) : void 0, n = this.config.showIcon ? this.renderIcon(e.icon, e.label) : "", a = i ? this.renderBadge(i) : "", o = this.renderMetadataPills(r), l = this.config.showDescription && e.description ? `<p class="text-xs text-gray-500 mt-0.5 truncate">${this.escapeHtml(e.description)}</p>` : "";
    return `
      <div class="${s}">
        <div class="flex items-center gap-3">
          ${n}
...
//# sourceMappingURL=entity-renderer-BnkXZh8v.js.map