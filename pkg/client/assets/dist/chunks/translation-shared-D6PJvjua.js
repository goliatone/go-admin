import { i as g, t as v } from "./status-vocabulary-DrEqqUD1.js";
...
var E = "inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium", m = "inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wider", C = "inline-flex items-center rounded-full px-3 py-1.5 text-sm font-medium", y = "inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium";
function e(t, r = "md") {
  return `${r === "sm" ? m : r === "lg" ? C : r === "count" ? y : E} ${a(t)}`;
}
...
var c = "rounded-xl border px-3 py-3 text-sm bg-white", L = "rounded-xl border p-5";
function dt(t) {
  return t === "blocker" ? {
...
function wt(t, r) {
  const o = 'button:not([disabled]), [href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])', i = () => Array.from(t.querySelectorAll(o)), l = (s) => {
    ...
    if (s.key === "Tab") {
      ...
      const d = n[0], b = n[n.length - 1];
      s.shiftKey && document.activeElement === d ? (s.preventDefault(), b.focus()) : !s.shiftKey && document.activeElement === b && (s.preventDefault(), d.focus());
    }
  };
  t.addEventListener("keydown", l);
  const u = i();
  return u.length > 0 && u[0].focus(), () => {
...
//# sourceMappingURL=translation-shared-D6PJvjua.js.map