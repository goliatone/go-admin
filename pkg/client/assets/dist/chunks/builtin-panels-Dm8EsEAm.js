import { n as ne, r as XT, t as xT } from "./chunk-BUbnbzFL.js";
var Ce = 1e3, Le = 12e3, ie = 8, oe = 3e4, _e = (E) => {
  const T = (E || "").trim();
  return T ? T.startsWith("/") ? T.replace(/\/+$/, "") : `/${T.replace(/\/+$/, "")}` : "";
}, De = (E) => {
  const T = window.location.protocol === "https:" ? "wss:" : "ws:", e = _e(E);
  return `${T}//${window.location.host}${e}/ws`;
}, Pe = (E, T, e) => {
  const R = E.trim();
  if (!R || !T || !e) return E;
  const [A, O] = R.split("#"), L = `${A}${A.includes("?") ? "&" : "?"}${encodeURIComponent(T)}=${encodeURIComponent(e)}`;
  return O ? `${L}#${O}` : L;
}, le = (E) => {
  if (!E) return null;
  const T = E.replace(/-/g, "+").replace(/_/g, "/"), e = T.padEnd(T.length + (4 - (T.length % 4 || 4)) % 4, "=");
  try {
    if (typeof globalThis.atob == "function") return globalThis.atob(e);
  } catch {
    return null;
  }
  return null;
  ...
  scheduleReconnect() {
    const E = this.options.maxReconnectAttempts ?? ie, T = this.options.reconnectDelayMs ?? Ce, e = this.options.maxReconnectDelayMs ?? Le;
    if (this.reconnectAttempts >= E) {
      this.setStatus("disconnected");
      return;
    }
    const R = this.reconnectAttempts, A = Math.min(T * Math.pow(2, R), e), O = A * (0.2 + Math.random() * 0.3);
  ...
  constructor(E) {
    const { url: T, authToken: e, tokenProvider: R, tokenRefreshBufferMs: A, tokenParam: O, appId: L, onEvent: I, ...o } = E, C = (S) => {
      if (L && S && !S.app_id) {
        I?.({
          ...S,
          app_id: L
    ...
    super({
      ...
      url: T,
      onEvent: C
    }), this.authToken = null, this.tokenRefreshTimer = null, this.tokenExpiresAt = null, this.baseUrl = T, this.tokenProvider = R, this.tokenRefreshBufferMs = A ?? oe, this.tokenParam = O || "token", e && this.setToken(e);
  }
  getWebSocketURL() {
    return this.authToken ? Pe(this.baseUrl, this.tokenParam, this.authToken) : this.baseUrl;
  }
  connect() {
...
}, D = (E) => String(E ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;"), eE = (E) => {
  if (!E) return "";
...
}, aE = (E, T) => E ? E.length > T ? E.substring(0, T) + "..." : E : "", $T = (E) => {
  const T = E.trim();
  if (!T) return null;
...
}, KT = (E) => E == null ? 0 : Array.isArray(E) ? E.length : typeof E == "object" ? Object.keys(E).length : 1, de = (E) => E ? E >= 500 ? "error" : E >= 400 ? "warn" : "" : "", wT = (E) => {
  if (!E) return "info";
  ...
  const T = [
    ...
    "KB",
    "MB"
  ], e = Math.min(Math.floor(Math.log(E) / Math.log(1024)), T.length - 1), R = E / Math.pow(1024, e);
  return `${e === 0 ? R : R.toFixed(1)} ${T[e]}`;
}, FO = (E) => Array.isArray(E) ? E : [];
async function JT(E, T, e = {}) {
  const { feedbackDuration: R = 1500, useIconFeedback: A = !1, successClass: O = A ? "debug-copy--success" : "copied", errorClass: L = "debug-copy--error" } = e;
  try {
    await navigator.clipboard.writeText(E);
    const I = T.innerHTML;
    return T.classList.add(O), A ? T.innerHTML = '<i class="iconoir-check"></i> Copied' : T.innerHTML = `
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="20 6 9 17 4 12"></polyline>
        </svg>
        Copied
      `, setTimeout(() => {
      T.innerHTML = I, T.classList.remove(O);
    }, R), !0;
  } catch {
...
function hO(E, T = {}) {
  E.querySelectorAll("[data-copy-trigger]").forEach((e) => {
    e.addEventListener("click", async (R) => {
      ...
      const A = e.closest("[data-copy-content]");
      A && await JT(A.getAttribute("data-copy-content") || "", e, T);
    });
  });
}
function YO(E) {
  E.querySelectorAll(".expandable-row").forEach((T) => {
    T.addEventListener("click", (e) => {
      e.target.closest("a, button, input") || e.currentTarget.classList.toggle("expanded");
    });
  });
}
function fO(E, T) {
  E.querySelectorAll("[data-sort-toggle]").forEach((e) => {
    e.addEventListener("change", (R) => {
      const A = R.target, O = A.dataset.sortToggle;
      O && T(O, A.checked);
    });
  });
}
...
function IT(E, T) {
  return [...T].sort((e, R) => e - R).map((e) => E[e]).filter(Boolean).map((e) => {
    let R = `-- Duration: ${jE(e.duration).text} | Rows: ${e.row_count ?? 0}`;
    return e.error && (R += ` | Error: ${e.error}`), e.timestamp && (R += ` | Time: ${e.timestamp}`), `${R}
${e.query || ""};`;
  }).join(`

`);
}
function pe(E, T, e = "text/sql") {
  const R = new Blob([E], { type: e }), A = URL.createObjectURL(R), O = document.createElement("a");
  O.href = A, O.download = T, O.click(), URL.revokeObjectURL(A);
}
function VO(E, T, e = {}) {
  const R = /* @__PURE__ */ new Set(), A = E.querySelector("[data-sql-toolbar]"), O = E.querySelector("[data-sql-selected-count]"), L = E.querySelector(".sql-select-all"), I = E.querySelectorAll(".sql-select-row");
  if (!A || I.length === 0) return;
  function o() {
    if (!A) return;
    const C = R.size;
    A.dataset.visible = C > 0 ? "true" : "false", O && (O.textContent = `${C} selected`), L && (L.checked = C > 0 && C === I.length, L.indeterminate = C > 0 && C < I.length);
  }
  I.forEach((C) => {
    C.addEventListener("click", (S) => {
      S.stopPropagation();
    }), C.addEventListener("change", () => {
      const S = parseInt(C.dataset.sqlIndex || "", 10);
      Number.isNaN(S) || (C.checked ? R.add(S) : R.delete(S), o());
    });
    ...
    I.forEach((C) => {
      C.checked = L.checked;
      const S = parseInt(C.dataset.sqlIndex || "", 10);
      Number.isNaN(S) || (L.checked ? R.add(S) : R.delete(S));
    }), o();
  })), E.querySelector('[data-sql-export="clipboard"]')?.addEventListener("click", async (C) => {
    if (C.preventDefault(), R.size === 0) return;
    const S = IT(T, R), n = C.currentTarget;
    await JT(S, n, e);
  }), E.querySelector('[data-sql-export="download"]')?.addEventListener("click", (C) => {
    C.preventDefault(), R.size !== 0 && pe(IT(T, R), `sql-queries-${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-").slice(0, 19)}.sql`);
  }), E.querySelector("[data-sql-clear-selection]")?.addEventListener("click", (C) => {
    C.preventDefault(), R.clear(), I.forEach((S) => {
      S.checked = !1;
    }), o();
  });
}
function bO(E, T) {
  E.querySelectorAll("[data-request-table]").forEach((e) => {
    e.addEventListener("click", (R) => {
      ...
      const O = A.closest("[data-request-id]");
      if (!O) return;
      const L = O.dataset.requestId;
      if (!L) return;
      const I = O.nextElementSibling;
      if (!I || !I.hasAttribute("data-detail-for") || I.dataset.detailFor !== L) return;
      const o = I.querySelector("[data-request-detail-template]");
      if (o) {
        const S = I.querySelector("td");
        S && (S.appendChild(o.content.cloneNode(!0)), o.remove());
      }
      const C = O.querySelector("[data-expand-icon]");
      T.has(L) ? (T.delete(L), I.style.display = "none", C && (C.textContent = "▶")) : (T.add(L), I.style.display = "table-row", C && (C.textContent = "▼"));
    });
  });
}
var Ge = {
...
function WO(E) {
  return E === "console" ? Ge : He;
}
var Be = /* @__PURE__ */ xT(((E, T) => {
  var e = (function(R) {
    var A = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, O = 0, L = {}, I = {
      ...
      util: {
        encode: function s(N) {
          return N instanceof o ? new o(N.type, s(N.content), N.alias) : Array.isArray(N) ? N.map(s) : N.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/\u00a0/g, " ");
        },
        type: function(s) {
          return Object.prototype.toString.call(s).slice(8, -1);
        },
        objId: function(s) {
          return s.__id || Object.defineProperty(s, "__id", { value: ++O }), s.__id;
        },
        clone: function s(N, _) {
          _ = _ || {};
          var P, l;
          switch (I.util.type(N)) {
            case "Object":
              if (l = I.util.objId(N), _[l]) return _[l];
              P = {}, _[l] = P;
              for (var p in N) N.hasOwnProperty(p) && (P[p] = s(N[p], _));
              return P;
            case "Array":
              return l = I.util.objId(N), _[l] ? _[l] : (P = [], _[l] = P, N.forEach(function(c, m) {
                P[m] = s(c, _);
              }), P);
            default:
              return N;
        ...
        setLanguage: function(s, N) {
          s.className = s.className.replace(RegExp(A, "gi"), ""), s.classList.add("language-" + N);
        },
        ...
        isActive: function(s, N, _) {
          for (var P = "no-" + N; s; ) {
            var l = s.classList;
            if (l.contains(N)) return !0;
            if (l.contains(P)) return !1;
            s = s.parentElement;
          }
          return !!_;
        }
      ...
      languages: {
        ...
        txt: L,
        extend: function(s, N) {
          var _ = I.util.clone(I.languages[s]);
          for (var P in N) _[P] = N[P];
          return _;
        },
        insertBefore: function(s, N, _, P) {
          P = P || I.languages;
          var l = P[s], p = {};
          for (var c in l) if (l.hasOwnProperty(c)) {
            if (c == N)
              for (var m in _) _.hasOwnProperty(m) && (p[m] = _[m]);
            _.hasOwnProperty(c) || (p[c] = l[c]);
          }
          var F = P[s];
          return P[s] = p, I.languages.DFS(I.languages, function(g, AE) {
            AE === F && g != s && (this[g] = p);
          }), p;
        },
        DFS: function s(N, _, P, l) {
          l = l || {};
          var p = I.util.objId;
          for (var c in N) if (N.hasOwnProperty(c)) {
            _.call(N, c, N[c], P || c);
            var m = N[c], F = I.util.type(m);
            F === "Object" && !l[p(m)] ? (l[p(m)] = !0, s(m, _, null, l)) : F === "Array" && !l[p(m)] && (l[p(m)] = !0, s(m, _, c, l));
          }
        }
      },
      plugins: {},
      highlightAll: function(s, N) {
        I.highlightAllUnder(document, s, N);
      },
      highlightAllUnder: function(s, N, _) {
        ...
        I.hooks.run("before-highlightall", P), P.elements = Array.prototype.slice.apply(P.container.querySelectorAll(P.selector)), I.hooks.run("before-all-elements-highlight", P);
        for (var l = 0, p; p = P.elements[l++]; ) I.highlightElement(p, N === !0, P.callback);
      },
      highlightElement: function(s, N, _) {
        var P = I.util.getLanguage(s), l = I.languages[P];
        I.util.setLanguage(s, P);
        var p = s.parentElement;
        p && p.nodeName.toLowerCase() === "pre" && I.util.setLanguage(p, P);
        var c = {
          element: s,
          language: P,
          grammar: l,
        ...
        function m(g) {
          c.highlightedCode = g, I.hooks.run("before-insert", c), c.element.innerHTML = c.highlightedCode, I.hooks.run("after-highlight", c), I.hooks.run("complete", c), _ && _.call(c.element);
        }
        if (I.hooks.run("before-sanity-check", c), p = c.element.parentElement, p && p.nodeName.toLowerCase() === "pre" && !p.hasAttribute("tabindex") && p.setAttribute("tabindex", "0"), !c.code) {
          I.hooks.run("complete", c), _ && _.call(c.element);
          return;
        }
        if (I.hooks.run("before-highlight", c), !c.grammar) {
          m(I.util.encode(c.code));
          return;
        }
        if (N && R.Worker) {
        ...
        } else m(I.highlight(c.code, c.grammar, c.language));
      },
      highlight: function(s, N, _) {
        ...
        if (I.hooks.run("before-tokenize", P), !P.grammar) throw new Error('The language "' + P.language + '" has no grammar.');
        return P.tokens = I.tokenize(P.code, P.grammar), I.hooks.run("after-tokenize", P), o.stringify(I.util.encode(P.tokens), P.language);
      },
      tokenize: function(s, N) {
        var _ = N.rest;
        if (_) {
          for (var P in _) N[P] = _[P];
          delete N.rest;
        }
        var l = new n();
        return i(l, l.head, s), S(s, l, N, l.head, 0), H(l);
      },
      hooks: {
        all: {},
        add: function(s, N) {
    ...
    R.Prism = I;
    function o(s, N, _, P) {
      this.type = s, this.content = N, this.alias = _, this.length = (P || "").length | 0;
    }
    o.stringify = function s(N, _) {
      ...
      p && (Array.isArray(p) ? Array.prototype.push.apply(l.classes, p) : l.classes.push(p)), I.hooks.run("wrap", l);
      var c = "";
      for (var m in l.attributes) c += " " + m + '="' + (l.attributes[m] || "").replace(/"/g, "&quot;") + '"';
    ...
    function S(s, N, _, P, l, p) {
      for (var c in _)
        if (!(!_.hasOwnProperty(c) || !_[c])) {
          var m = _[c];
          m = Array.isArray(m) ? m : [m];
          for (var F = 0; F < m.length; ++F) {
            if (p && p.cause == c + "," + F) return;
            var g = m[F], AE = g.inside, eT = !!g.lookbehind, RT = !!g.greedy, Ne = g.alias;
            if (RT && !g.pattern.global) {
              var re = g.pattern.toString().match(/[imsuy]*$/)[0];
              g.pattern = RegExp(g.pattern.source, re + "g");
            }
            for (var AT = g.pattern || g, y = P.next, v = l; y !== N.tail && !(p && v >= p.reach); v += y.value.length, y = y.next) {
              var q = y.value;
              if (N.length > s.length) return;
              if (!(q instanceof o)) {
                var SE = 1, V;
                if (RT) {
                  if (V = C(AT, v, s, eT), !V || V.index >= s.length) break;
                  var tE = V.index, se = V.index + V[0].length, $ = v;
                  for ($ += y.value.length; tE >= $; )
                    y = y.next, $ += y.value.length;
                  if ($ -= y.value.length, v = $, y.value instanceof o) continue;
                  for (var z = y; z !== N.tail && ($ < se || typeof z.value == "string"); z = z.next)
                    SE++, $ += z.value.length;
                  SE--, q = s.slice(v, $), V.index -= v;
                } else if (V = C(AT, 0, q, eT), !V) continue;
                var tE = V.index, IE = V[0], oE = q.slice(0, tE), ST = q.slice(tE + IE.length), _E = v + q.length;
                p && _E > p.reach && (p.reach = _E);
                var OE = y.prev;
                oE && (OE = i(N, OE, oE), v += oE.length), M(N, OE, SE);
                var ae = new o(c, AE ? I.tokenize(IE, AE) : IE, Ne, IE);
                if (y = i(N, OE, ae), ST && i(N, y, ST), SE > 1) {
                  var DE = {
                    cause: c + "," + F,
                    reach: _E
                  };
                  S(s, N, _, y.prev, v, DE), p && DE.reach > p.reach && (p.reach = DE.reach);
                }
              }
            }
          }
    ...
    function i(s, N, _) {
      ...
      return N.next = l, P.prev = l, s.length++, l;
    }
    function M(s, N, _) {
      for (var P = N.next, l = 0; l < _ && P !== s.tail; l++) P = P.next;
      N.next = P, P.prev = N, s.length -= l;
    }
    function H(s) {
    ...
    if (!R.document)
      return R.addEventListener && (I.disableWorkerMessageHandler || R.addEventListener("message", function(s) {
        var N = JSON.parse(s.data), _ = N.language, P = N.code, l = N.immediateClose;
        R.postMessage(I.highlight(P, I.languages[_], _)), l && R.close();
      }, !1)), I;
    var U = I.util.currentScript();
    U && (I.filename = U.src, U.hasAttribute("data-manual") && (I.manual = !0));
    function d() {
      I.manual || I.highlightAll();
    }
    if (!I.manual) {
      var B = document.readyState;
      B === "loading" || B === "interactive" && U && U.defer ? document.addEventListener("DOMContentLoaded", d) : window.requestAnimationFrame ? window.requestAnimationFrame(d) : window.setTimeout(d, 16);
    }
    return I;
  ...
  typeof T < "u" && T.exports && (T.exports = e), typeof global < "u" && (global.Prism = e), e.languages.markup = {
  ...
  }), Object.defineProperty(e.languages.markup.tag, "addInlined", { value: function(A, O) {
    var L = {};
    L["language-" + O] = {
      pattern: /(^<!\[CDATA\[)[\s\S]+?(?=\]\]>$)/i,
      lookbehind: !0,
    ...
    var R = "Loading…", A = function(U, d) {
      ...
      h: "c",
      tex: "latex"
    }, I = "data-src-status", o = "loading", C = "loaded", S = "failed", n = "pre[data-src]:not([" + I + '="' + C + '"]):not([' + I + '="' + o + '"])';
    function i(U, d, B) {
      var s = new XMLHttpRequest();
      s.open("GET", U, !0), s.onreadystatechange = function() {
        s.readyState == 4 && (s.status < 400 && s.responseText ? d(s.responseText) : s.status >= 400 ? B(A(s.status, s.statusText)) : B(O));
      }, s.send(null);
    }
    function M(U) {
      var d = /^\s*(\d+)\s*(?:(,)\s*(?:(\d+)\s*)?)?$/.exec(U || "");
      if (d) {
        var B = Number(d[1]), s = d[2], N = d[3];
        return s ? N ? [B, Number(N)] : [B, void 0] : [B, B];
      }
    }
    e.hooks.add("before-highlightall", function(U) {
      U.selector += ", " + n;
    }), e.hooks.add("before-sanity-check", function(U) {
      var d = U.element;
      if (d.matches(n)) {
        U.code = "", d.setAttribute(I, o);
        var B = d.appendChild(document.createElement("CODE"));
        B.textContent = R;
        var s = d.getAttribute("data-src"), N = U.language;
        if (N === "none") {
          var _ = (/\.(\w+)$/.exec(s) || [, "none"])[1];
          N = L[_] || _;
        }
        e.util.setLanguage(B, N), e.util.setLanguage(d, N);
        var P = e.plugins.autoloader;
        P && P.loadLanguages(N), i(s, function(l) {
          d.setAttribute(I, C);
          var p = M(d.getAttribute("data-range"));
          if (p) {
            var c = l.split(/\r\n?|\n/g), m = p[0], F = p[1] == null ? c.length : p[1];
            m < 0 && (m += c.length), m = Math.max(0, Math.min(m - 1, c.length)), F < 0 && (F += c.length), F = Math.max(0, Math.min(F, c.length)), l = c.slice(m, F).join(`
`), d.hasAttribute("data-start") || d.setAttribute("data-start", String(m + 1));
          }
          B.textContent = l, e.highlightElement(B);
        }, function(l) {
          d.setAttribute(I, S), B.textContent = l;
        });
      }
    }), e.plugins.fileHighlight = { highlight: function(d) {
      for (var B = (d || document).querySelectorAll(n), s = 0, N; N = B[s++]; ) e.highlightElement(N);
    } };
    var H = !1;
...
var t = (E) => E.flatMap(me), me = (E) => NE(he(E)).map(Fe), Fe = (E) => E.replace(/ +/g, " ").trim(), he = (E) => ({
  ...
  if (E[T] === "{") return ge(E, T + 1);
  if (E[T] === "[") return ye(E, T + 1);
  {
    let e = "";
  ...
  if (E.type === "concatenation") return E.items.map(NE).reduce(Ve, [""]);
  if (E.type === "mandatory_block") return E.items.flatMap(NE);
  if (E.type === "optional_block") return ["", ...E.items.flatMap(NE)];
...
var kT = (E) => ({
  ...
  text: "«EOF»",
  start: E
}), K = kT(1 / 0), Q = (E) => (T) => T.type === E.type && T.text === E.text, w = {
  ARRAY: Q({
    text: "ARRAY",
...
}, qT = (E) => E === a.RESERVED_DATA_TYPE || E === a.RESERVED_KEYWORD || E === a.RESERVED_FUNCTION_NAME || E === a.RESERVED_KEYWORD_PHRASE || E === a.RESERVED_DATA_TYPE_PHRASE || E === a.RESERVED_CLAUSE || E === a.RESERVED_SELECT || E === a.RESERVED_SET_OPERATION || E === a.RESERVED_JOIN || E === a.ARRAY_KEYWORD || E === a.CASE || E === a.END || E === a.WHEN || E === a.ELSE || E === a.THEN || E === a.LIMIT || E === a.BETWEEN || E === a.AND || E === a.OR || E === a.XOR, be = (E) => E === a.AND || E === a.OR || E === a.XOR, We = [
  "KEYS.NEW_KEYSET",
  ...
  "TIME",
  "TIMEZONE"
], xe = t(["SELECT [ALL | DISTINCT] [AS STRUCT | AS VALUE]"]), $e = t([
  "WITH [RECURSIVE]",
  "FROM",
  "WHERE",
  "GROUP BY",
...
]), OT = t(["CREATE [OR REPLACE] [TEMP|TEMPORARY|SNAPSHOT|EXTERNAL] TABLE [IF NOT EXISTS]"]), PE = t([
...
function Ze(E) {
  let T = K;
  return E.map((e) => e.text === "OFFSET" && T.text === "[" ? (T = e, Object.assign(Object.assign({}, e), { type: a.RESERVED_FUNCTION_NAME })) : (T = e, e));
}
function je(E) {
  var T;
  const e = [];
  for (let R = 0; R < E.length; R++) {
    const A = E[R];
    if ((w.ARRAY(A) || w.STRUCT(A)) && ((T = E[R + 1]) === null || T === void 0 ? void 0 : T.text) === "<") {
      const O = ze(E, R + 1), L = E.slice(R, O + 1);
      e.push({
        type: a.IDENTIFIER,
        raw: L.map(NT("raw")).join(""),
        text: L.map(NT("text")).join(""),
        start: A.start
      }), R = O;
    } else e.push(A);
  }
...
var NT = (E) => (T) => T.type === a.IDENTIFIER || T.type === a.COMMA ? T[E] + " " : T[E];
function ze(E, T) {
  let e = 0;
...
var ER = [
  ...
  "BOOL",
  "BOOLEAN"
], RR = t(["SELECT [DISTINCT]", "MODIFY QUERY SELECT [DISTINCT]"]), AR = t([
  "SET",
  "WITH",
  "FROM",
  "SAMPLE",
...
]), rT = t(["CREATE [OR REPLACE] [TEMPORARY] TABLE [IF NOT EXISTS]"]), lE = t([
  "ALL EXCEPT",
  "ON CLUSTER",
  "UPDATE",
  ...
  "PARTITION ID",
  "{DESC | DESCRIBE} TABLE"
]), SR = t(["UNION [ALL | DISTINCT]", "PARALLEL WITH"]), tR = t(["[GLOBAL] [INNER|LEFT|RIGHT|FULL|CROSS] [OUTER|SEMI|ANTI|ANY|ALL|ASOF] JOIN", "[LEFT] ARRAY JOIN"]), IR = t(["{ROWS | RANGE} BETWEEN", "ALTER MATERIALIZE STATISTICS"]), NR = {
  name: "clickhouse",
  tokenizerOptions: {
    reservedSelect: RR,
    reservedClauses: [
...
function rR(E) {
  return E.map((T, e) => {
    const R = E[e + 1] || K, A = E[e - 1] || K;
    return T.type === a.RESERVED_SELECT && (R.type === a.COMMA || A.type === a.RESERVED_CLAUSE || A.type === a.COMMA) ? Object.assign(Object.assign({}, T), { type: a.RESERVED_KEYWORD }) : w.SET(T) && R.type === a.OPEN_PAREN ? Object.assign(Object.assign({}, T), { type: a.RESERVED_FUNCTION_NAME }) : T;
  });
}
var sR = [
  "ARRAY_AGG",
  ...
  "UPDATE SET",
  "INSERT"
]), sT = t(["CREATE [GLOBAL TEMPORARY | EXTERNAL] TABLE [IF NOT EXISTS]"]), ME = t([
  ...
  "INSERT",
  "FOR SYSTEM NAME"
]), aT = t(["CREATE [OR REPLACE] TABLE"]), UE = t([
  ...
  "SET",
  "RETURNING"
]), nT = t(["CREATE [OR REPLACE] [TEMPORARY | TEMP] TABLE [IF NOT EXISTS]"]), uE = t([
  "UPDATE",
  "ON CONFLICT",
  "DELETE FROM",
  "DROP TABLE [IF EXISTS]",
  ...
  "SIMILAR TO",
  "IS [NOT] DISTINCT FROM"
]), bR = t(["TIMESTAMP WITH TIME ZONE"]), WR = {
  name: "duckdb",
  tokenizerOptions: {
    reservedSelect: YR,
    reservedClauses: [
...
]), CT = t(["CREATE [TEMPORARY] [EXTERNAL] TABLE [IF NOT EXISTS]"]), cE = t([
  "CREATE [MATERIALIZED] VIEW [IF NOT EXISTS]",
  "UPDATE",
  "DELETE FROM",
  "DROP TABLE [IF EXISTS]",
  ...
  "STORED BY",
  "ROW FORMAT"
]), wR = t(["UNION [ALL | DISTINCT]"]), JR = t([
  "JOIN",
  "{LEFT | RIGHT | FULL} [OUTER] JOIN",
  "{INNER | CROSS} JOIN",
  "LEFT SEMI JOIN"
]), kR = t(["{ROWS | RANGE} BETWEEN"]), qR = t([]), QR = {
  name: "hive",
  tokenizerOptions: {
    reservedSelect: $R,
    reservedClauses: [
...
function LE(E) {
  return E.map((T, e) => {
    const R = E[e + 1] || K;
    if (w.SET(T) && R.text === "(") return Object.assign(Object.assign({}, T), { type: a.RESERVED_FUNCTION_NAME });
    const A = E[e - 1] || K;
    return w.VALUES(T) && A.text === "=" ? Object.assign(Object.assign({}, T), { type: a.RESERVED_FUNCTION_NAME }) : T;
  });
}
var ZR = [
  "ACCESSIBLE",
  ...
  "COALESCE",
  "NULLIF"
], EA = t(["SELECT [ALL | DISTINCT | DISTINCTROW]"]), TA = t([
  "WITH [RECURSIVE]",
  "FROM",
  "WHERE",
  "GROUP BY",
  ...
  "SET",
  "RETURNING"
]), LT = t(["CREATE [OR REPLACE] [TEMPORARY] TABLE [IF NOT EXISTS]"]), dE = t([
  ...
  "YEAR",
  "YEARWEEK"
], rA = t(["SELECT [ALL | DISTINCT | DISTINCTROW]"]), sA = t([
  "WITH [RECURSIVE]",
  "FROM",
  "WHERE",
  "GROUP BY",
  ...
  "ON DUPLICATE KEY UPDATE",
  "SET"
]), iT = t(["CREATE [TEMPORARY] TABLE [IF NOT EXISTS]"]), pE = t([
  "CREATE [OR REPLACE] [SQL SECURITY DEFINER | SQL SECURITY INVOKER] VIEW [IF NOT EXISTS]",
  "UPDATE [LOW_PRIORITY] [IGNORE]",
  "DELETE [LOW_PRIORITY] [QUICK] [IGNORE] FROM",
  "DROP [TEMPORARY] TABLE [IF EXISTS]",
  ...
  "YEAR",
  "YEARWEEK"
], PA = t(["SELECT [ALL | DISTINCT | DISTINCTROW]"]), lA = t([
  "WITH [RECURSIVE]",
  "FROM",
  "WHERE",
  "GROUP BY",
  ...
  "ON DUPLICATE KEY UPDATE",
  "SET"
]), oT = t(["CREATE [TEMPORARY] TABLE [IF NOT EXISTS]"]), GE = t([
  "CREATE [OR REPLACE] [SQL SECURITY DEFINER | SQL SECURITY INVOKER] VIEW [IF NOT EXISTS]",
  "UPDATE [LOW_PRIORITY] [IGNORE]",
  "DELETE [LOW_PRIORITY] [QUICK] [IGNORE] FROM",
  "DROP [TEMPORARY] TABLE [IF EXISTS]",
  ...
  "WORK",
  "XOR"
], HA = [], BA = t(["SELECT [ALL | DISTINCT]"]), mA = t([
  "WITH",
  "FROM",
  "WHERE",
  "GROUP BY",
  ...
  "{LEFT | RIGHT} [OUTER] JOIN",
  "INNER JOIN"
]), YA = t(["{ROWS | RANGE | GROUPS} BETWEEN"]), fA = t([]), gA = {
  name: "n1ql",
  tokenizerOptions: {
    reservedSelect: BA,
    reservedClauses: [...mA, ..._T],
  ...
  formatOptions: { onelineClauses: _T }
}, yA = [
  "ADD",
  "AGENT",
  "AGGREGATE",
  ...
  "PRESENTV",
  "PREVIOUS"
], WA = t(["SELECT [ALL | DISTINCT | UNIQUE]"]), vA = t([
  "WITH",
  "FROM",
  "WHERE",
  "GROUP BY",
  ...
  "UPDATE SET",
  "RETURNING"
]), DT = t(["CREATE [GLOBAL TEMPORARY | PRIVATE TEMPORARY | SHARDED | DUPLICATED | IMMUTABLE BLOCKCHAIN | BLOCKCHAIN | IMMUTABLE] TABLE"]), HE = t([
...
function JA(E) {
  let T = K;
  return E.map((e) => w.SET(e) && w.BY(T) ? Object.assign(Object.assign({}, e), { type: a.RESERVED_KEYWORD }) : (qT(e.type) && (T = e), e));
}
var kA = [
  "ABS",
  "ACOS",
  ...
  "SET",
  "RETURNING"
]), PT = t(["CREATE [GLOBAL | LOCAL] [TEMPORARY | TEMP | UNLOGGED] TABLE [IF NOT EXISTS]"]), BE = t([
  ...
  "NULLS {FIRST | LAST}",
  "WITH ORDINALITY"
]), eS = t(["[TIMESTAMP | TIME] {WITH | WITHOUT} TIME ZONE"]), RS = {
  name: "postgresql",
  tokenizerOptions: {
    reservedSelect: ZA,
    reservedClauses: [
  ...
  "VALUES",
  "SET"
]), lT = t(["CREATE [TEMPORARY | TEMP | LOCAL TEMPORARY | LOCAL TEMP] TABLE [IF NOT EXISTS]"]), mE = t([
  ...
  "LOAD DATA [LOCAL] INPATH",
  "[OVERWRITE] INTO TABLE"
]), MT = t(["CREATE [EXTERNAL] TABLE [IF NOT EXISTS]"]), FE = t([
...
function uS(E) {
  return E.map((T, e) => {
    const R = E[e - 1] || K, A = E[e + 1] || K;
    return w.WINDOW(T) && A.type === a.OPEN_PAREN ? Object.assign(Object.assign({}, T), { type: a.RESERVED_FUNCTION_NAME }) : T.text === "ITEMS" && T.type === a.RESERVED_KEYWORD && !(R.text === "COLLECTION" && A.text === "TERMINATED") ? Object.assign(Object.assign({}, T), {
      type: a.IDENTIFIER,
      text: T.raw
    }) : T;
  });
...
var cS = [
  ...
  "SET",
  "RETURNING"
]), UT = t(["CREATE [TEMPORARY | TEMP] TABLE [IF NOT EXISTS]"]), hE = t([
  ...
  "VALUES",
  "SET"
]), uT = t(["CREATE [GLOBAL TEMPORARY | LOCAL TEMPORARY] TABLE"]), YE = t([
  ...
  "NATURAL [INNER] JOIN",
  "NATURAL {LEFT | RIGHT | FULL} [OUTER] JOIN"
]), XS = t(["ON {UPDATE | DELETE} [SET NULL | SET DEFAULT]", "{ROWS | RANGE} BETWEEN"]), xS = t([]), $S = {
  name: "sql",
  tokenizerOptions: {
    reservedSelect: VS,
    reservedClauses: [
  ...
  "SUBSET",
  "DEFINE"
]), cT = t(["CREATE TABLE [IF NOT EXISTS]"]), fE = t([
  "CREATE [OR REPLACE] [MATERIALIZED] VIEW",
  "UPDATE",
  "DELETE FROM",
  "DROP TABLE [IF EXISTS]",
  ...
  "NATURAL [INNER] JOIN",
  "NATURAL {LEFT | RIGHT | FULL} [OUTER] JOIN"
]), jS = t(["{ROWS | RANGE | GROUPS} BETWEEN", "IS [NOT] DISTINCT FROM"]), zS = t([]), Et = {
  name: "trino",
  tokenizerOptions: {
    reservedSelect: kS,
    reservedClauses: [
  ...
  "{INNER | CROSS} JOIN",
  "{CROSS | OUTER} APPLY"
]), Ot = t(["ON {UPDATE | DELETE} [SET NULL | SET DEFAULT]", "{ROWS | RANGE} BETWEEN"]), Nt = t([]), rt = {
  name: "transactsql",
  tokenizerOptions: {
    reservedSelect: At,
    reservedClauses: [
  ...
  "WEEKOFYEAR",
  "YEAR"
], Ct = t(["SELECT [ALL | DISTINCT | DISTINCTROW]"]), Lt = t([
  "WITH",
  "FROM",
  "WHERE",
  "GROUP BY",
...
]), pT = t(["CREATE [ROWSTORE] [REFERENCE | TEMPORARY | GLOBAL TEMPORARY] TABLE [IF NOT EXISTS]"]), yE = t([
  "CREATE VIEW",
  "UPDATE",
  "DELETE [FROM]",
  "DROP [TEMPORARY] TABLE [IF EXISTS]",
...
]), GT = t(["CREATE [OR REPLACE] [VOLATILE] TABLE [IF NOT EXISTS]", "CREATE [OR REPLACE] [LOCAL | GLOBAL] {TEMP|TEMPORARY} TABLE [IF NOT EXISTS]"]), VE = t([
  ...
  "[NATURAL] {LEFT | RIGHT | FULL} [OUTER] JOIN",
  "{CROSS | NATURAL} JOIN"
]), Gt = t(["{ROWS | RANGE} BETWEEN", "ON {UPDATE | DELETE} [SET NULL | SET DEFAULT]"]), Ht = t([]), Bt = {
  name: "snowflake",
  tokenizerOptions: {
    reservedSelect: ut,
    reservedClauses: [
    ...
    reservedJoins: pt,
    reservedKeywordPhrases: Gt,
    reservedDataTypePhrases: Ht,
    reservedKeywords: Mt,
  ...
  transactsql: () => rt,
  trino: () => Et
}), EE = (E) => E[E.length - 1], QT = (E) => E.sort((T, e) => e.length - T.length || T.localeCompare(e)), rE = (E) => E.replace(/\s+/gu, " "), bE = (E) => /\n/.test(E), W = (E) => E.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), HT = /\s+/uy, k = (E) => new RegExp(`(?:${E})`, "uy"), Ft = (E) => E.split("").map((T) => / /gu.test(T) ? "\\s+" : `[${T.toUpperCase()}${T.toLowerCase()}]`).join(""), ht = (E) => E + "(?:-" + E + ")*", Yt = ({ prefixes: E, requirePrefix: T }) => `(?:${E.map(Ft).join("|")}${T ? "" : "|"})`, ft = (E) => new RegExp(`(?:${E.map(W).join("|")}).*?(?=\r
|\r|
|$)`, "uy"), BT = (E, T = []) => {
  const e = E === "open" ? 0 : 1;
  return k(["()", ...T].map((R) => R[e]).map(W).join("|"));
}, mT = (E) => k(`${QT(E).map(W).join("|")}`), gt = ({ rest: E, dashes: T }) => E || T ? `(?![${E || ""}${T ? "-" : ""}])` : "", X = (E, T = {}) => {
  if (E.length === 0) return /^\b$/u;
  const e = gt(T), R = QT(E).map(W).join("|").replace(/ /gu, "\\s+");
  return new RegExp(`(?:${R})${e}\\b`, "iuy");
}, WE = (E, T) => {
  if (E.length)
    return k(`(?:${E.map(W).join("|")})(?:${T})`);
}, yt = () => {
  const E = {
    "<": ">",
    "[": "]",
    "(": ")",
    "{": "}"
  }, T = "{left}(?:(?!{right}').)*?{right}", e = Object.entries(E).map(([A, O]) => T.replace(/{left}/g, W(A)).replace(/{right}/g, W(O))), R = W(Object.keys(E).join(""));
  ...
  "{}": String.raw`(?:\{[^\}]*\})`,
  "q''": yt()
}, ZT = (E) => typeof E == "string" ? FT[E] : "regex" in E ? E.regex : Yt(E) + FT[E.quote], Vt = (E) => k(E.map((T) => "regex" in T ? T.regex : ZT(T)).join("|")), jT = (E) => E.map(ZT).join("|"), hT = (E) => k(jT(E)), bt = (E = {}) => k(zT(E)), zT = ({ first: E, rest: T, dashes: e, allowFirstCharNumber: R } = {}) => {
  const A = "\\p{Alphabetic}\\p{Mark}_", O = "\\p{Decimal_Number}", L = W(E ?? ""), I = W(T ?? ""), o = R ? `[${A}${O}${L}][${A}${O}${I}]*` : `[${A}${L}][${A}${O}${I}]*`;
  return e ? ht(o) : o;
};
function Ee(E, T) {
...
var Wt = class {
  ...
  createParseError() {
    const E = this.input.slice(this.index, this.index + 10), { line: T, col: e } = Ee(this.input, this.index);
...
If possible, please select a more specific dialect (like sqlite, postgresql, etc).` : `SQL dialect used: "${this.dialectName}".`;
...
}, YT = /\/\*/uy, vt = /[\s\S]/uy, Xt = /\*\//uy, xt = class {
  constructor() {
    this.lastIndex = 0;
  }
  exec(E) {
    let T = "", e, R = 0;
    if (e = this.matchSection(YT, E))
      T += e, R++;
    else return null;
    for (; R > 0; ) if (e = this.matchSection(YT, E))
      T += e, R++;
    else if (e = this.matchSection(Xt, E))
      T += e, R--;
    else if (e = this.matchSection(vt, E)) T += e;
    else return null;
    return [T];
  }
  matchSection(E, T) {
  ...
  buildRulesBeforeParams(E) {
    ...
    return this.validRules([
      ...
      },
      {
        ...
        regex: E.nestedBlockComments ? new xt() : /(\/\*[^]*?\*\/)/uy
      },
      {
        type: a.LINE_COMMENT,
        regex: ft((T = E.lineCommentTypes) !== null && T !== void 0 ? T : ["--"])
      },
      {
      ...
      },
      {
        ...
        regex: X((e = E.reservedKeywordPhrases) !== null && e !== void 0 ? e : [], E.identChars),
        text: h
      },
      {
        type: a.RESERVED_DATA_TYPE_PHRASE,
        regex: X((R = E.reservedDataTypePhrases) !== null && R !== void 0 ? R : [], E.identChars),
        text: h
      },
      {
        type: a.CASE,
      ...
      {
        type: a.RESERVED_CLAUSE,
        regex: X(E.reservedClauses, E.identChars),
        text: h
      },
      {
        type: a.RESERVED_SELECT,
        regex: X(E.reservedSelect, E.identChars),
        text: h
      },
      {
        type: a.RESERVED_SET_OPERATION,
        regex: X(E.reservedSetOperations, E.identChars),
        text: h
      },
      {
        type: a.WHEN,
      ...
      },
      {
        ...
        regex: X(E.reservedJoins, E.identChars),
        text: h
      },
      {
        type: a.AND,
      ...
      }] : [],
      {
        ...
        regex: X(E.reservedFunctionNames, E.identChars),
        text: h
      },
      {
        type: a.RESERVED_DATA_TYPE,
        regex: X(E.reservedDataTypes, E.identChars),
        text: h
      },
      {
        type: a.RESERVED_KEYWORD,
        regex: X(E.reservedKeywords, E.identChars),
        text: h
      }
    ]);
  }
  buildRulesAfterParams(E) {
    ...
    return this.validRules([
      {
        type: a.VARIABLE,
        regex: E.variableTypes ? Vt(E.variableTypes) : void 0
      },
      {
        type: a.STRING,
      ...
      },
      {
        ...
        regex: mT([".", ...(e = E.propertyAccessOperators) !== null && e !== void 0 ? e : []])
      }
    ]);
  }
  buildParamRules(E, T) {
    ...
    const I = {
      ...
      positional: typeof T?.positional == "boolean" ? T.positional : (O = E.paramTypes) === null || O === void 0 ? void 0 : O.positional,
      custom: T?.custom || ((L = E.paramTypes) === null || L === void 0 ? void 0 : L.custom) || []
    };
    return this.validRules([
      {
        type: a.NAMED_PARAMETER,
        regex: WE(I.named, zT(E.paramChars || E.identChars)),
        key: (o) => o.slice(1)
      },
      {
        type: a.QUOTED_PARAMETER,
        regex: WE(I.quoted, jT(E.identTypes)),
        key: (o) => (({ tokenKey: C, quoteChar: S }) => C.replace(new RegExp(W("\\" + S), "gu"), S))({
      ...
      ...I.custom.map((o) => {
        ...
        return {
          ...
          regex: k(o.regex),
          key: (C = o.key) !== null && C !== void 0 ? C : ((S) => S)
        };
      })
    ]);
  }
...
}, h = (E) => rE(E.toUpperCase()), fT = /* @__PURE__ */ new Map(), Kt = (E) => {
  let T = fT.get(E);
  return T || (T = wt(E), fT.set(E, T)), T;
}, wt = (E) => ({
  tokenizer: new $t(E.tokenizerOptions, E.name),
  formatOptions: Jt(E.formatOptions)
}), Jt = (E) => {
  var T;
  return {
...
var qt = class {
  ...
  (function(e, R) {
    ...
    function e(S, n, i) {
      return this.id = ++e.highestId, this.name = S, this.symbols = n, this.postprocess = i, this;
    }
    e.highestId = 0, e.prototype.toString = function(S) {
      var n = typeof S > "u" ? this.symbols.map(C).join(" ") : this.symbols.slice(0, S).map(C).join(" ") + " ● " + this.symbols.slice(S).map(C).join(" ");
      return this.name + " → " + n;
    };
    function R(S, n, i, M) {
      this.rule = S, this.dot = n, this.reference = i, this.data = [], this.wantedBy = M, this.isComplete = this.dot === S.symbols.length;
    }
    R.prototype.toString = function() {
      return "{" + this.rule.toString(this.dot) + "}, from: " + (this.reference || 0);
    }, R.prototype.nextState = function(S) {
    ...
    A.prototype.process = function(S) {
      for (var n = this.states, i = this.wants, M = this.completed, H = 0; H < n.length; H++) {
        var U = n[H];
        if (U.isComplete) {
          if (U.finish(), U.data !== I.fail) {
            for (var d = U.wantedBy, B = d.length; B--; ) {
              var s = d[B];
              this.complete(s, U);
            }
            if (U.reference === this.index) {
              var N = U.rule.name;
              (this.completed[N] = this.completed[N] || []).push(U);
            }
          }
        } else {
          var N = U.rule.symbols[U.dot];
          if (typeof N != "string") {
            this.scannable.push(U);
            continue;
          }
          if (i[N]) {
            if (i[N].push(U), M.hasOwnProperty(N))
              for (var _ = M[N], B = 0; B < _.length; B++) {
                var P = _[B];
                this.complete(U, P);
              }
          } else
            i[N] = [U], this.predict(N);
        }
      }
    }, A.prototype.predict = function(S) {
      for (var n = this.grammar.byName[S] || [], i = 0; i < n.length; i++) {
        var M = n[i], H = this.wants[S], U = new R(M, 0, this.index, H);
        this.states.push(U);
      }
    }, A.prototype.complete = function(S, n) {
      var i = S.nextState(n);
      this.states.push(i);
    };
    function O(S, n) {
      ...
      this.rules.forEach(function(M) {
        i.hasOwnProperty(M.name) || (i[M.name] = []), i[M.name].push(M);
      });
    }
    O.fromCompiled = function(M, n) {
      var i = M.Lexer;
      M.ParserStart && (n = M.ParserStart, M = M.ParserRules);
    ...
    L.prototype.reset = function(S, n) {
...
` && (this.line += 1, this.lastLineBreak = this.index), { value: S };
    ...
    }, L.prototype.formatError = function(S, n) {
      var i = this.buffer;
      if (typeof i == "string") {
        var M = i.split(`
`).slice(Math.max(0, this.line - 5), this.line), H = i.indexOf(`
`, this.index);
        H === -1 && (H = i.length);
        var U = this.index - this.lastLineBreak, d = String(this.line).length;
        return n += " at line " + this.line + " col " + U + `:

`, n += M.map(function(s, N) {
          return B(this.line - M.length + N + 1, d) + " " + s;
        }, this).join(`
`), n += `
` + B("", d + U) + `^
`, n;
      } else return n + " at index " + (this.index - 1);
      function B(s, N) {
        var _ = String(s);
    ...
    I.fail = {}, I.prototype.feed = function(S) {
      ...
      for (var i; ; ) {
        try {
          ...
          this.table.push(d);
          var M = new Error(this.reportLexerError(c));
          throw M.offset = this.current, M.token = c.token, M;
        }
        var H = this.table[this.current];
        this.options.keepHistory || delete this.table[this.current - 1];
        var U = this.current + 1, d = new A(this.grammar, U);
        this.table.push(d);
        for (var B = i.text !== void 0 ? i.text : i.value, s = n.constructor === L ? i.value : i, N = H.scannable, _ = N.length; _--; ) {
          var P = N[_], l = P.rule.symbols[P.dot];
          if (l.test ? l.test(s) : l.type ? l.type === i.type : l.literal === B) {
            var p = P.nextState({
              data: s,
              token: i,
              isToken: !0,
        ...
        if (d.process(), d.states.length === 0) {
          var M = new Error(this.reportError(i));
          throw M.offset = this.current, M.token = i, M;
        }
        this.options.keepHistory && (H.lexerState = n.save()), this.current++;
      }
      return H && (this.lexerState = n.save()), this.results = this.finish(), this;
    }, I.prototype.reportLexerError = function(S) {
      var n, i, M = S.token;
      return M ? (n = "input " + JSON.stringify(M.text[0]) + " (lexer error)", i = this.lexer.formatError(M, "Syntax error")) : (n = "input (lexer error)", i = S.message), this.reportErrorCommon(i, n);
    }, I.prototype.reportError = function(S) {
      var n = (S.type ? S.type + " token: " : "") + JSON.stringify(S.value !== void 0 ? S.value : S), i = this.lexer.formatError(S, "Syntax error");
      return this.reportErrorCommon(i, n);
    }, I.prototype.reportErrorCommon = function(S, n) {
      var i = [];
      i.push(S);
      var M = this.table.length - 2, H = this.table[M], U = H.states.filter(function(d) {
        var B = d.rule.symbols[d.dot];
        return B && typeof B != "string";
      });
      return U.length === 0 ? (i.push("Unexpected " + n + `. I did not expect any more input. Here is the state of my parse table:
`), this.displayStateStack(H.states, i)) : (i.push("Unexpected " + n + `. Instead, I was expecting to see one of the following:
`), U.map(function(d) {
        return this.buildFirstStateStack(d, []) || [d];
      }, this).forEach(function(d) {
        var B = d[0], s = B.rule.symbols[B.dot], N = this.getSymbolDisplay(s);
        i.push("A " + N + " based on:"), this.displayStateStack(d, i);
      }, this)), i.push(""), i.join(`
`);
    }, I.prototype.displayStateStack = function(S, n) {
      for (var i, M = 0, H = 0; H < S.length; H++) {
        var U = S[H], d = U.rule.toString(U.dot);
        d === i ? M++ : (M > 0 && n.push("    ^ " + M + " more lines identical to this"), M = 0, n.push("    " + d)), i = d;
      }
    }, I.prototype.getSymbolDisplay = function(S) {
      return o(S);
    }, I.prototype.buildFirstStateStack = function(S, n) {
      if (n.indexOf(S) !== -1) return null;
      if (S.wantedBy.length === 0) return [S];
      var i = S.wantedBy[0], M = [S].concat(n), H = this.buildFirstStateStack(i, M);
      return H === null ? null : [S].concat(H);
    }, I.prototype.save = function() {
      var S = this.table[this.current];
      return S.lexerState = this.lexerState, S;
    }, I.prototype.restore = function(S) {
      var n = S.index;
      this.current = n, this.table[n] = S, this.table.splice(n + 1), this.lexerState = S.lexerState, this.results = this.finish();
    }, I.prototype.rewind = function(S) {
      ...
      var S = [], n = this.grammar.start;
      return this.table[this.table.length - 1].states.forEach(function(i) {
        i.rule.name === n && i.dot === i.rule.symbols.length && i.reference === 0 && i.data !== I.fail && S.push(i);
      }), S.map(function(i) {
        return i.data;
      });
    };
...
function Zt(E) {
  return E.map(jt).map(zt).map(EI).map(TI).map(eI);
}
var jt = (E, T, e) => {
  if (qT(E.type)) {
    const R = RI(e, T);
    if (R && R.type === a.PROPERTY_ACCESS_OPERATOR) return Object.assign(Object.assign({}, E), {
      type: a.IDENTIFIER,
      text: E.raw
    });
    const A = j(e, T);
    if (A && A.type === a.PROPERTY_ACCESS_OPERATOR) return Object.assign(Object.assign({}, E), {
      type: a.IDENTIFIER,
      text: E.raw
    });
  }
  ...
  if (E.type === a.RESERVED_FUNCTION_NAME) {
    const R = j(e, T);
    if (!R || !Te(R)) return Object.assign(Object.assign({}, E), {
      type: a.IDENTIFIER,
      text: E.raw
    });
  }
  ...
  if (E.type === a.RESERVED_DATA_TYPE) {
    const R = j(e, T);
    if (R && Te(R)) return Object.assign(Object.assign({}, E), { type: a.RESERVED_PARAMETERIZED_DATA_TYPE });
  }
  return E;
}, TI = (E, T, e) => {
  if (E.type === a.IDENTIFIER) {
    const R = j(e, T);
    if (R && ee(R)) return Object.assign(Object.assign({}, E), { type: a.ARRAY_IDENTIFIER });
  }
  return E;
}, eI = (E, T, e) => {
  if (E.type === a.RESERVED_DATA_TYPE) {
    const R = j(e, T);
    if (R && ee(R)) return Object.assign(Object.assign({}, E), { type: a.ARRAY_KEYWORD });
  }
  return E;
}, RI = (E, T) => j(E, T, -1), j = (E, T, e = 1) => {
  let R = 1;
  for (; E[T + R * e] && AI(E[T + R * e]); ) R++;
  return E[T + R * e];
}, Te = (E) => E.type === a.OPEN_PAREN && E.text === "(", ee = (E) => E.type === a.OPEN_PAREN && E.text === "[", AI = (E) => E.type === a.BLOCK_COMMENT || E.type === a.LINE_COMMENT, Re = class {
  constructor(E) {
  ...
  formatError(E) {
    const { line: T, col: e } = Ee(this.input, E.start);
...
var G = new Re((E) => []), J = ([[E]]) => E, Y = (E) => ({
  ...
  text: E.text,
  raw: E.raw
}), f = (E, { leading: T, trailing: e }) => (T?.length && (E = Object.assign(Object.assign({}, E), { leadingComments: T })), e?.length && (E = Object.assign(Object.assign({}, E), { trailingComments: e })), E), SI = (E, { leading: T, trailing: e }) => {
  if (T?.length) {
    const [R, ...A] = E;
    E = [f(R, { leading: T }), ...A];
  }
  ...
  ParserRules: [
    ...
    },
    {
      ...
      postprocess: ([E, [T]]) => ({
        type: u.statement,
        children: E,
        hasSemicolon: T.type === a.DELIMITER
      })
    },
    {
    ...
    },
    {
      ...
      postprocess: ([E, T, e, R, A, O]) => ({
        type: u.case_when,
        whenKw: f(Y(E), { trailing: T }),
        thenKw: f(Y(R), { trailing: A }),
        condition: e,
        result: O
      })
    },
  ...
  ParserStart: "main"
}, II = /* @__PURE__ */ XT(Qt(), 1), { Parser: OI, Grammar: NI } = II.default;
function rI(E) {
  let T = {};
  const e = new Re((A) => [...Zt(E.tokenize(A, T)), kT(A.length)]), R = new OI(NI.fromCompiled(tI), { lexer: e });
  return { parse: (A, O) => {
    T = O;
    const { results: L } = R.feed(A);
    if (L.length === 1) return L[0];
    throw L.length === 0 ? new Error("Parse error: Invalid SQL") : new Error(`Parse error: Ambiguous grammar
...
var Ae = class {
  ...
  trimHorizontalWhitespace() {
    for (; sI(EE(this.items)); ) this.items.pop();
  }
  trimWhitespace() {
    for (; aI(EE(this.items)); ) this.items.pop();
  }
  addNewline(E) {
    if (this.items.length > 0) switch (EE(this.items)) {
      case r.NEWLINE:
  ...
  addIndentation() {
    for (let E = 0; E < this.indentation.getLevel(); E++) this.items.push(r.SINGLE_INDENT);
  }
  toString() {
...
}, sI = (E) => E === r.SPACE || E === r.SINGLE_INDENT, aI = (E) => E === r.SPACE || E === r.SINGLE_INDENT || E === r.NEWLINE;
function yT(E, T) {
...
function VT(E) {
  return be(E) || E === a.RESERVED_CLAUSE || E === a.RESERVED_SELECT || E === a.RESERVED_SET_OPERATION || E === a.RESERVED_JOIN || E === a.LIMIT;
}
var XE = "top-level", nI = "block-level", Se = class {
  constructor(E) {
  ...
  increaseBlockLevel() {
    this.indentTypes.push(nI);
  }
  decreaseTopLevel() {
    this.indentTypes.length > 0 && EE(this.indentTypes) === XE && this.indentTypes.pop();
  }
  decreaseBlockLevel() {
  ...
  constructor(E) {
    super(new Se("")), this.expressionWidth = E, this.length = 0, this.trailingSpace = !1;
  }
  add(...E) {
  ...
  addToLength(E) {
    if (typeof E == "string")
      ...
      if (E === r.MANDATORY_NEWLINE || E === r.NEWLINE) throw new QE();
  ...
  constructor({ cfg: T, dialectCfg: e, params: R, layout: A, inline: O = !1 }) {
    this.inline = !1, this.nodes = [], this.index = -1, this.cfg = T, this.dialectCfg = e, this.inline = O, this.params = R, this.layout = A;
  }
  format(T) {
  ...
  formatPropertyAccess(T) {
    this.formatNode(T.object), this.layout.add(r.NO_SPACE, T.operator), this.formatNode(T.property);
  }
  formatParenthesis(T) {
    const e = this.formatInlineExpression(T.children);
    e ? (this.layout.add(T.openParen), this.layout.add(...e.getLayoutItems()), this.layout.add(r.NO_SPACE, T.closeParen, r.SPACE)) : (this.layout.add(T.openParen, r.NEWLINE), Z(this.cfg) ? (this.layout.add(r.INDENT), this.layout = this.formatSubExpression(T.children)) : (this.layout.indentation.increaseBlockLevel(), this.layout.add(r.INDENT), this.layout = this.formatSubExpression(T.children), this.layout.indentation.decreaseBlockLevel()), this.layout.add(r.NEWLINE, r.INDENT, T.closeParen, r.SPACE));
  }
  formatBetweenPredicate(T) {
    this.layout.add(this.showKw(T.betweenKw), r.SPACE), this.layout = this.formatSubExpression(T.expr1), this.layout.add(r.NO_SPACE, r.SPACE, this.showNonTabularKw(T.andKw), r.SPACE), this.layout = this.formatSubExpression(T.expr2), this.layout.add(r.SPACE);
  }
  formatCaseExpression(T) {
    this.formatNode(T.caseKw), this.layout.indentation.increaseBlockLevel(), this.layout = this.formatSubExpression(T.expr), this.layout = this.formatSubExpression(T.clauses), this.layout.indentation.decreaseBlockLevel(), this.layout.add(r.NEWLINE, r.INDENT), this.formatNode(T.endKw);
  }
  formatCaseWhen(T) {
    this.layout.add(r.NEWLINE, r.INDENT), this.formatNode(T.whenKw), this.layout = this.formatSubExpression(T.condition), this.formatNode(T.thenKw), this.layout = this.formatSubExpression(T.result);
  }
  formatCaseElse(T) {
    this.layout.add(r.NEWLINE, r.INDENT), this.formatNode(T.elseKw), this.layout = this.formatSubExpression(T.result);
  }
  formatClause(T) {
    this.isOnelineClause(T) ? this.formatClauseInOnelineStyle(T) : Z(this.cfg) ? this.formatClauseInTabularStyle(T) : this.formatClauseInIndentedStyle(T);
  }
  isOnelineClause(T) {
    return Z(this.cfg) ? this.dialectCfg.tabularOnelineClauses[T.nameKw.text] : this.dialectCfg.onelineClauses[T.nameKw.text];
  }
  formatClauseInIndentedStyle(T) {
    this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T.nameKw), r.NEWLINE), this.layout.indentation.increaseTopLevel(), this.layout.add(r.INDENT), this.layout = this.formatSubExpression(T.children), this.layout.indentation.decreaseTopLevel();
  }
  formatClauseInOnelineStyle(T) {
    this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T.nameKw), r.SPACE), this.layout = this.formatSubExpression(T.children);
  }
  formatClauseInTabularStyle(T) {
    this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T.nameKw), r.SPACE), this.layout.indentation.increaseTopLevel(), this.layout = this.formatSubExpression(T.children), this.layout.indentation.decreaseTopLevel();
  }
  formatSetOperation(T) {
    this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T.nameKw), r.NEWLINE), this.layout.add(r.INDENT), this.layout = this.formatSubExpression(T.children);
  }
  formatLimitClause(T) {
    this.withComments(T.limitKw, () => {
      this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T.limitKw));
    }), this.layout.indentation.increaseTopLevel(), Z(this.cfg) ? this.layout.add(r.SPACE) : this.layout.add(r.NEWLINE, r.INDENT), T.offset ? (this.layout = this.formatSubExpression(T.offset), this.layout.add(r.NO_SPACE, ",", r.SPACE), this.layout = this.formatSubExpression(T.count)) : this.layout = this.formatSubExpression(T.count), this.layout.indentation.decreaseTopLevel();
  }
  formatAllColumnsAsterisk(T) {
    this.layout.add("*", r.SPACE);
  }
  formatLiteral(T) {
    this.layout.add(T.text, r.SPACE);
  }
  formatIdentifier(T) {
    this.layout.add(this.showIdentifier(T), r.SPACE);
  }
  formatParameter(T) {
    this.layout.add(this.params.get(T), r.SPACE);
  }
  formatOperator({ text: T }) {
    this.cfg.denseOperators || this.dialectCfg.alwaysDenseOperators.includes(T) ? this.layout.add(r.NO_SPACE, T) : T === ":" ? this.layout.add(r.NO_SPACE, T, r.SPACE) : this.layout.add(T, r.SPACE);
  }
  formatComma(T) {
  ...
  formatLineComment(T) {
    bE(T.precedingWhitespace || "") ? this.layout.add(r.NEWLINE, r.INDENT, T.text, r.MANDATORY_NEWLINE, r.INDENT) : this.layout.getLayoutItems().length > 0 ? this.layout.add(r.NO_NEWLINE, r.SPACE, T.text, r.MANDATORY_NEWLINE, r.INDENT) : this.layout.add(T.text, r.MANDATORY_NEWLINE, r.INDENT);
  }
  formatBlockComment(T) {
    T.type === u.block_comment && this.isMultilineBlockComment(T) ? (this.splitBlockComment(T.text).forEach((e) => {
      this.layout.add(r.NEWLINE, r.INDENT, e);
    }), this.layout.add(r.NEWLINE, r.INDENT)) : this.layout.add(T.text, r.SPACE);
  }
  isMultilineBlockComment(T) {
    return bE(T.text) || bE(T.precedingWhitespace || "");
  }
  isDocComment(T) {
    const e = T.split(/\n/);
    return /^\/\*\*?$/.test(e[0]) && e.slice(1, e.length - 1).every((R) => /^\s*\*/.test(R)) && /^\s*\*\/$/.test(EE(e));
  }
  splitBlockComment(T) {
  ...
  formatInlineExpression(T) {
    ...
    try {
      return new ZE({
        ...
        dialectCfg: this.dialectCfg,
        params: this.params,
        layout: new CI(this.cfg.expressionWidth),
        inline: !0
      }).format(T);
    } catch (R) {
      if (R instanceof QE) {
  ...
  formatJoin(T) {
    Z(this.cfg) ? (this.layout.indentation.decreaseTopLevel(), this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T), r.SPACE), this.layout.indentation.increaseTopLevel()) : this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T), r.SPACE);
  }
  formatKeyword(T) {
    this.layout.add(this.showKw(T), r.SPACE);
  }
  formatLogicalOperator(T) {
    this.cfg.logicalOperatorNewline === "before" ? Z(this.cfg) ? (this.layout.indentation.decreaseTopLevel(), this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T), r.SPACE), this.layout.indentation.increaseTopLevel()) : this.layout.add(r.NEWLINE, r.INDENT, this.showKw(T), r.SPACE) : this.layout.add(this.showKw(T), r.NEWLINE, r.INDENT);
  }
  formatDataType(T) {
    this.layout.add(this.showDataType(T), r.SPACE);
  }
  showKw(T) {
    return VT(T.tokenType) ? yT(this.showNonTabularKw(T), this.cfg.indentStyle) : this.showNonTabularKw(T);
  }
  showNonTabularKw(T) {
  ...
  showFunctionKw(T) {
    return VT(T.tokenType) ? yT(this.showNonTabularFunctionKw(T), this.cfg.indentStyle) : this.showNonTabularFunctionKw(T);
  }
  ...
  constructor(E, T) {
    this.dialect = E, this.cfg = T, this.params = new qt(this.cfg.params);
  }
  format(E) {
  ...
  parse(E) {
    return rI(this.dialect.tokenizer).parse(E, this.cfg.paramTypes || {});
  }
  formatAst(E) {
...
`.repeat(this.cfg.linesBetweenQueries + 1));
  ...
  formatStatement(E) {
    const T = new LI({
      ...
      params: this.params,
      layout: new Ae(new Se(kt(this.cfg)))
    }).format(E.children);
    return E.hasSemicolon && (this.cfg.newlineBeforeSemicolon ? T.add(r.NEWLINE, ";") : T.add(r.NO_NEWLINE, ";")), T.toString();
  }
...
function oI(E) {
  for (const T of [
    ...
    "commaPosition",
    "tabulateAlias"
  ]) if (T in E) throw new sE(`${T} config is no more supported.`);
  if (E.expressionWidth <= 0) throw new sE(`expressionWidth config must be positive number. Received ${E.expressionWidth} instead.`);
  if (E.params && !_I(E.params) && console.warn('WARNING: All "params" option values should be strings.'), E.paramTypes && !DI(E.paramTypes)) throw new sE("Empty regex given in custom paramTypes. That would result in matching infinite amount of parameters.");
  return E;
}
function _I(E) {
...
var PI = function(E, T) {
  ...
  if (typeof T.language == "string" && !lI.includes(T.language)) throw new sE(`Unsupported SQL dialect: ${T.language}`);
  const e = te[T.language || "sql"];
  return uI(E, Object.assign(Object.assign({}, T), { dialect: mt[e] }));
}, uI = (E, T) => {
  var { dialect: e } = T, R = PI(T, ["dialect"]);
  if (typeof E != "string") throw new Error("Invalid query argument. Expected string, instead got " + typeof E);
  const A = oI(Object.assign(Object.assign({}, MI), R));
  return new iI(Kt(e), A).format(E);
}, nE = /* @__PURE__ */ XT(Be(), 1);
function cI(E) {
...
function dI(E, T = !1) {
  if (!E || typeof E != "string") return "";
  const e = T ? cI(E) : E;
  return nE.default.highlight(e, nE.default.languages.sql, "sql");
}
function TE(E, T = !0) {
  let e;
  ...
  return nE.default.highlight(e, nE.default.languages.json, "json");
}
function pI(E) {
...
function BI(E, T, e = {}) {
  const { maskPlaceholder: R = "***", maxDetailLength: A } = e, O = [], L = [];
  if (E.id && L.push(`<span>ID: <code>${D(E.id)}</code></span>`), E.remote_ip && L.push(`<span>IP: <code>${D(E.remote_ip)}</code></span>`), E.content_type && L.push(`<span>Content-Type: <code>${D(E.content_type)}</code></span>`), L.length > 0 && O.push(`<div class="${T.detailMetadataLine}">${L.join("")}</div>`), E.headers && Object.keys(E.headers).length > 0) {
    const I = Object.entries(E.headers).map(([o, C]) => {
      const S = A && C.length > A ? aE(C, A) : C, n = C === R ? ` <span class="${T.detailMasked}">(masked)</span>` : "";
      return `<dt>${D(o)}</dt><dd>${D(S)}${n}</dd>`;
    }).join("");
    O.push(`
      <div class="${T.detailSection}">
        <span class="${T.detailLabel}">Request Headers</span>
        <dl class="${T.detailKeyValueTable}">${I}</dl>
      </div>
    `);
  }
  if (E.query && Object.keys(E.query).length > 0) {
    const I = Object.entries(E.query).map(([o, C]) => {
      const S = C === R ? ` <span class="${T.detailMasked}">(masked)</span>` : "";
      return `<dt>${D(o)}</dt><dd>${D(C)}${S}</dd>`;
    }).join("");
    O.push(`
      <div class="${T.detailSection}">
  ...
  if (E.request_body) {
    const I = E.request_size ? ` (${tT(E.request_size)})` : "", o = E.body_truncated ? ' <span class="' + T.detailMasked + '">(truncated)</span>' : "";
    let C;
    try {
      C = TE(JSON.parse(E.request_body), !0);
    } catch {
      C = D(E.request_body);
    }
    O.push(`
      <div class="${T.detailSection}">
        <span class="${T.detailLabel}">Request Body${I}${o}</span>
  ...
  if (E.response_headers && Object.keys(E.response_headers).length > 0) {
    const I = Object.entries(E.response_headers).map(([o, C]) => {
      const S = A && C.length > A ? aE(C, A) : C;
      return `<dt>${D(o)}</dt><dd>${D(S)}</dd>`;
    }).join("");
    O.push(`
      <div class="${T.detailSection}">
  ...
  if (E.response_body) {
    const I = E.response_size ? ` (${tT(E.response_size)})` : "";
    let o;
    try {
      o = TE(JSON.parse(E.response_body), !0);
    } catch {
      o = D(E.response_body);
    }
    O.push(`
      <div class="${T.detailSection}">
        <span class="${T.detailLabel}">Response Body${I}</span>
        <div class="${T.detailBody}">
          <pre>${o}</pre>
        </div>
...
function mI(E, T, e, R) {
  const { display: A, classToken: O } = pI(E.method), L = E.path || "", I = E.status || 0, o = jE(E.duration, R.slowThresholdMs), C = GI(E, T), S = R.expandedRequestIds?.has(C) || !1, n = e.badgeMethod(O), i = e.badgeStatus(I), M = o.isSlow ? e.durationSlow : "", H = I >= 400 ? e.rowError : "", U = R.truncatePath ? aE(L, R.maxPathLength || 50) : L;
  let d = "";
  const B = A;
  if (B === "POST" || B === "PUT" || B === "PATCH") {
    const l = (E.content_type || E.headers?.["Content-Type"] || E.headers?.["content-type"] || "").split(";")[0].trim();
    l && (d = ` <span class="${e.badgeContentType}">${D(l)}</span>`);
  }
  const s = `<span class="${e.expandIcon}" data-expand-icon>${S ? "▼" : "▶"}</span>`, N = S ? "table-row" : "none", _ = BI(E, e, {
    maskPlaceholder: R.maskPlaceholder,
    maxDetailLength: R.maxDetailLength
  }), P = S ? _ : `<template data-request-detail-template>${_}</template>`;
  return `
    <tr class="${H}" data-request-id="${D(C)}" style="cursor:pointer">
      <td>${s}<span class="${n}">${D(A)}</span>${d}</td>
      <td class="${e.path}" title="${D(L)}">${D(U)}</td>
      <td><span class="${i}">${D(I || "-")}</span></td>
      <td class="${e.duration} ${M}">${o.text}</td>
      <td class="${e.timestamp}">${D(eE(E.timestamp))}</td>
    </tr>
    <tr class="${e.detailRow}" data-detail-for="${D(C)}" style="display:${N}">
      <td colspan="5">${P}</td>
    </tr>
  `;
}
function xE(E, T, e = {}) {
  const { newestFirst: R = !0, slowThresholdMs: A = 50, maxEntries: O, showSortToggle: L = !1, truncatePath: I = !0, maxPathLength: o = 50 } = e, C = L ? HI("requests", R, T) : "";
  if (!E.length) return C + `<div class="${T.emptyState}">No requests captured</div>`;
  const S = O ? Math.max(0, E.length - O) : 0;
  let n = (O ? E.slice(-O) : E).map((M, H) => ({
    entry: M,
    originalIndex: S + H
  }));
  R && (n = [...n].reverse());
  const i = n.map(({ entry: M, originalIndex: H }) => mI(M, H, T, {
    ...e,
    slowThresholdMs: A,
    truncatePath: I,
    maxPathLength: o
...
function fI(E, T, e, R) {
  const A = jE(E.duration, R.slowThresholdMs), O = A.isSlow, L = !!E.error, I = `sql-row-${T}`, o = E.query || "", C = dI(o, !0), S = [e.expandableRow];
  O && S.push(e.slowQuery), L && S.push(e.errorQuery);
  const n = O ? e.durationSlow : "", i = YI(e, R.useIconCopyButton || !1, I);
  return `
    <tr class="${S.join(" ")}" data-row-id="${I}">
      <td class="${e.selectCell}"><input type="checkbox" class="sql-select-row" data-sql-index="${T}"></td>
      <td class="${e.duration} ${n}">${A.text}</td>
      <td>${D(CE(E.row_count ?? "-"))}</td>
      <td class="${e.timestamp}">${D(eE(E.timestamp))}</td>
      <td>${L ? `<span class="${e.badgeError}">Error</span>` : ""}</td>
      <td class="${e.queryText}"><span class="${e.expandIcon}">&#9654;</span>${D(o)}</td>
    </tr>
    <tr class="${e.expansionRow}" data-expansion-for="${I}">
      <td colspan="6">
...
function $E(E, T, e = {}) {
  const { newestFirst: R = !0, slowThresholdMs: A = 50, maxEntries: O = 50, showSortToggle: L = !1, useIconCopyButton: I = !1 } = e, o = L ? FI("sql", R, T) : "", C = hI(T);
  if (!E.length) return o + `<div class="${T.emptyState}">No SQL queries captured</div>`;
  let S = O ? E.slice(-O) : E;
  R && (S = [...S].reverse());
  const n = S.map((i, M) => fI(i, M, T, {
    ...e,
    slowThresholdMs: A,
    useIconCopyButton: I
  })).join("");
...
function yI(E, T, e) {
  const R = E.level || "INFO", A = String(R).toUpperCase(), O = wT(String(R)), L = E.message || "", I = E.source || "", o = T.badgeLevel(O), C = O === "error" ? T.rowError : "", S = e.truncateMessage ? aE(L, e.maxMessageLength || 100) : L, n = e.showSource ? `<td class="${T.timestamp}">${D(I)}</td>` : "";
  return `
    <tr class="${C}">
      <td><span class="${o}">${D(A)}</span></td>
      <td class="${T.timestamp}">${D(eE(E.timestamp))}</td>
      <td class="${T.message}" title="${D(L)}">${D(S)}</td>
      ${n}
    </tr>
  `;
}
function KE(E, T, e = {}) {
  const { newestFirst: R = !0, maxEntries: A = 100, showSortToggle: O = !1, showSource: L = !1, truncateMessage: I = !0, maxMessageLength: o = 100 } = e, C = O ? gI("logs", R, T) : "";
  if (!E.length) return C + `<div class="${T.emptyState}">No logs captured</div>`;
  let S = A ? E.slice(-A) : E;
  R && (S = [...S].reverse());
  const n = S.map((M) => yI(M, T, {
    ...e,
    showSource: L,
    truncateMessage: I,
    maxMessageLength: o
  })).join(""), i = L ? "<th>Source</th>" : "";
  return `
    ${C}
    <table class="${T.table}">
      <thead>
...
function VI(E, T, e) {
  const R = E.method || "GET", A = E.path || "", O = E.handler || "-", L = E.name || "", I = T.badgeMethod(R), o = e.showName ? `<td class="${T.timestamp}">${D(L)}</td>` : "";
  return `
    <tr>
      <td><span class="${I}">${D(R)}</span></td>
      <td class="${T.path}">${D(A)}</td>
      <td>${D(O)}</td>
      ${o}
    </tr>
  `;
...
function wE(E, T, e = {}) {
  ...
  const A = E.map((L) => VI(L, T, { showName: R })).join(""), O = R ? "<th>Name</th>" : "";
  return `
...
function x(E, T, e, R = {}) {
  const { useIconCopyButton: A = !1, filterFn: O, showCount: L = !0 } = R, I = T && typeof T == "object" && !Array.isArray(T), o = Array.isArray(T);
  let C = T ?? {};
  if (I && O && (C = O(T)), I && Object.keys(C).length === 0 || o && C.length === 0 || !I && !o && !C) return `<div class="${e.emptyState}">No ${E.toLowerCase()} data available</div>`;
  const S = RE(C), n = TE(C, !0), i = KT(C), M = o ? "items" : I ? "keys" : "entries", H = Ie(e, A, `copy-${E.toLowerCase().replace(/\s+/g, "-")}-${Date.now()}`), U = L ? `<span class="${e.muted}">${CE(i)} ${M}</span>` : "";
  return `
    <section class="${e.jsonPanel}" data-copy-content="${D(S)}">
...
function vO(E, T, e = {}) {
  ...
  const A = RE(E), O = TE(E, !0), L = Ie(T, R, `viewer-${Date.now()}`);
  return `
    <div class="${T.jsonViewer}" data-copy-content="${D(A)}">
...
function vI(E, T, e) {
  const { useIconCopyButton: R = !1, showCount: A = !0 } = e, O = RE(E), L = TE(E, !0), I = bI(T, R), o = A ? `<span class="${T.muted}">${CE(KT(E))} keys</span>` : "";
  return `
...
function XI(E, T, e) {
  ...
  const A = E.slice(-R).reverse().map((O) => WI(O, T)).join("");
  return `
    <table class="${T.table}">
      <thead>
        <tr>
...
function JE(E, T, e = {}) {
  const { dataFilterFn: R } = e, A = E.data || {}, O = R ? R(A) : A, L = E.logs || [], I = Object.keys(O).length > 0, o = L.length > 0;
  if (!I && !o) return `<div class="${T.emptyState}">No custom data captured</div>`;
  let C = "";
  return I && (C += vI(O, T, e)), o && (C += `
      <div class="${T.jsonPanel}">
        <div class="${T.jsonHeader}">
          <span class="${T.jsonViewerTitle}">Custom Logs</span>
          <span class="${T.muted}">${CE(L.length)} entries</span>
        </div>
        <div class="${T.jsonContent}">
          ${XI(L, T, e)}
        </div>
      </div>
    `), I && o ? `<div class="${T.jsonGrid}">${C}</div>` : C;
}
function xI(E) {
...
function KI(E, T, e) {
  const R = $I(E.type), A = xI(E.type), O = T.badgeLevel(A), L = E.message || "", I = E.source || "", o = !!E.stack, C = E.type === "network_error" && E.extra?.request_url ? String(E.extra.request_url) : I && E.line ? `${I}:${E.line}${E.column ? ":" + E.column : ""}` : I || "", S = o ? `<span class="${T.expandIcon}">&#9654;</span>` : "", n = o ? T.expandableRow : "", i = e.compact ? D(L.length > 100 ? L.slice(0, 100) + "..." : L) : D(L), M = !e.compact && C ? `<td class="${T.timestamp}" title="${D(C)}">${D(C.length > 60 ? "..." + C.slice(-57) : C)}</td>` : "", H = !e.compact && E.url ? `<td class="${T.timestamp}" title="${D(E.url)}">${D(E.url.length > 40 ? "..." + E.url.slice(-37) : E.url)}</td>` : "";
  let U = "";
  return o && (U = `
    ...
    <tr class="${T.rowError} ${n}">
      <td>${S}<span class="${O}">${D(R)}</span></td>
      <td class="${T.timestamp}">${D(eE(E.timestamp))}</td>
      <td class="${T.message}" title="${D(L)}">${i}</td>
      ${M}
      ${H}
    </tr>
    ${U}
...
function kE(E, T, e = {}) {
  const { newestFirst: R = !0, maxEntries: A = 100, compact: O = !1, showSortToggle: L = !1 } = e, I = L ? wI(R, T) : "";
  if (!E.length) return I + `<div class="${T.emptyState}">No JS errors captured</div>`;
  let o = A ? E.slice(-A) : E;
  R && (o = [...o].reverse());
  const C = o.map((i) => KI(i, T, {
    ...e,
    compact: O
  })).join(""), S = O ? "" : "<th>Location</th>", n = O ? "" : "<th>Page</th>";
  return `
    ${I}
    <table class="${T.table}">
      <thead>
...
function kI(E) {
  const T = ET(E.verdict), e = E.user_info || {};
  let R = "";
    ...
    },
    {
      ...
      color: T.missing_keys > 0 ? "#ef4444" : "#6b7280"
        ...
        <div style="
          ...
          min-width: 80px;
        ">
          <span style="font-size: 20px; font-weight: 700; color: ${e.color};">${e.value}</span>
          <span style="font-size: 11px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px;">${e.label}</span>
        </div>
      `).join("")}
    </div>
  `;
...
function QI(E, T) {
  const e = JI(E.status), R = (A) => A ? '<span style="color: #22c55e; font-weight: bold;">✓</span>' : '<span style="color: #ef4444; font-weight: bold;">✗</span>';
  return `
  ...
  return T.length === 0 ? "" : `
    ...
    ">
      <h3 style="
        ...
        gap: 8px;
      ">
        <span style="color: ${ET(E.verdict).color};">Next Actions</span>
      </h3>
      <ul style="
        margin: 0;
        padding: 0 0 0 20px;
        ...
        line-height: 1.6;
      ">
        ${T.map((e) => e.startsWith("  -") ? `<li style="margin-left: 20px; color: #94a3b8;">${D(e.trim().slice(2))}</li>` : `<li>${D(e)}</li>`).join("")}
      </ul>
    </div>
  `;
}
...
function bT(E, T, e = {}) {
  const { showRawJSON: R = !0, showCollapsible: A = !0 } = e;
  return E ? `
...
function EO(E, T) {
  ...
  const e = ET(E.verdict), R = E.summary || {
    module_count: 0,
    required_keys: 0,
    claims_keys: 0,
    missing_keys: 0
...
function eO(E) {
  const T = TT(E.verdict), e = TO(E.verdict);
  return `
    <div style="
      display: flex;
      align-items: center;
...
function SO(E) {
  const T = TT(E.severity), e = String(E.message || "").trim(), R = String(E.hint || "").trim(), A = String(E.code || "").trim(), O = String(E.component || "").trim();
  if (!e) return "";
  const L = [A, O].filter(Boolean).join(" • ");
  return `
    <div style="
      display: flex;
      gap: 10px;
...
function IO(E, T) {
  if (!T) return "";
  const e = String(T.description || "").trim(), R = String(T.cta || T.label || "").trim(), A = !!T.runnable, O = !!T.applicable, L = !!T.requires_confirmation, I = String(T.confirm_text || "").trim(), o = T.kind || "manual";
  let C = "enabled", S = "";
  O ? A || (C = "manual", S = o === "manual" ? "Manual action required" : "Action not available") : (C = "not-applicable", S = "Not applicable for current status");
  const n = C !== "enabled", i = n ? "background: #374151; color: #6b7280; cursor: not-allowed;" : "background: #3b82f6; color: #fff; cursor: pointer;";
  return `
    <div style="
      margin-top: 12px;
      padding: 12px;
      ...
      <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
        ${R ? `
          <button
            ...
            ${I ? `data-doctor-action-confirm="${D(I)}"` : ""}
...
function rO(E) {
  const T = TT(E.status), e = String(E.label || E.id || "").trim(), R = String(E.summary || "").trim(), A = String(E.help || E.description || "").trim(), O = E.duration_ms !== void 0 ? `${E.duration_ms}ms` : "";
  return `
    <div style="
      border: 1px solid ${T.borderColor};
...
function WT(E, T, e = {}) {
  const { showRawJSON: R = !0, problemsOnly: A = !1 } = e;
  if (!E) return `<div class="${T.emptyState}">No doctor diagnostics available</div>`;
  let O = E.checks || [];
  A && (O = O.filter((C) => C.status === "warn" || C.status === "error"));
  const L = {
    error: 0,
    warn: 1,
    info: 2,
  ...
  O = [...O].sort((C, S) => {
    const n = L[C.status || "ok"] ?? 4, i = L[S.status || "ok"] ?? 4;
    return n !== i ? n - i : (C.label || C.id || "").localeCompare(S.label || S.id || "");
  });
  const I = O.some((C) => C.status === "warn" || C.status === "error");
  let o = "";
  return O.length === 0 ? A && !I ? o = `
        <div style="
          text-align: center;
          padding: 40px 20px;
          color: #22c55e;
      ...
      ` : o = `<div class="${T.emptyState}">No doctor checks available</div>` : o = O.map((C) => rO(C)).join(""), `
...
function vT(E) {
  return E.eventTypes ? Array.isArray(E.eventTypes) ? E.eventTypes : [E.eventTypes] : [Oe(E)];
}
function nO(E) {
...
var LO = class {
  ...
  register(E) {
    this.panels.set(E.id, E), this.notifyListeners({
      type: "register",
      panelId: E.id,
      panel: E
    });
  ...
  unregister(E) {
    ...
    this.panels.delete(E) && this.notifyListeners({
      type: "unregister",
      panelId: E,
      panel: T
    });
  ...
  getAllEventTypes() {
    ...
    for (const T of this.panels.values()) for (const e of vT(T)) E.add(e);
    return Array.from(E);
  }
  findByEventType(E) {
    for (const T of this.panels.values()) if (vT(T).includes(E)) return T;
  }
  subscribe(E) {
...
var b = iO(), oO = {
  ...
  category: "system",
  order: 40,
  render: (E, T) => wE(E || [], T, { showName: !0 }),
  renderConsole: (E, T) => wE(E || [], T, { showName: !0 }),
  renderToolbar: (E, T) => wE(E || [], T, { showName: !1 }),
  getCount: (E) => (E || []).length,
  supportsToolbar: !0
}, lO = {
  id: "config",
  ...
  category: "data",
  order: 10,
  render: (E, T, e) => x("Template Context", E, T, {
    useIconCopyButton: !0,
    showCount: !0
  }),
  renderConsole: (E, T, e) => {
  ...
  handleEvent: (E, T) => {
    ...
    return {
      ...
      logs: R.logs ? [...e.logs || [], ...R.logs].slice(-500) : e.logs
    };
  },
  supportsToolbar: !0
}, cO = {
  ...
  order: 45,
  showFilters: !1,
  render: (E, T, e) => bT(E, T, { showRawJSON: !0 }),
  renderConsole: (E, T, e) => bT(E, T, { showRawJSON: !0 }),
  renderToolbar: (E, T, e) => EO(E, T),
  getCount: (E) => {
    const T = E;
  ...
  order: 46,
  showFilters: !1,
  render: (E, T, e) => WT(E, T, { showRawJSON: !0 }),
  renderConsole: (E, T, e) => WT(E, T, { showRawJSON: !0 }),
  getCount: (E) => {
    const T = E;
...
function GO() {
  b.register(oO), b.register(_O), b.register(DO), b.register(cO), b.register(PO), b.register(dO), b.register(pO), b.register(lO), b.register(MO), b.register(UO), b.register(uO);
}
GO();
export {
  D as A,
...
//# sourceMappingURL=builtin-panels-Dm8EsEAm.js.map