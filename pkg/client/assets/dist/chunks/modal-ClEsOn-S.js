...
function m(e) {
  return Array.from(e.querySelectorAll(_)).filter((t) => t.tabIndex >= 0 && u(t));
}
function v(e, t) {
  if (!t) return null;
...
var C = class {
  ...
  register(e) {
    const t = e.zIndexTarget ?? e.container, i = {
      ...
      zIndex: y + ++this.nextLayerIndex * x,
      closing: !1,
      released: !1,
  ...
  focusInitial(e, t) {
    const i = t === void 0 ? e.initialFocus : t, r = v(e.container, i);
    if (r && l(r, !0)) return;
    const s = m(e.container)[0];
    s && l(s, !0) || (e.container.hasAttribute("tabindex") || (e.container.setAttribute("tabindex", "-1"), e.addedFallbackTabIndex = !0), l(e.container));
  }
  trapFocus(e, t) {
    ...
    const r = this.ownerDocument.activeElement, s = i.indexOf(r);
    if (s === -1) {
      t.preventDefault(), l(t.shiftKey ? i[i.length - 1] : i[0]);
      return;
    }
    t.shiftKey && s === 0 ? (t.preventDefault(), l(i[i.length - 1])) : !t.shiftKey && s === i.length - 1 && (t.preventDefault(), l(i[0]));
  }
  lockBody(e) {
  ...
  release(e, t) {
    ...
    const s = this.topmost();
    e.returnFocus && u(e.returnFocus) && (!s || s.container.contains(e.returnFocus)) && l(e.returnFocus) || s && this.focusInitial(s);
  }
}, f = /* @__PURE__ */ new WeakMap();
function w(e) {
  const t = e.container.ownerDocument;
  let i = f.get(t);
  return i || (i = new C(t), f.set(t, i)), i.register(e);
}
var A = g("Modal"), c = Object.freeze({
  root: "go-admin-modal",
  ...
  async show() {
    ...
    try {
      ...
      const r = this._options.size ?? "lg", s = this._options.flexColumn ? "go-admin-modal__container--flex flex flex-col" : "", d = this._options.containerClass, n = document.createElement("div");
      this.container = n, n.className = [
        ...
        s,
        d
      ].filter(Boolean).join(" "), n.setAttribute("data-go-admin-modal", "true"), n.setAttribute("data-size", r), n.setAttribute("role", "dialog"), n.setAttribute("aria-modal", "true"), n.setAttribute("data-maximized", String(this._isMaximized)), this._options.maximizable && n.setAttribute("data-maximizable", "true"), this._layer = w({
        container: n,
        zIndexTarget: i,
      ...
      }), n.innerHTML = this.renderContent(), this._applyAccessibleName(), i.appendChild(n), document.body.appendChild(i), this._mounted = !0, (typeof requestAnimationFrame == "function" ? requestAnimationFrame : (o) => (o(0), 0))(() => {
  ...
  _applyAccessibleName() {
    if (this.container) {
      if (this.container.removeAttribute("aria-label"), this.container.removeAttribute("aria-labelledby"), this.container.removeAttribute("aria-describedby"), this._options.labelledBy) this.container.setAttribute("aria-labelledby", this._options.labelledBy);
        ...
        e ? (e.id || (e.id = `go-admin-modal-title-${++L}`), this.container.setAttribute("aria-labelledby", e.id)) : (this.container.setAttribute("aria-label", "Dialog"), A.warn("Modal opened without labelledBy, ariaLabel, or heading; using fallback accessible name."));
      }
...
}, T = "w-full border rounded-lg bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:border-gray-600 dark:bg-slate-800 dark:text-white dark:placeholder-gray-500 px-3 py-2 text-sm border-gray-300", E = 0, D = class extends p {
  constructor(e) {
    super({
      size: "sm",
  ...
  renderContent() {
    const e = this.config.inputClass ?? T;
    return `
  ...
  bindContentEvents() {
    const e = this.container?.querySelector("[data-prompt-input]"), t = this.container?.querySelector("[data-prompt-error]"), i = this.container?.querySelector("[data-prompt-confirm]"), r = this.container?.querySelector("[data-prompt-cancel]"), s = (n) => {
    ...
    const d = async () => {
      ...
      const o = await this.config.onConfirm(n), h = o === !1 ? "Value is invalid." : typeof o == "string" ? o : o && typeof o == "object" && typeof o.error == "string" ? o.error : "";
      if (h) {
        s(h), e?.focus();
        return;
      }
    ...
    i?.addEventListener("click", () => {
      ...
      n.key === "Enter" && (n.preventDefault(), d());
...
//# sourceMappingURL=modal-ClEsOn-S.js.map