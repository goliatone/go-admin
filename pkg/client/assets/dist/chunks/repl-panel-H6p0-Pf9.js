...
import { r as ne } from "./icons-zihLPh2q.js";
      ...
      3236: (O, r, a) => {
        ...
        class $ extends b.CoreTerminal {
            ...
            if (this._themeService) for (const D of C) {
              ...
              switch (D.type) {
                case 0:
                  ...
                  this.coreService.triggerDataEvent(`${M.C0.ESC}]${B};${(0, y.toRgbString)(j)}${M.C1_ESCAPED.ST}`);
                  break;
                case 1:
          ...
          _syncTextArea() {
            ...
            if (!D) return;
            const T = Math.min(this.buffer.x, this.cols - 1), B = this._renderService.dimensions.css.cell.height, j = D.getWidth(T), W = this._renderService.dimensions.css.cell.width * j, G = this.buffer.y * this._renderService.dimensions.css.cell.height, Y = T * this._renderService.dimensions.css.cell.width;
          ...
          bindMouse() {
            ...
            this.register(this.coreMouseService.onProtocolChange(((W) => {
              W ? (this.optionsService.rawOptions.logLevel === "debug" && this._logService.debug("Binding to mouse events:", this.coreMouseService.explainEvents(W)), this.element.classList.add("enable-mouse-events"), this._selectionService.disable()) : (this._logService.debug("Unbinding from mouse events."), this.element.classList.remove("enable-mouse-events"), this._selectionService.enable()), 8 & W ? B.mousemove || (D.addEventListener("mousemove", j.mousemove), B.mousemove = j.mousemove) : (D.removeEventListener("mousemove", B.mousemove), B.mousemove = null), 16 & W ? B.wheel || (D.addEventListener("wheel", j.wheel, { passive: !1 }), B.wheel = j.wheel) : (D.removeEventListener("wheel", B.wheel), B.wheel = null), 2 & W ? B.mouseup || (D.addEventListener("mouseup", j.mouseup), B.mouseup = j.mouseup) : (this._document.removeEventListener("mouseup", B.mouseup), D.removeEventListener("mouseup", B.mouseup), B.mouseup = null), 4 & W ? B.mousedrag || (B.mousedrag = j.mousedrag) : (this._document.removeEventListener("mousemove", B.mousedrag), B.mousedrag = null);
          ...
          _reportWindowsOptions(C) {
            if (this._renderService) switch (C) {
              ...
              case I.WindowsOptionsReportType.GET_CELL_SIZE_PIXELS:
                const B = this._renderService.dimensions.css.cell.width.toFixed(0), j = this._renderService.dimensions.css.cell.height.toFixed(0);
                this.coreService.triggerDataEvent(`${M.C0.ESC}[6;${j};${B}t`);
            }
          }
          cancel(C, D) {
        ...
        let o = r.DomRendererRowFactory = class {
          ...
          createRow(v, d, h, m, E, x, b, k, R, H, P) {
            ...
            for (let G = 0; G < L; G++) {
              ...
              const oe = this._isCellInSelection(G, d), de = h && G === x, _e = W && G >= H && G <= P;
              let ue = !1;
              ...
              K.isOverline() && (j.push("xterm-overline"), F === " " && (F = " ")), K.isStrikethrough() && j.push("xterm-strikethrough"), _e && (w.style.textDecoration = "underline");
              let ee = K.getFgColor(), ae = K.getFgColorMode(), te = K.getBgColor(), he = K.getBgColorMode();
              const fe = !!K.isInverse();
              if (fe) {
                const J = ee;
                ee = te, te = J;
                const Se = ae;
              ...
              switch (this._decorationService.forEachDecorationAtCell(G, d, void 0, ((J) => {
                J.options.layer !== "top" && le || (J.backgroundColorRGB && (he = 50331648, te = J.backgroundColorRGB.rgba >> 8 & 16777215, ie = J.backgroundColorRGB), J.foregroundColorRGB && (ae = 50331648, ee = J.foregroundColorRGB.rgba >> 8 & 16777215, ve = J.foregroundColorRGB), le = J.options.layer === "top");
              })), !le && oe && (ie = this._coreBrowserService.isFocused ? y.selectionBackgroundOpaque : y.selectionInactiveBackgroundOpaque, te = ie.rgba >> 8 & 16777215, he = 50331648, le = !0, y.selectionForeground && (ae = 50331648, ee = y.selectionForeground.rgba >> 8 & 16777215, ve = y.selectionForeground)), le && j.push("xterm-decoration-top"), he) {
                case 16777216:
                case 33554432:
                  se = y.ansi[te], j.push(`xterm-bg-${te}`);
                  break;
                case 50331648:
                  se = _.rgba.toColor(te >> 16, te >> 8 & 255, 255 & te), this._addStyle(w, `background-color:#${l((te >>> 0).toString(16), "0", 6)}`);
                  break;
                default:
                  fe ? (se = y.foreground, j.push(`xterm-bg-${n.INVERTED_DEFAULT_COLOR}`)) : se = y.background;
              }
              switch (ie || K.isDim() && (ie = _.color.multiplyOpacity(se, 0.5)), ae) {
                case 16777216:
                case 33554432:
                  K.isBold() && ee < 8 && this._optionsService.rawOptions.drawBoldTextInBrightColors && (ee += 8), this._applyMinimumContrast(w, se, y.ansi[ee], K, ie, void 0) || j.push(`xterm-fg-${ee}`);
                  break;
                case 50331648:
                  const J = _.rgba.toColor(ee >> 16 & 255, ee >> 8 & 255, 255 & ee);
                  this._applyMinimumContrast(w, se, J, K, ie, ve) || this._addStyle(w, `color:#${l(ee.toString(16), "0", 6)}`);
                  break;
                default:
                  this._applyMinimumContrast(w, se, y.foreground, K, ie, void 0) || fe && j.push(`xterm-fg-${n.INVERTED_DEFAULT_COLOR}`);
              }
              j.length && (w.className = j.join(" "), j.length = 0), de || Q || ue ? w.textContent = F : I++, B !== this.defaultSpacing && (w.style.letterSpacing = `${B}px`), M.push(w), G = re;
            }
        ...
        let e = r.CharacterJoinerService = class me {
          ...
          _getJoinedRanges(t, i, o, l, v) {
            ...
            for (let m = 1; m < this._characterJoiners.length; m++) try {
              ...
              for (let x = 0; x < E.length; x++) me._mergeRanges(h, E[x]);
            } catch (E) {
              console.error(E);
            }
          ...
          print(S, y, w) {
            ...
            for (let B = y; B < w; ++B) {
              if (L = S[B], I = this._unicodeService.wcwidth(L), L < 127 && F) {
                const j = F[String.fromCharCode(L)];
                j && (L = j.charCodeAt(0));
              }
    ...
    this.root.innerHTML = `
      <div class="debug-repl__header">
        ...
        <div class="debug-repl__actions">
          <button class="debug-btn" data-repl-action="reconnect">${ne("refresh", { size: "14px" })} Reconnect</button>
          <button class="debug-btn" data-repl-action="clear">${ne("clear", { size: "14px" })} Clear</button>
          <button class="debug-btn debug-btn--danger" data-repl-action="kill">${ne("delete", { size: "14px" })} Kill</button>
        </div>
      </div>
...
//# sourceMappingURL=repl-panel-H6p0-Pf9.js.map