  ...
  renderResults() {
    ...
    this.dropdown.innerHTML = s.join(""), this.dropdown.querySelectorAll(".searchbox-item").forEach((r) => {
      r.addEventListener("click", (i) => {
        i.stopPropagation();
        const a = parseInt(r.dataset.index || "0", 10), o = e[a];
...
var f = {
  ...
  renderMetadataPills(e) {
    ...
    const t = this.config.metadataFields.map((s) => {
      ...
      if (!r) return null;
      const i = s.replace(/([A-Z])/g, " $1").replace(/^./, (a) => a.toUpperCase());
      return `<span class="text-xs text-gray-400">${n(i)}: <span class="text-gray-600">${n(r)}</span></span>`;
    }).filter(Boolean);
...
//# sourceMappingURL=entity-renderer-DjI18qqO.js.map