import { o as k } from "./lineage-contracts-Clh6Zaep.js";
import { c as re } from "./dom-helpers-CDdChTSn.js";
import { c as ne, d as ie, g as O, h as ae, l as oe, n as H, p as le, r as ce, s as A, u as de, y as ue } from "./source-management-pages-Cz6zy9go.js";
function N(e) {
...
function me(e) {
  ...
  const r = Date.now() - n.getTime(), s = Math.floor(r / 6e4), a = Math.floor(r / 36e5), o = Math.floor(r / 864e5);
  return s < 1 ? "just now" : s < 60 ? `${s}m ago` : a < 24 ? `${a}h ago` : o < 7 ? `${o}d ago` : n.toLocaleDateString();
}
function pe(e) {
  switch (e) {
    case "active":
...
function fe(e) {
  ...
  ].map((t) => C(String(t ?? ""))).filter((t) => t.length > 0) : [];
}
function he(e, t) {
  const n = C(e).split("/").filter(Boolean), r = C(t).split("/").filter(Boolean);
  return n.length !== r.length ? !1 : r.every((s, a) => s.startsWith(":") || s === n[a]);
}
function y(e, t) {
  const n = C(String(e ?? ""));
  return n ? fe(t).some((r) => he(n, r)) : !1;
}
function P(e, t, n) {
...
var He = [k.SOURCE_DOCUMENT, k.SOURCE_REVISION];
function xe(e, t) {
  const n = S(e.links, t, "workspace", "anchor", "source", "self");
  if (n) return n;
  const r = _(t.routes?.source_workspace ?? t.routes?.source_detail, e.source?.id ?? "");
  return y(r, t.routes) ? r : "";
}
function be(e, t) {
  const n = I(e.drill_in?.href, t) || S(e.links, t, "anchor", "workspace", "comments", "artifacts", "source", "self");
  if (n) return n;
...
function ye(e, t, n) {
  ...
  for (const d of c) {
    const m = I(String(d?.href ?? "").trim(), n);
    if (m) return m;
  }
  return "";
}
...
var J = "bg-white border border-gray-200 rounded-xl mb-4 p-4 shadow-sm", w = "h-10 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1 transition-colors", $e = "h-10 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-blue-500 bg-blue-50 text-blue-600 shadow-sm hover:bg-blue-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1 transition-colors", D = "h-10 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-blue-600 bg-blue-600 text-white shadow-sm hover:bg-blue-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1 transition-colors", Y = "block w-full h-10 ps-9 pe-8 border border-gray-200 rounded-lg text-sm placeholder-gray-500 focus:outline-none focus:ring-0 focus:border-gray-200", B = "block w-full h-10 rounded-lg border border-gray-200 bg-white py-2 px-3 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500", X = "inline-flex items-center gap-2 h-10 rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-700 cursor-pointer hover:bg-gray-50", j = "bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden", g = "px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider", f = "px-6 py-4 align-top", _e = "rounded-lg border border-gray-200 bg-gray-50 p-4", we = "rounded-xl border border-gray-200 bg-white p-6";
...
function z(e, t = "") {
  return `<div class="${we}${t ? ` ${t}` : ""}">${e}</div>`;
}
function Z(e, t, n) {
  return `
    <button type="button" data-runtime-action="${i(e)}" class="${n ? $e : w}">
...
function q(e, t, n) {
  ...
  return `
    ...
    ${F(e.page_info, "source-browser-page")}
  `;
}
function L(e, t, n, r) {
  ...
  return `
    <div class="rounded-xl border border-gray-200 bg-white p-3">
      <div class="flex flex-wrap gap-2">
        ${n.map((s) => {
    const a = S(s.links, t, "anchor", "workspace", "self"), o = s.id === r, c = s.item_count ?? 0, d = o ? "border-blue-600 bg-blue-600 text-white" : "border-gray-300 bg-white text-gray-700 hover:border-gray-400 hover:bg-gray-50";
    return a ? `
              <a
                href="${i(a)}"
...
function V(e, t, n) {
  ...
  const r = String(e.active_panel ?? "overview").trim(), s = String(e.active_anchor ?? "").trim(), a = e.continuity, o = [...a.predecessors ?? [], ...a.successors ?? []], c = a.summary ? `<p class="text-sm text-gray-700">${i(a.summary)}</p>` : '<p class="text-sm text-gray-500">No continuity summary available.</p>', d = (e.timeline?.entries ?? []).length > 0 ? `<div class="space-y-3">
    ...
    const h = I(l.drill_in?.href, n) || S(l.links, n, "anchor", "timeline", "workspace", "source", "self");
    return `
                ${b(`
    ...
    const h = S(l.links, n, "anchor", "workspace", "agreement", "self");
    return `
                ${b(`
                  <div class="flex items-start justify-between gap-4">
                    <div>
    ...
    const h = I(l.drill_in?.href, n) || S(l.links, n, "anchor", "workspace", "artifacts", "self");
    return `
                ${b(`
                  <div class="flex items-start justify-between gap-4">
                    <div>
        ...
        </div>` : v(e.artifacts?.empty_state?.title ?? "No artifacts", e.artifacts?.empty_state?.description ?? "No artifacts are available in this workspace."), $ = (e.comments?.items ?? []).length > 0 ? `<div class="space-y-3">${e.comments.items.map(ee).join("")}</div>` : v(e.comments?.empty_state?.title ?? "No comments", e.comments?.empty_state?.description ?? "No comment threads are available in this workspace."), se = (e.handles?.items ?? []).length > 0 ? `<div class="grid gap-3">
          ${e.handles.items.map((l) => {
    const h = S(l.links, n, "workspace", "source", "self");
    return `
                ${b(`
                  <div class="flex items-start justify-between gap-4">
                    <div>
  ...
  return `
    <div class="p-6 space-y-6">
      ...
      ${L("timeline", "Revision Timeline", r, d)}
      ${L("agreements", "Related Agreements", r, m)}
      ${L("artifacts", "Related Artifacts", r, u)}
      ${L("comments", "Related Comments", r, `${e.comments?.sync_status ? `<div class="mb-3">${p(e.comments.sync_status)}</div>` : ""}${$}`)}
      ${L("handles", "Active Handles", r, se)}
    </div>
  `;
}
function W(e) {
...
function G(e) {
  ...
  return t.length === 0 ? v(e.empty_state?.title ?? "No comments", e.empty_state?.description ?? "No comments have been synced for this revision.", !0) : `
    <div class="p-6 space-y-4">
      ...
      ${F(e.page_info, "source-comment-page")}
    </div>
  `;
}
function Ce(e) {
...
function K(e, t, n, r) {
  ...
  return `
    ${s}
    ${Re(a, t, n, r)}
    ${F(e.page_info, "source-search-page")}
  `;
}
function F(e, t) {
...
var Me = class {
  ...
  bindEvents() {
    this.root.addEventListener("click", (e) => {
      ...
      if (r && this.liveController instanceof ne) {
        ...
        s && C(window.location.href) === C(s) && (e.preventDefault(), this.liveController.navigateToHref(s));
      }
  ...
  async goToPage(e, t) {
    e === "source-browser-page" && this.liveController instanceof H && await this.liveController.goToPage(t), e === "source-comment-page" && this.liveController instanceof ce && await this.liveController.goToPage(t), e === "source-search-page" && this.liveController instanceof A && await this.liveController.goToPage(t);
  }
...
function te() {
  ...
  if (!e || !t) return null;
  Le(document);
  const n = N("esign-page-config"), r = N("source-management-page-model"), s = String(n?.page ?? e.dataset.esignPage ?? "").trim();
  if (!s) return null;
  const a = new Me({
    page: s,
    config: n ?? {},
...
function Pe() {
  ...
  const r = N("esign-page-config"), s = N("source-management-page-model");
...
//# sourceMappingURL=source-management-runtime-C_0n2WmU.js.map