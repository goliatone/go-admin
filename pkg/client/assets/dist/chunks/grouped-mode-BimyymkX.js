import { createLogger as J } from "../shared/logger.js";
import { escapeAttribute as W, escapeHTML as f } from "../shared/html.js";
import { a as K, n as Q, o as j } from "./status-vocabulary-BYdivV6D.js";
function v(e) {
...
function se(e) {
  ...
  const t = e;
  if (!te(t)) return null;
  const r = L({ reason_code: t.reason_code }), s = { enabled: typeof t.enabled == "boolean" ? t.enabled : !1 }, n = m(t.reason), a = m(t.severity), o = m(t.kind), i = m(t.permission), d = t.metadata && typeof t.metadata == "object" && !Array.isArray(t.metadata) ? t.metadata : null, l = Z(t.remediation), u = ee(t.available_transitions);
...
function ne(e) {
  if (!y(e)) return null;
  const t = e.selection_sensitive === !0, r = m(e.selection_state_endpoint), s = re(e.debounce_ms);
...
function Me(e) {
  if (!y(e)) return null;
  const t = Array.isArray(e.data) ? e.data : Array.isArray(e.records) ? e.records : null, r = t && t.map((a) => O(a) ?? a), s = ae(e.$meta), n = { ...e };
...
function $e(e, t) {
  const r = m(t);
  return r && $(e._action_state)[r] || null;
}
var ie = J("DataGrid"), g = {
...
function c(e, t = {}) {
  const r = Q(e), s = r?.tone ?? "neutral";
  return {
    label: r?.label ?? j(e),
    shortLabel: t.shortLabel,
    colorClass: K(s, "badge"),
    bgClass: oe[s],
    textClass: le[s],
    borderClass: ce[s],
...
function de(e, t = {}) {
  ...
  const { size: s = "default", showIcon: n = !0, showLabel: a = !0, extraClass: o = "" } = t, i = {
    ...
    sm: "px-2 py-0.5",
    default: ""
  }, d = n ? ue(r, s) : "", l = a ? `<span>${f(r.label)}</span>` : "";
...
function Ge(e = {}) {
  return (t) => typeof t != "string" || !t ? '<span class="text-gray-400">-</span>' : de(t, e);
}
function Pe(e = {}) {
  return (t) => typeof t != "string" || !t ? "" : fe(t, e);
}
function Ve(e) {
  e.schema_version !== 1 && ie.warn("[TranslationStatusVocabulary] Unknown schema version:", e.schema_version);
}
function Ue() {
  return `
...
function Fe(e, t = {}) {
  ...
  for (const d of e) {
    ...
    const l = ge(d);
    if (!l) return null;
    const u = V(d), p = be(d, u), b = D(l, s, n, r);
    a.push({
      groupId: l,
      displayLabel: he(d, u),
      records: u,
...
var q = "datagrid-view-mode-", R = 200, xe = 256;
...
function ot(e, t, r = {}) {
  const { showExpandIcon: s = !0, fixedColumnCount: n = 2 } = r, a = s ? `<span class="expand-icon mr-2" aria-hidden="true">${e.expanded ? "▼" : "▶"}</span>` : "", o = Ae(e), i = f(Se(e)), d = e.records.length, l = d > 1 ? `<span class="ml-2 text-xs text-gray-500">(${d} locales)</span>` : "";
  return `
...
//# sourceMappingURL=grouped-mode-BimyymkX.js.map