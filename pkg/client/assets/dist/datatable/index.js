...
import { extractExchangeError as wl, generateExchangeReport as xl, groupRowResultsByStatus as Sl, isExchangeError as Cl, parseImportResult as $l } from "../toast/error-helpers.js";
...
import { h as M, r as lr } from "../chunks/http-client-Dm229xuF.js";
...
function qa(r, e = {}) {
  ...
  const n = mt(r), s = n && n > 0 ? B(`${n} ${n === 1 ? "locale" : "locales"}`, e, "info") : "";
  return `
...
function vr(r, e = {}) {
  ...
  const d = new Set(i), u = wr(l), p = c.slice(0, n).map((f) => {
    ...
    let C, D, E;
    g ? (C = "bg-green-100 text-green-700 border-green-300", D = "●", E = "Complete") : v ? (C = "bg-amber-100 text-amber-700 border-amber-300", D = "◐", E = "Incomplete") : (C = "bg-white text-gray-400 border-gray-300 border-dashed", D = "○", E = "Missing");
    const x = t === "sm" ? "text-[10px] px-1.5 py-0.5" : "text-xs px-2 py-1", K = s ? `<span class="font-medium">${f.toUpperCase()}</span>` : "";
    return `
        <span class="inline-flex items-center gap-0.5 ${x} rounded border ${C}"
...
const I = '<span class="text-gray-400">-</span>', Cr = [
  "name",
  "label",
  "title",
  "slug",
...
function Ce(r, e) {
  ...
  if (typeof r != "object")
    return String(r);
  const n = [...kr(e), ...Cr], s = /* @__PURE__ */ new Set();
  for (const o of n) {
    if (s.has(o)) continue;
    s.add(o);
  ...
  registerDefaultRenderers() {
    this.renderers.set("_badge", (e) => {
      ...
      for (const f of u) {
        const y = Ir(f);
        if (!y) continue;
        const v = c[y] || "view-grid", g = nr(v, { size: "14px", extraClass: "flex-shrink-0" }), C = Br(l);
        d.push(
          `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${C}">${g}<span>${J(y)}</span></span>`
        );
      }
      if (d.length === 0)
        return I;
...
function Ur(r, e = {}) {
  ...
  for (const l of r) {
    ...
    if (!c)
      return null;
    const d = Ct(l), u = Yr(l, d), p = wt(c, n, s, t);
    o.push({
      groupId: c,
      displayLabel: Qr(l, d),
      records: d,
...
const A = class A {
  constructor(e) {
    ...
    ), C = this.config.enableGroupedMode ? n?.expandMode !== void 0 || g.size > 0 || p : !1, E = (this.config.enableGroupedMode ? n?.viewMode || h : null) || this.config.defaultViewMode || "flat";
    this.state = {
      currentPage: 1,
  ...
  deriveCreateTranslationLocaleOptions(e, t, n, s, o) {
    ...
    for (const v of l) {
      ...
      if (!g || f.has(g))
        continue;
      f.add(g);
      const C = d?.toLowerCase() === g, D = u.includes(g), E = [];
  ...
  if (e) {
    const o = e.disabled === !0, a = Le(e, 0), i = Ze(e), l = o ? (e.disabledReason || "Action unavailable").trim() : "", c = l ? `detail-action-reason-${a}` : "", d = c ? `aria-describedby="${c}"` : "", u = l ? `${e.label} unavailable: ${l}` : e.label, p = o && e.remediation?.href && e.remediation?.label ? `
        ...
        ` : "", h = l ? `title="${w(l)}"` : "", f = o && l ? `<span
           ...
           title="${w(l)}"
           data-detail-action-reason="${w(a)}"
           id="detail-action-reason-${w(a)}"
         >?</span>` : "";
    n = `
  ...
  return t.length > 0 && (s = `
    ...
    const l = a.disabled === !0, c = Le(a, e ? i + 1 : i), d = Ze(a), u = l ? (a.disabledReason || "Action unavailable").trim() : "", p = u ? `detail-action-reason-${c}` : "", h = p ? `aria-describedby="${p}"` : "", f = u ? `${a.label} unavailable: ${u}` : a.label, y = a.variant === "danger" && i > 0 ? '<div class="my-1 border-t border-gray-100"></div>' : "", v = u ? `title="${w(u)}"` : "", g = l && a.remediation?.href && a.remediation?.label ? `
          ...
          ` : "", C = l && u ? `<span
             class="inline-flex items-center justify-center w-4 h-4 rounded-full bg-amber-100 text-amber-600 text-xs cursor-help"
             title="${w(u)}"
             data-detail-action-reason="${w(c)}"
             id="detail-action-reason-${w(c)}"
           >?</span>` : "";
    return `
        ${y}
...
function $i(r) {
  ...
  return e.forEach((n) => {
    if (n.hasAttribute("data-quick-filters-init"))
      return;
    const s = n.dataset.size || "default", o = Co(
      n,
      (a) => r(a, n),
      { size: s }
    );
...
function ki(r = {}) {
  ...
  const c = e.map((d) => {
    ...
    const g = d.icon ? `<span>${d.icon}</span>` : "", C = p ? "" : `title="${_e(f)}"`;
    return `<span class="${y} ${v}" ${C}>${g}<span>${Oe(d.label)}</span></span>`;
  }).join("");
...
class Xt {
  ...
  render() {
    ...
    const e = this.config.labels;
    this.container.innerHTML = `
      <div class="async-progress" role="region" aria-label="${$(e.title)}">
        ${this.renderHeader()}
        ${this.renderContent()}
        ${this.renderFooter()}
      </div>
    ...
    if (!this.job)
      return `
        <div class="progress-header idle">
          <h4 class="progress-title">${$(e.title)}</h4>
          <span class="progress-status">${$(e.noActiveJob)}</span>
        </div>
      `;
    const t = ut(this.job.status, "exchange"), n = this.getStatusLabel(), s = this.pollingState === "paused" ? `<span class="progress-status ${t}">${$(n)}</span>` : V(this.job.status, { domain: "exchange", size: "sm" });
    return `
    ...
    return `
      <div class="progress-info">
        <span class="info-item">
          <span class="info-label">${$(e.jobId)}:</span>
          <code class="info-value">${$(this.job.id)}</code>
        </span>
        ${t ? `
          <span class="info-item">
            <span class="info-label">${$(e.elapsed)}:</span>
    ...
    return `
      <div class="progress-conflicts">
        <span class="conflicts-header">
          <span class="conflicts-label">${$(e.conflicts)}:</span>
  ...
  renderFooter() {
    ...
    return this.pollingState === "paused" && t.push(`<button type="button" class="resume-btn">${$(e.resume)}</button>`), this.pollingState === "polling" && t.push(`<button type="button" class="cancel-btn">${$(e.cancel)}</button>`), (this.error || this.job?.status === "failed") && t.push(`<button type="button" class="retry-btn">${$(e.retry)}</button>`), (this.job?.status === "completed" || this.job?.status === "failed") && t.push(`<button type="button" class="dismiss-btn">${$(e.dismiss)}</button>`), t.length === 0 ? "" : `
...
export {
  ...
  $a as SideBySideEditor,
  qt as StatusLegend,
  qe as TranslationBlockerModal,
  $o as TranslationPanel,
  ...
  Co as createTranslationQuickFilters,
