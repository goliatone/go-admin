import { n as c, t as _ } from "../chunks/html-Cx1oHGAm.js";
import { extractStructuredError as G } from "../toast/error-helpers.js";
import { t as te } from "../chunks/http-client-D9Z2A1Pg.js";
import { $ as I, C as se, E as re, H as ie, S as ne, U as oe, _ as le, d as S, g as ce, h as de, l as H, m as ue, o as me, p as fe, rt as ye, s as P, v as ge } from "../chunks/translation-shared-BSLmw_rJ.js";
function r(e) {
...
function xe(e, a, t = "") {
  const s = encodeURIComponent(r(a)), i = `${x(e)}/translations/families/${s}`, n = new URLSearchParams();
...
function ve(e, a, t = "") {
  const s = encodeURIComponent(r(a)), i = `${x(e)}/translations/families/${s}/variants`, n = new URLSearchParams();
  r(t) && n.set("channel", r(t));
  const l = n.toString();
  return l ? `${i}?${l}` : i;
}
function Ce(e = {}) {
  const a = Q(e), t = { locale: a.locale };
...
function $e(e) {
  const a = u(e.data), t = u(e.meta), s = u(t.family), i = u(t.refresh), n = u(a.navigation), l = E(u(s.quick_create), { missingLocales: f(s.missing_locales) });
  return {
...
function qe(e) {
  ...
  return {
    items: (Array.isArray(n) ? n : []).map((l) => ke(u(l))),
    total: p(i.total),
    page: p(i.page, 1),
...
function X(e) {
  const a = u(e.data), t = Object.keys(a).length ? a : e, s = t.source_variant ? O(u(t.source_variant)) : null, i = Array.isArray(t.blockers) ? t.blockers.map((m) => Ae(u(m))) : [], n = Array.isArray(t.locale_variants) ? t.locale_variants.map((m) => O(u(m))) : [], l = Array.isArray(t.active_assignments) ? t.active_assignments.map((m) => Ie(u(m))) : [], d = u(t.publish_gate), o = u(t.readiness_summary), y = E(u(t.quick_create), {
  ...
  return {
    ...
    publishGate: {
      allowed: b(d.allowed),
      overrideAllowed: b(d.override_allowed),
      blockedBy: f(d.blocked_by),
      reviewRequired: b(d.review_required)
    },
    readinessSummary: {
      state: k(o.state),
...
function la(e, a) {
  ...
  const t = r(a.locale).toLowerCase(), s = e.localeVariants.some((o) => o.locale === t) ? e.localeVariants.map((o) => o.locale === t ? {
    ...
    updatedAt: "",
    publishedAt: ""
  }].sort((o, y) => o.locale.localeCompare(y.locale));
  ...
  if (a.assignment) {
    const o = {
      ...
      createdAt: "",
      updatedAt: ""
    }, y = i.findIndex((m) => m.id === o.id || m.targetLocale === o.targetLocale);
    y >= 0 ? i[y] = o : i = [...i, o].sort((m, v) => m.targetLocale.localeCompare(v.targetLocale));
  }
  const n = e.blockers.map((o) => ({ ...o })).filter((o) => !(o.blockerCode === "missing_locale" && o.locale === t)), l = A(e.readinessSummary.availableLocales, a.family.availableLocales, [t]), d = J(A(e.readinessSummary.missingLocales, a.family.missingLocales), t);
  return {
    ...e,
...
function ca(e, a) {
  const t = { ...e }, s = { ...u(t.translation_readiness) }, i = r(a.locale).toLowerCase(), n = r(t.requested_locale).toLowerCase(), l = r(t.translation_family_id || t.family_id || s.family_id || s.family_id);
  if (l && l !== a.familyId) return t;
  const d = A(f(t.available_locales), f(s.available_locales), a.family.availableLocales, [i]), o = J(A(f(t.missing_required_locales), f(s.missing_required_locales), a.family.missingLocales), i);
  return t.available_locales = d, t.missing_required_locales = o, t.translation_family_id = l || a.familyId, s.family_id = l || a.familyId, s.state = a.family.readinessState, s.available_locales = d, s.missing_required_locales = o, s.blocker_codes = [...a.family.blockerCodes], s.missing_required_locale_count = a.family.missingRequiredLocaleCount, s.pending_review_count = a.family.pendingReviewCount, s.outdated_locale_count = a.family.outdatedLocaleCount, s.missing_locales = [...a.family.quickCreate.missingLocales], s.recommended_locale = a.family.quickCreate.recommendedLocale, s.required_for_publish = [...a.family.quickCreate.requiredForPublish], s.default_assignment = {
...
function Be(e, a, t) {
  const s = x(e), i = r(t.sourceRecordId);
...
function Ge(e, a) {
  const t = x(a.contentBasePath || `${x(a.basePath || "/admin")}/content`), s = e.readinessSummary.missingLocales, i = e.quickCreate.disabledReason || "Locale creation is unavailable for this family.", n = (d) => {
    ...
    return `
      <button
        ...
        title="${_(o ? i : `Create ${d.toUpperCase()} locale`)}"
      >
        Create locale
      </button>
    `;
  }, l = e.localeVariants.map((d) => {
    const o = Be(t, e, d), y = o ? `<a href="${_(o)}" class="text-sm font-medium text-sky-700 hover:text-sky-800">Open locale</a>` : '<span class="text-sm text-gray-400">No content route</span>', m = d.fields.title || d.fields.slug || `${e.contentType} ${d.locale.toUpperCase()}`;
    return `
      <li class="flex items-start justify-between gap-4 rounded-xl border border-gray-200 bg-white p-6">
        <div class="min-w-0">
          <div class="flex flex-wrap items-center gap-2">
            <span class="text-sm font-semibold text-gray-900">${c(d.locale.toUpperCase())}</span>
            ${d.isSource ? '<span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-700">Source</span>' : ""}
            <span class="rounded-full px-2 py-0.5 text-xs font-medium ${Ee(d.status)}">${c(h(d.status))}</span>
          </div>
          <p class="mt-2 text-sm text-gray-600">${c(m)}</p>
          <p class="mt-1 text-xs text-gray-500">Updated ${c(D(d.updatedAt || d.createdAt)) || "n/a"}</p>
        </div>
        <div class="flex-shrink-0">${y}</div>
      </li>
    `;
  });
  for (const d of s) l.push(`
      <li class="flex items-start justify-between gap-4 rounded-xl border border-rose-200 bg-rose-50 p-6">
        <div>
          <div class="flex items-center gap-2">
            <span class="text-sm font-semibold text-rose-900">${c(d.toUpperCase())}</span>
        ...
        <div class="flex-shrink-0">${n(d)}</div>
      </li>
    `);
  return `
...
function He(e) {
  return e.activeAssignments.length ? `
    ...
    const t = ze(a.dueDate), s = t === "none" ? "No due date" : h(t);
    return `
              <li class="rounded-xl border border-gray-200 bg-gray-50 p-6">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <span class="rounded-full px-2 py-0.5 text-xs font-medium ${Fe(a.status)}">${c(h(a.status))}</span>
                  <span class="rounded-full px-2 py-0.5 text-xs font-medium ${Oe(t)}">${c(s)}</span>
                </div>
                ...
                <p class="mt-1 text-xs text-gray-500">Updated ${c(D(a.updatedAt || a.createdAt)) || "n/a"}</p>
              </li>
            `;
  }).join("")}
      </ul>
  ...
  const a = e.blockers.length ? e.blockers.map((t) => {
    ...
    return `
            <li class="flex flex-wrap items-center gap-2">
              <span class="rounded-full px-2 py-0.5 text-xs font-medium ${Ue(t.blockerCode)}">${c(h(t.blockerCode))}</span>
              ...
              ${a.map((t) => `
                    <li class="rounded-xl border border-gray-200 bg-gray-50 p-6">
                      <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="rounded-full px-2 py-0.5 text-xs font-medium ${t.tone === "success" ? "bg-emerald-100 text-emerald-700" : t.tone === "warning" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-700"}">${c(D(t.timestamp))}</span>
                      </div>
...
function Qe(e) {
  return `
    <div class="${re}" aria-busy="true" aria-label="Loading">
...
function B(e, a) {
  return `
    <div class="flex items-center justify-center py-16" role="status" aria-label="Empty">
      <div class="max-w-md ${fe} p-8 text-center shadow-sm">
        <h2 class="${de}">${c(e)}</h2>
        <p class="${ue} mt-2">${c(a)}</p>
      </div>
    </div>
  `;
}
...
function Je(e, a = {}) {
  ...
  if (e.status === "empty") return `
      ${B("Family detail unavailable", e.message || "This family detail view does not have a backing payload yet.")}
      ${R(e)}
    `;
  if (e.status === "error" || e.status === "conflict") return `
      <div class="translation-family-detail-error">
        ${Xe(e.status === "conflict" ? "Family detail conflict" : "Family detail failed to load", e.message || (e.status === "conflict" ? "The family detail payload is out of date. Reload to fetch the latest state." : "The translation family detail request failed."))}
        ${R(e)}
      </div>
    `;
  const t = e.detail;
  if (!t) return B("Family detail unavailable", "No family detail payload was returned.");
...
function Z(e) {
  ...
  n.className = oe, n.setAttribute("data-translation-create-locale-modal", "true"), n.innerHTML = `
    <div class="${ie}" role="dialog" aria-modal="true" aria-labelledby="translation-create-locale-title">
      <form class="p-6">
        <div class="flex items-start justify-between gap-4">
          ...
          <button type="button" data-close-modal="true" class="${me}">Close</button>
        </div>
          ...
          <label class="grid gap-2">
            ...
            <select name="locale" class="rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900">
              ${t.missingLocales.map((g) => `
                <option value="${_(g)}" ${g === i ? "selected" : ""}>
                  ${c(g.toUpperCase())}${g === t.recommendedLocale ? " (recommended)" : ""}
                </option>
              `).join("")}
            </select>
          </label>
    ...
    "urgent"
                  ...
                  <option value="${g}" ${g === (t.defaultAssignment.priority || "normal") ? "selected" : ""}>${h(g)}</option>
                `).join("")}
              </select>
            </label>
            <label class="grid gap-2">
              <span class="text-sm font-medium text-gray-900">Due date</span>
              <input type="datetime-local" name="due_date" value="${_(ea(t.defaultAssignment.dueDate))}" class="rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900">
            </label>
          </div>
        </div>
        ...
        <div class="mt-6 flex items-center justify-end gap-3">
          ...
          <button type="submit" class="${P}">${c(e.submitLabel || "Create locale")}</button>
        </div>
      </form>
    </div>
  `, a.body.appendChild(n);
  const l = n.querySelector('[role="dialog"]'), d = n.querySelector("form"), o = n.querySelector('select[name="locale"]'), y = n.querySelector('input[name="auto_create_assignment"]'), m = n.querySelector('input[name="assignee_id"]'), v = n.querySelector('select[name="priority"]'), w = n.querySelector('input[name="due_date"]'), F = n.querySelector('[data-assignment-fields="true"]'), C = n.querySelector('[data-create-locale-feedback="true"]'), $ = n.querySelector('button[type="submit"]'), q = () => {
    ae(), n.remove();
  }, N = () => {
    !F || !y || (F.hidden = !y.checked);
  }, ae = l ? ye(l, q) : () => {
  };
  N(), y?.addEventListener("change", N), n.querySelectorAll('[data-close-modal="true"]').forEach((g) => {
    g.addEventListener("click", q);
  }), n.addEventListener("click", (g) => {
    g.target === n && q();
  }), d?.addEventListener("submit", async (g) => {
    if (g.preventDefault(), !o || !$) return;
    C && (C.hidden = !0, C.textContent = ""), $.disabled = !0, $.classList.add("opacity-60", "cursor-not-allowed");
    const U = r(o.value).toLowerCase();
    try {
      const T = await e.onSubmit({
        locale: U,
        autoCreateAssignment: y?.checked,
        assigneeId: m?.value,
        priority: v?.value,
        dueDate: aa(w?.value || "")
      });
      q(), await e.onSuccess?.(T);
    } catch (T) {
      const j = Ze(T, U);
      C && (C.hidden = !1, C.textContent = j), L("error", j);
    } finally {
...
function sa(e) {
  return {
    ...
    quickCreate: E(M(e.dataset.quickCreate || "", {}), {}),
    ...
    a.querySelectorAll('[data-action="create-locale"]').forEach((i) => {
      i.addEventListener("click", (n) => {
        ...
        Z({
          ...
          onSuccess: async (d) => {
            L("success", `${d.locale.toUpperCase()} locale created.`);
            const o = typeof window < "u" && window.location.pathname.endsWith("/edit"), y = ta(d, t.localeURLs, t.requestedLocale, o);
...
async function V(e, a = {}) {
  ...
  if (z(e, n, i), typeof e.querySelector == "function") {
    if (n.status === "ready" && n.detail) {
      const d = ee({
        basePath: `${x(i.basePath || "/admin")}/api`,
        fetch: a.fetch
      });
      e.querySelectorAll('[data-family-create-locale="true"]').forEach((o) => {
        o.dataset.translationCreateBound !== "true" && (o.dataset.translationCreateBound = "true", o.addEventListener("click", (y) => {
          y.preventDefault();
          const m = n.detail;
          if (!m) {
          ...
          if (o.getAttribute("aria-disabled") === "true") {
            L("warning", m.quickCreate.disabledReason || "Locale creation is unavailable.");
            return;
          }
          const v = r(o.dataset.locale).toLowerCase() || m.quickCreate.recommendedLocale || "";
          Z({
            familyId: m.familyId,
            quickCreate: m.quickCreate,
            initialLocale: v,
            heading: `Create ${v.toUpperCase()} locale`,
            onSubmit: (w) => d.createLocale(m.familyId, w),
            onSuccess: async (w) => {
              L("success", `${w.locale.toUpperCase()} locale created.`), await V(e, {
                ...a,
                ...i,
                endpoint: s
              });
...
function ee(e = {}) {
  ...
  return {
    async list(s = {}) {
      return qe(await (await a(he(t, s), { headers: { Accept: "application/json" } })).json());
    },
    async detail(s, i = "") {
      return X(await (await a(xe(t, s, i), { headers: { Accept: "application/json" } })).json());
    },
    async createLocale(s, i = {}) {
      ...
      if (!l.ok) throw await Re(l);
      return $e(await l.json());
    }
  };
}
function x(e) {
