import { b as i, h as u, s as g } from "../chunks/dom-helpers-Cd24RS2-.js";
import { m as w } from "../chunks/formatters-DYQo8z6P.js";
...
class _ {
  ...
  setupEventListeners() {
    ...
    e?.addEventListener("click", () => this.loadConflicts()), t?.addEventListener("click", () => this.loadConflicts()), n?.addEventListener("click", () => this.closeConflictDetail()), o?.addEventListener("change", () => this.loadConflicts()), a?.addEventListener("change", () => this.renderConflicts()), l?.addEventListener("change", () => this.renderConflicts()), r?.addEventListener("click", () => this.openResolveModal("resolved")), f?.addEventListener("click", () => this.openResolveModal("ignored")), s?.addEventListener("click", () => this.closeResolveModal()), d?.addEventListener("submit", (c) => this.submitResolution(c)), document.addEventListener("keydown", (c) => {
      ...
      c?.addEventListener("click", (x) => {
        const y = x.target;
        (y === c || y.getAttribute("aria-hidden") === "true") && (c === p ? this.closeConflictDetail() : c === h && this.closeResolveModal());
      });
    });
  }
  /**
  ...
  async loadConflicts() {
    ...
    try {
      ...
      if (!n.ok)
        throw new Error(
          await M(n, `HTTP ${n.status}`, {
            appendStatusToFallback: !1
          })
        );
      const o = await n.json();
      this.conflicts = o.conflicts || [], this.populateProviderFilter(), this.updateStats(), this.renderConflicts(), P(
  ...
  populateProviderFilter() {
    ...
    if (!e) return;
    const t = e.value, n = [...new Set(this.conflicts.map((o) => o.provider).filter(Boolean))];
  ...
  openConflictDetail(e) {
    ...
    if (o && (o.textContent = t.reason || "Data conflict"), a && (a.textContent = t.entity_kind || "-"), l && (l.innerHTML = this.getStatusBadge(t.status)), r && (r.textContent = t.provider || "-"), f && (f.textContent = t.external_id || "-"), s && (s.textContent = t.internal_id || "-"), d && (d.textContent = t.binding_id || "-"), p && (p.textContent = t.id), h && (h.textContent = t.run_id || "-"), c && (c.textContent = w(t.created_at)), x && (x.textContent = String(t.version || 1)), y) {
      let v = !1;
      const C = S(t.payload_json, t.payload || {}, {
        onError: () => {
          v = !0;
        }
      });
      y.textContent = v ? t.payload_json || "{}" : JSON.stringify(C, null, 2);
    }
    if (t.status === "resolved" || t.status === "ignored") {
      if (g(E), u(B), R && (R.textContent = t.resolved_at ? w(t.resolved_at) : ""), $ && ($.textContent = t.resolved_by_user_id ? `By user ${t.resolved_by_user_id}` : "-"), k) {
        let v = !1;
        const C = S(t.resolution_json, t.resolution || {}, {
          onError: () => {
            v = !0;
          }
        });
        k.textContent = v ? t.resolution_json || "{}" : JSON.stringify(C, null, 2);
      }
    } else
      u(E), g(B);
    g(n);
  ...
  async submitResolution(e) {
    ...
    try {
      ...
      if (!s.ok)
        throw new Error(
          await M(s, `HTTP ${s.status}`, {
            appendStatusToFallback: !1
          })
        );
      L("Conflict resolved", "success"), P(this.elements.announcements, "Conflict resolved"), this.closeResolveModal(), this.closeConflictDetail(), await this.loadConflicts();
    } catch (s) {
...
function N(m) {
  ...
  I(() => t.init()), typeof window < "u" && (window.esignIntegrationConflictsController = t);
}
export {
