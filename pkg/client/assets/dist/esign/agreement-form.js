import { j as sn } from "../chunks/dom-helpers-Cd24RS2-.js";
import { l as Wt, a as Vt } from "../chunks/runtime-bhSs9hEJ.js";
import { parseJSONValue as Kt } from "../shared/json-parse.js";
import { readHTTPError as an, readHTTPJSONObject as on, readHTTPJSON as Pt, readHTTPStructuredErrorResult as cn } from "../shared/transport/http-client.js";
import { escapeHTML as Ue } from "../shared/html.js";
import { formatRelativeTimeVerbosePast as Yt } from "../shared/time-formatters.js";
...
async function fn(n, e, t = bt.THUMBNAIL_MAX_WIDTH, i = bt.THUMBNAIL_MAX_HEIGHT) {
  const l = await Vt({
    url: n,
    withCredentials: !0
...
class gn {
  ...
  async setDocument(e, t = null, i = null) {
    ...
    try {
      if (p = await this.fetchDocumentPdfUrl(e), s !== this.requestVersion)
        return;
      const { dataUrl: y, pageCount: _ } = await fn(
        p,
        e,
  ...
  render() {
    ...
    if (e) {
      ...
      if (this.state.error) {
        p?.classList.remove("hidden"), this.elements.errorMessage && (this.elements.errorMessage.textContent = this.state.errorMessage || "Preview unavailable"), this.elements.errorSuggestion && (this.elements.errorSuggestion.textContent = this.state.errorSuggestion || "", this.elements.errorSuggestion.classList.toggle("hidden", !this.state.errorSuggestion)), this.elements.errorRetryBtn && this.elements.errorRetryBtn.classList.toggle("hidden", !this.state.errorRetryable), this.elements.errorDebugInfo && (un() ? (this.elements.errorDebugInfo.textContent = this.state.error, this.elements.errorDebugInfo.classList.remove("hidden")) : this.elements.errorDebugInfo.classList.add("hidden"));
        return;
      }
...
async function In(n) {
  ...
  return typeof window < "u" && window.__esignSyncCoreModule ? Dt(window.__esignSyncCoreModule) : (Et.has(e) || Et.set(e, _n(e)), Et.get(e));
}
async function _n(n) {
  if (typeof window < "u" && typeof window.__esignSyncCoreLoader == "function")
    return Dt(await window.__esignSyncCoreLoader(n));
...
class En {
  ...
  async bootstrap() {
    ...
    if (!t.ok)
      throw new Error(
        await an(t, `HTTP ${t.status}`, {
          appendStatusToFallback: !1
        })
      );
    const i = await on(t), s = this.normalizeResourceRef(i?.resource_ref);
    if (!s)
  ...
  async ensureRuntime() {
    return this.syncModule ? this.syncModule : (this.syncModulePromise || (this.syncModulePromise = In(this.syncConfig.client_base_path)), this.syncModule = await this.syncModulePromise, this.cache || (this.cache = this.syncModule.createInMemoryCache()), this.transport || (this.transport = this.syncModule.createFetchSyncTransport({
...
function Pe(n = {}) {
  ...
  return {
    ...
    serverRevision: Ft(e?.serverRevision),
    currentStep: Ft(e?.currentStep),
...
function Dn(n) {
  ...
  function w() {
    const a = Ke("send-readiness-loading"), f = Ke("send-readiness-results"), u = Ke("send-validation-status"), h = Ke("send-validation-issues"), M = Ke("send-issues-list"), Y = Ke("send-confirmation"), L = Ke("review-agreement-title"), b = Ke("review-document-title"), g = Ke("review-participant-count"), U = Ke("review-stage-count"), J = Ke("review-participants-list"), q = Ke("review-fields-summary"), N = document.getElementById("title");
    if (!a || !f || !u || !h || !M || !Y || !L || !b || !g || !U || !J || !q || !(N instanceof HTMLInputElement))
      return;
    ...
    }), L.textContent = ie, b.textContent = j, g.textContent = `${he.length} (${de.length} signers)`, U.textContent = String(ee.size > 0 ? ee.size : 1), J.innerHTML = "", he.forEach((Q) => {
      const B = ft(Q, 'input[name*=".name"]'), De = ft(Q, 'input[name*=".email"]'), _e = ft(Q, 'select[name*=".role"]', "signer"), ce = ft(Q, ".signing-stage-input"), me = An(Q, ".notify-input", !0), Me = document.createElement("div");
...
function Pn(n) {
  ...
  function Y() {
    if (w.forEach((g, U) => {
      ...
      Mt(g.dataset.step) === T ? g.classList.remove("hidden") : g.classList.add("hidden");
  ...
  function b() {
    w.forEach((g) => {
      g.addEventListener("click", () => {
        const U = Mt(g.dataset.step);
        L(U);
      });
    }), u?.addEventListener("click", () => L(T - 1)), h?.addEventListener("click", () => L(T + 1)), M?.addEventListener("click", () => {
        ...
        ue.preventDefault(), i.disabled = !0, Me(le ? "Sending..." : H ? "Starting Review..." : "Saving...", !0), (async () => {
          try {
            ...
            if (r.toUpperCase() === "STALE_REVISION") {
              const c = Fn(z, Number(u.getState()?.serverRevision || 0));
...
function vt(n, e) {
  ...
  return {
    ...
    page: ut(n.page, t),
    excludeLastPage: Xt(n.excludeLastPage ?? n.exclude_last_page),
    excludePages: s,
    required: p
  };
}
...
function zn(n, e) {
  ...
  return n.forEach((s, l) => {
    ...
    if (p.type === "initials_each_page") {
      const _ = Bn(p.fromPage, p.toPage, t), F = /* @__PURE__ */ new Set();
...
function pt(n, e = 0) {
  const t = n || {}, i = Ie(t.id) || `fi_init_${e}`, s = Ie(t.definitionId || t.definition_id || t.field_definition_id) || i, l = ke(t.page ?? t.page_number, 1), p = gt(t.x ?? t.pos_x, 0), y = gt(t.y ?? t.pos_y, 0), _ = gt(t.width, Bt), F = gt(t.height, $t);
  return {
    id: i,
    definitionId: s,
    type: Ie(t.type) || "text",
...
function On(n, e = 0) {
  ...
  return {
    ...
    placement_source: Zt(t.placementSource),
...
function jn(n) {
  ...
  } = n, a = Le("document_id"), f = Le("selected-document"), u = Le("document-picker"), h = Le("document-search"), M = Le("document-list"), Y = Le("change-document-btn"), L = Le("selected-document-title"), b = Le("selected-document-info"), g = Le("document_page_count"), U = Le("document-remediation-panel"), J = Le("document-remediation-message"), q = Le("document-remediation-status"), N = Le("document-remediation-trigger-btn"), ie = Le("document-remediation-dismiss-btn"), j = Le("title"), he = 300, Se = 5, X = 10, de = Le("document-typeahead"), ee = Le("document-typeahead-dropdown"), ye = Le("document-recent-section"), pe = Le("document-recent-list"), Ee = Le("document-search-section"), Be = Le("document-search-list"), xe = Le("document-empty-state"), Ae = Le("document-dropdown-loading"), Q = Le("document-search-loading"), B = {
    isOpen: !1,
    query: "",
    recentDocuments: [],
    searchResults: [],
  ...
  function I() {
    a && (a.value = ""), L && (L.textContent = ""), b && (b.textContent = ""), m(0), y.updateDocument({
      id: null,
      title: null,
      pageCount: null
  ...
  function ne(S, v, E) {
    if (!a || !L || !b || !f || !u)
      return;
    a.value = String(S || ""), L.textContent = v || "", b.textContent = `${E} pages`, m(E), f.classList.remove("hidden"), u.classList.add("hidden"), w(), se(v);
    const $ = ke(E, 0);
    y.updateDocument({
      id: S,
      title: v,
  ...
  function le() {
    ...
    v && (L.textContent.trim() || (L.textContent = v.title || "Untitled"), (!b.textContent.trim() || b.textContent.trim() === "pages") && (b.textContent = `${v.pageCount || 0} pages`), m(v.pageCount || 0), f.classList.remove("hidden"), u.classList.add("hidden"));
  }
  async function z() {
    try {
      ...
      }).map((K) => Rt(K)).filter((K) => K.id !== ""), oe(De), le();
    } catch (S) {
  ...
  async function r(S, v, E) {
    ...
    for (; Date.now() - O < K; ) {
      ...
      const re = (await Hn(Fe))?.dispatch || {}, we = String(re?.status || "").trim().toLowerCase();
      if (we === "succeeded") {
  ...
  async function R() {
    try {
      ...
      const E = await Lt(v), $ = Array.isArray(E?.records) ? E.records : Array.isArray(E?.items) ? E.items : [];
      B.recentDocuments = $.map((O) => Rt(O)).filter((O) => O.id !== "").slice(0, Se);
    } catch (S) {
  ...
  async function C(S) {
    ...
    try {
      ...
      const K = await Lt(O), ve = Array.isArray(K?.records) ? K.records : Array.isArray(K?.items) ? K.items : [];
      B.searchResults = ve.map((Fe) => Rt(Fe)).filter((Fe) => Fe.id !== "").slice(0, X);
    } catch ($) {
  ...
  function be(S, v, E) {
    S && (S.innerHTML = v.map(($, O) => {
      const K = O, ve = B.selectedIndex === K, Fe = Ue(String($.id || "").trim()), D = Ue(String($.title || "").trim()), re = String(ke($.pageCount, 0)), we = qe($.compatibilityTier), tt = ze($.compatibilityReason), Qe = Ue(we), kt = Ue(tt), rn = je(we) ? '<span class="ml-1 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium text-amber-700">Unsupported</span>' : "";
      return `
        <button type="button"
          ...
          data-document-compatibility-reason="${kt}"
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="text-xs text-gray-500">${re} pages ${rn}</div>
          </div>
        </button>
      `;
  ...
  function Ye(S) {
    ...
    switch (S.key) {
      case "ArrowDown":
        S.preventDefault(), B.selectedIndex = Math.min(B.selectedIndex + 1, E), ge(), Re();
        break;
      case "ArrowUp":
        S.preventDefault(), B.selectedIndex = Math.max(B.selectedIndex - 1, 0), ge(), Re();
        break;
      case "Enter":
      ...
      case "Tab":
        ...
        break;
      case "Home":
        S.preventDefault(), B.selectedIndex = 0, ge(), Re();
        break;
      case "End":
        S.preventDefault(), B.selectedIndex = E, ge(), Re();
        break;
    }
  }
  function Te() {
...
function Gn(n = {}) {
  ...
  function F(f = {}) {
    ...
    const Y = Je(h, ".participant-id-input"), L = Je(h, 'input[name="participants[].name"]'), b = Je(h, 'input[name="participants[].email"]'), g = Ct(h, 'select[name="participants[].role"]'), U = Je(h, 'input[name="participants[].signing_stage"]'), J = Je(h, 'input[name="participants[].notify"]'), q = h.querySelector(".signing-stage-wrapper");
    if (!Y || !L || !b || !g) return;
    const N = y++;
    Y.name = `participants[${N}].id`, Y.value = M, L.name = `participants[${N}].name`, b.name = `participants[${N}].email`, g.name = `participants[${N}].role`, U && (U.name = `participants[${N}].signing_stage`), J && (J.name = `participants[${N}].notify`), f.name && (L.value = f.name), f.email && (b.value = f.email), f.role && (g.value = f.role), U && f.signing_stage && (U.value = String(f.signing_stage)), J && (J.checked = f.notify !== !1);
    const ie = () => {
  ...
  function T() {
    ...
    return f.forEach((h) => {
      const M = h.getAttribute("data-participant-id"), Y = Ct(h, 'select[name*=".role"]'), L = Je(h, 'input[name*=".name"]'), b = Je(h, 'input[name*=".email"]');
      Y?.value === "signer" && u.push({
        id: String(M || ""),
        name: L?.value || b?.value || "Signer",
        email: b?.value || ""
      });
    }), u;
  }
  function w() {
    ...
    return i.querySelectorAll(".participant-entry").forEach((u) => {
      const h = u.getAttribute("data-participant-id"), M = Je(u, 'input[name*=".name"]')?.value || "", Y = Je(u, 'input[name*=".email"]')?.value || "", L = Ct(u, 'select[name*=".role"]')?.value || "signer", b = Number.parseInt(Je(u, ".signing-stage-input")?.value || "1", 10), g = Je(u, ".notify-input")?.checked !== !1;
      f.push({
        tempId: String(h || ""),
        name: M,
        email: Y,
...
function Jn(n) {
  ...
  } = n, P = Xe("field-definitions-container"), T = document.getElementById("field-definition-template"), w = Xe("add-field-btn"), a = Xe("add-field-btn-container"), f = Xe("add-field-definition-empty-btn"), u = Xe("field-definitions-empty-state"), h = Xe("field-rules-container"), M = document.getElementById("field-rule-template"), Y = Xe("add-field-rule-btn"), L = Xe("field-rules-empty-state"), b = Xe("field-rules-preview"), g = Xe("field_rules_json"), U = Xe("field_placements_json");
  let J = 0, q = 0, N = 0;
  function ie() {
  ...
  function ee() {
    ...
    L.classList.toggle("hidden", m.length > 0);
  }
  function ye() {
    if (!P) return [];
    const m = [];
  ...
  function Q(m) {
    ...
    const oe = i(), ae = vt({
      ...
      required: !0
    }, oe), r = ae.fromPage > 0 ? ae.fromPage : 1, o = ae.toPage > 0 ? ae.toPage : oe, c = ae.page > 0 ? ae.page : ae.toPage > 0 ? ae.toPage : oe, R = ae.excludeLastPage, C = ae.excludePages.join(","), k = d?.value === "initials_each_page";
    if (I.classList.toggle("hidden", !k), A.classList.toggle("hidden", !k), te.classList.toggle("hidden", !k), fe.classList.toggle("hidden", !k), G.classList.toggle("hidden", k), ne && (ne.value = String(r)), V && (V.value = String(o)), H && (H.value = String(c)), z && (z.value = C), le && (le.checked = R), k) {
      const W = Un(
        r,
        o,
        oe,
  ...
  function B(m = {}) {
    ...
    const fe = Ce(I, ".field-rule-id-input"), se = Oe(I, ".field-rule-type-select"), ne = Oe(I, ".field-rule-participant-select"), V = Ce(I, ".field-rule-from-page-input"), H = Ce(I, ".field-rule-to-page-input"), le = Ce(I, ".field-rule-page-input"), z = Oe(I, ".field-rule-required-select"), oe = Ce(I, ".field-rule-exclude-last-input"), ae = Ce(I, ".field-rule-exclude-pages-input"), r = qt(I, ".remove-field-rule-btn");
  ...
  function De(m = {}) {
    ...
    }), te.value === "date_signed" && V.classList.remove("hidden"), qt(I, ".remove-field-definition-btn")?.addEventListener("click", () => {
      I.remove(), de(), p?.();
    });
...
function Qn(n) {
  ...
  function q() {
    return a.fieldInstances.map((r, o) => On(r, o));
  }
  function N(r = {}) {
    const { silent: o = !1 } = r, c = b();
    c.fieldInstancesContainer && (c.fieldInstancesContainer.innerHTML = "");
    const R = q();
    return s && (s.value = JSON.stringify(R)), o || w?.(), R;
  }
  function ie() {
    const r = b(), o = Array.from(document.querySelectorAll(".placement-field-item")), c = o.length, R = new Set(
  ...
  function j(r, o = !1) {
    ...
    R && (R.textContent = "Placed", R.classList.remove("text-amber-600"), R.classList.add("text-green-600")), o && c.classList.add("just-linked");
  }
  function he(r) {
    const o = h(r);
    if (!o) return;
  ...
  function de(r, o) {
    ...
    const R = () => xe(r, o);
    return c.addEventListener("click", R), c.addEventListener("keydown", (C) => {
    ...
    r.linkAllBtn && !r.linkAllBtn.dataset.bound && (r.linkAllBtn.dataset.bound = "true", r.linkAllBtn.addEventListener("click", () => {
      ...
      if (o !== 0) {
        for (const c of a.linkGroupState.unlinkedDefinitions)
          a.linkGroupState = Ut(a.linkGroupState, c);
      ...
      let o = 0;
      for (const c of a.linkGroupState.definitionToGroup.keys())
        a.linkGroupState.unlinkedDefinitions.has(c) || (a.linkGroupState = zt(a.linkGroupState, c), o += 1);
      o > 0 && window.toastManager && window.toastManager.success(`Unlinked ${o} field${o > 1 ? "s" : ""}`), Be();
    })), ee();
  }
  function pe() {
    return y().map((o) => {
      const c = String(o.definitionId || "").trim(), R = a.linkGroupState.definitionToGroup.get(c) || "", C = a.linkGroupState.unlinkedDefinitions.has(c);
      return { ...o, definitionId: c, linkGroupId: R, isUnlinked: C };
    });
  }
  function Ee() {
    const r = b();
    ...
    const o = pe();
    r.linkBatchActions && r.linkBatchActions.classList.toggle("hidden", a.linkGroupState.groups.size === 0), o.forEach((c, R) => {
      ...
      const Re = at[k] || at.text, Ye = a.fieldInstances.some((ve) => ve.definitionId === C), Te = document.createElement("div");
      ...
      S.className = `w-3 h-3 rounded ${Re.bg}`;
  ...
  function xe(r, o) {
    o ? (a.linkGroupState = zt(a.linkGroupState, r), window.toastManager?.info?.("Field unlinked")) : (a.linkGroupState = Ut(a.linkGroupState, r), window.toastManager?.info?.("Field re-linked")), Be();
  }
  async function Ae(r) {
    const o = a.pdfDoc;
    if (!o) return;
    ...
    const R = c.canvas.getContext("2d"), C = await o.getPage(r), k = C.getViewport({ scale: a.scale });
  ...
  function Q(r) {
    const o = Kn(a.linkGroupState, r);
    o && (a.linkGroupState = Qt(a.linkGroupState, o.updatedGroup));
  }
  function B(r) {
    const o = /* @__PURE__ */ new Map();
    y().forEach((R) => {
      const C = String(R.definitionId || "").trim();
      C && o.set(C, {
    ...
    for (; c < 10; ) {
      ...
      a.fieldInstances.push(R.newPlacement), j(R.newPlacement.definitionId, !0), c += 1;
    }
    c > 0 && (_e(), ie(), N(), X(c));
  }
  function De(r) {
  ...
  function _e() {
    ...
    o && (o.innerHTML = "", o.style.pointerEvents = "auto", a.fieldInstances.filter((c) => c.page === a.currentPage).forEach((c) => {
      const R = at[c.type] || at.text, C = a.selectedFieldId === c.id, k = c.placementSource === Ge.AUTO_LINKED, W = document.createElement("div"), Z = k ? "border-dashed" : "border-solid";
      W.className = `field-overlay absolute cursor-move ${R.border} border-2 ${Z} rounded`, W.style.cssText = `
          left: ${c.x * a.scale}px;
          top: ${c.y * a.scale}px;
      ...
      if (be.className = `absolute -top-5 left-0 text-xs whitespace-nowrap px-1 rounded text-white ${R.bg}`, be.textContent = `${c.type.replace("_", " ")} - ${c.participantName}`, W.appendChild(be), k) {
        const Re = document.createElement("div");
        Re.className = "absolute -top-1 -right-1 w-4 h-4 bg-blue-500 rounded-full flex items-center justify-center", Re.title = "Auto-linked from template", Re.innerHTML = `<svg class="w-2.5 h-2.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
            <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/>
            <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/>
          </svg>`, W.appendChild(Re);
      }
      ...
      Ne.type = "button", Ne.className = "absolute -top-2 -right-2 w-5 h-5 bg-red-500 text-white rounded-full text-xs flex items-center justify-center hover:bg-red-600", Ne.innerHTML = "×", Ne.addEventListener("click", (Re) => {
        Re.stopPropagation(), qe(c.id);
      }), W.appendChild(Ne), W.addEventListener("mousedown", (Re) => {
        Re.target === ge ? We(Re, c) : Re.target !== Ne && Me(Re, c, W);
  ...
  function ce(r, o, c, R = {}) {
    const C = ht[r.fieldType] || ht.text, k = R.placementSource || Ge.MANUAL, W = R.linkGroupId || en(a.linkGroupState, r.definitionId)?.id, Z = {
      id: u("fi"),
  ...
  function Me(r, o, c) {
    ...
    const R = r.clientX, C = r.clientY, k = o.x * a.scale, W = o.y * a.scale;
    function Z(ge) {
      const Ne = ge.clientX - R, Re = ge.clientY - C;
      o.x = Math.max(0, (k + Ne) / a.scale), o.y = Math.max(0, (W + Re) / a.scale), o.placementSource = Ge.MANUAL, c.style.left = `${o.x * a.scale}px`, c.style.top = `${o.y * a.scale}px`;
    }
    function be() {
  ...
  function We(r, o) {
    ...
    const c = r.clientX, R = r.clientY, C = o.width, k = o.height;
    function W(be) {
      const ge = (be.clientX - c) / a.scale, Ne = (be.clientY - R) / a.scale;
  ...
  function ze(r, o) {
    const R = b().canvas;
    !r || !R || (r.addEventListener("dragover", (C) => {
      C.preventDefault(), C.dataTransfer && (C.dataTransfer.dropEffect = "copy"), R.classList.add("ring-2", "ring-blue-500", "ring-inset");
    }), r.addEventListener("dragleave", () => {
      R.classList.remove("ring-2", "ring-blue-500", "ring-inset");
    }), r.addEventListener("drop", (C) => {
      C.preventDefault(), R.classList.remove("ring-2", "ring-blue-500", "ring-inset");
      const k = C.dataTransfer?.getData("application/json") || "";
      if (!k) return;
      const W = Kt(k, null);
      if (!W) return;
      const Z = R.getBoundingClientRect(), be = (C.clientX - Z.left) / a.scale, ge = (C.clientY - Z.top) / a.scale;
      ce(W, be, ge);
    }));
  }
  function je() {
  ...
  function fe() {
    ...
    o.forEach((c) => {
      const R = Number.parseInt(c.dataset.index || "", 10), C = f.suggestions[R];
      if (!C) return;
  ...
  function se(r) {
    r.querySelectorAll(".accept-suggestion-btn").forEach((o) => {
      ...
      o.addEventListener("click", () => {
        const c = Number.parseInt(o.dataset.index || "", 10), R = f.suggestions[c];
        R && G(R);
      });
    });
  }
  function ne() {
    ...
    `, Y(r, "#close-suggestions-modal")?.addEventListener("click", () => {
      ...
      r.classList.add("hidden");
      const o = L(r, "#placement-policy-preset-modal"), c = b().policyPreset;
  ...
  function V(r) {
    ...
    const c = L(o, "#suggestions-list"), R = L(o, "#resolver-info"), C = L(o, "#run-stats");
    !c || !R || !C || (R.innerHTML = f.resolverScores.map((k) => `
    ...
    `, c.innerHTML = f.suggestions.map((k, W) => {
      const Z = _(k.field_definition_id), be = at[Z?.type || "text"] || at.text, ge = T(String(Z?.type || "field").replace(/_/g, " ")), Ne = T(String(k?.id || "")), Re = Math.max(1, Number(k?.page_number || 1)), Ye = Math.round(Number(k?.x || 0)), Te = Math.round(Number(k?.y || 0)), S = Math.max(0, Number(k?.confidence || 0)), v = T(A(String(k?.resolver_id || "")));
      return `
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="flex items-center gap-2">
              ...
              <div>
                ...
                <div class="text-xs text-gray-500">Page ${Re}, (${Ye}, ${Te})</div>
              </div>
            </div>
            <div class="flex items-center gap-2">
  ...
  function H() {
    ...
    r.forEach((c) => {
      ...
      }, C = ht[R.fieldType || "text"] || ht.text;
      a.currentPage = a.totalPages, ce(R, 300, o + C.height / 2, { placementSource: Ge.AUTO_FALLBACK }), o += C.height + 20;
  ...
  async function le() {
    ...
    `;
    try {
      const R = await fetch(`${t}/esign/agreements/${c}/auto-place`, {
        method: "POST",
        credentials: "same-origin",
        headers: {
      ...
      if (!R.ok)
        throw await F(R, "Auto-placement failed");
      const C = await Zn(R), k = Xn(C) ? C.run || {} : C;
      ...
      console.error("Auto-place error:", R);
      const C = R && typeof R == "object" ? R : {}, k = x(C.message || "Auto-placement failed", C.code || "", C.status || 0);
  ...
  async function oe() {
    ...
    const R = ++a.loadRequestVersion, C = String(i.value || "").trim(), k = encodeURIComponent(C), W = `${e}/panels/esign_documents/${k}/source/pdf`;
    try {
      const be = await Vt({
        url: W,
        withCredentials: !0,
  ...
  function ae(r) {
    ...
    a.fieldInstances = o.map((c, R) => pt(c, R)), N({ silent: !0 });
  }
  return N({ silent: !0 }), {
    bindEvents: z,
...
function ei(n) {
  ...
  function L(J) {
    if (!F)
      return;
    const q = Ot(J), N = /* @__PURE__ */ new Map();
  ...
  function g(J) {
    const q = Ot(J?.review);
    s instanceof HTMLInputElement && (s.checked = !q.enabled), l instanceof HTMLInputElement && (l.checked = q.enabled), y instanceof HTMLSelectElement && (y.value = q.gate), _ instanceof HTMLInputElement && (_.checked = q.commentsEnabled), L(q), x && (x.innerHTML = "", q.participants.filter((N) => String(N.participantType || "").trim() === "external").forEach((N) => M(N)), u()), h();
  }
  function U() {
    [s, l].forEach((J) => {
...
function ti(n) {
  ...
  function w() {
    ...
    l.querySelectorAll(".participant-entry").forEach((M) => {
      const Y = String(M.getAttribute("data-participant-id") || "").trim(), L = st(M, 'input[name*=".name"]'), b = st(M, 'input[name*=".email"]'), g = st(M, 'select[name*=".role"]', "signer"), U = Ht(M, ".notify-input", !0), J = st(M, ".signing-stage-input"), q = Number(J || "1") || 1;
      a.push({
        id: Y,
        name: L,
        email: b,
    ...
    p.querySelectorAll(".field-definition-entry").forEach((M) => {
      const Y = String(M.getAttribute("data-field-definition-id") || "").trim(), L = st(M, ".field-type-select", "signature"), b = st(M, ".field-participant-select"), g = Number(st(M, 'input[name*=".page"]', "1")) || 1, U = Ht(M, 'input[name*=".required"]');
      Y && f.push({
        id: Y,
        type: L,
        participant_id: b,
...
function ni(n) {
  ...
  function he(X, de = {}) {
    Y(() => {
      if (L(X), b(X), U(X), J(X), q(X), _(), N(X), ie(X), de.updatePreview !== !1) {
        const ye = X?.document;
        ye?.id ? F.setDocument(
          ye.id,
          ye.title || null,
...
function ai(n) {
  ...
  function P(L, b) {
    return i.normalizeLoadedState({
      ...
      fieldDefinitions: L.fieldDefinitions,
      fieldPlacements: L.fieldPlacements,
      fieldRules: L.fieldRules,
      titleSource: L.titleSource,
      syncPending: !0,
  ...
  async function T() {
    ...
    const L = i.normalizeLoadedState(i.getState());
    ...
    const b = String(L?.serverDraftId || "").trim();
    if (!b) {
      if (!y(L))
        try {
      ...
      return i.setState(L, { syncPending: !!L.syncPending, notify: !1 }), Ve("resume_reconcile_complete", Pe({
        state: L,
        storageKey: t,
        ownership: x?.() || void 0,
        sendAttemptId: null,
    ...
    try {
      const g = await l.load(b), U = i.normalizeLoadedState({
        ...
        resourceRef: g?.resource_ref || L.resourceRef || null,
        serverDraftId: String(g?.id || b).trim() || b,
        serverRevision: Number(g?.revision || 0),
        lastSyncedAt: String(g?.updated_at || g?.updatedAt || "").trim() || L.lastSyncedAt,
        syncPending: !1
      }), J = String(L.serverDraftId || "").trim() === String(U.serverDraftId || "").trim(), q = J && L.syncPending === !0 ? P(L, U) : U;
      return i.setState(q, { syncPending: !!q.syncPending, notify: !1 }), Ve("resume_reconcile_complete", Pe({
        ...
        sendAttemptId: null,
        extra: {
          source: J && L.syncPending === !0 ? "merged_local_over_remote" : "remote_draft",
  ...
  function a() {
    const L = document.getElementById("resume-dialog-modal"), b = i.getState(), g = String(b?.document?.title || "").trim() || String(b?.document?.id || "").trim() || "Unknown document", U = w("resume-draft-title"), J = w("resume-draft-document"), q = w("resume-draft-step"), N = w("resume-draft-time");
    U && (U.textContent = b.details?.title || "Untitled Agreement"), J && (J.textContent = g), q && (q.textContent = String(b.currentStep || 1)), N && (N.textContent = _(b.updatedAt)), L?.classList.remove("hidden"), F("wizard_resume_prompt_shown", {
      step: Number(b.currentStep || 1),
      has_server_draft: !!b.serverDraftId
    });
  }
  async function f(L = {}) {
    const b = L.deleteServerDraft === !0, g = String(i.getState()?.serverDraftId || "").trim();
...
function oi(n) {
  ...
  async function x(w, a = "") {
    const f = Number(w?.status || 0), u = await cn(
      w,
...
function jt(n, e = {}) {
  ...
  return !!(p !== "" && _ !== i.SERVER_SEED || y !== "" || (Array.isArray(n.participants) ? n.participants : []).some((P, T) => li(P, T)) || Array.isArray(n.fieldDefinitions) && n.fieldDefinitions.length > 0 || Array.isArray(n.fieldPlacements) && n.fieldPlacements.length > 0 || Array.isArray(n.fieldRules) && n.fieldRules.length > 0 || n.review?.enabled);
}
function di(n = {}) {
...
function fi(n = {}) {
  const {
    ...
    config: e,
    isEditMode: y
  }), L = Cn(), b = (D, re = Y.AUTOFILL) => tn(D, re), g = (D) => jt(D, {
    normalizeTitleSource: b,
    titleSource: Y
  }), U = hn({
    apiBasePath: p,
    basePath: s
  }), J = w.form.root, q = mi(w.form.submitBtn, "submit button"), N = w.form.announcements;
  let ie = null, j = null, he = null, Se = null, X = null, de = null, ee = null, ye = null, pe = null, Ee = It();
  const Be = (D, re = {}) => {
    Se?.applyStateToUI(D, re);
  }, xe = () => Se?.debouncedTrackChanges?.(), Ae = () => ye?.trackWizardStateChanges?.(), Q = (D) => Yt(D, {
    emptyFallback: "unknown",
    invalidFallback: "Invalid Date"
  }), B = () => ee?.restoreSyncStatusFromState(), De = (D) => ee?.updateSyncStatus(D), _e = (D) => ee?.showSyncConflictDialog(D), ce = (D, re = "", we = 0) => ee?.mapUserFacingError(D, re, we) || String(D || "").trim(), me = (D, re) => ee ? ee.parseAPIError(D, re) : Promise.resolve({ status: Number(D.status || 0), code: "", details: {}, message: re }), Me = (D, re = "", we = 0) => ee?.announceError(D, re, we), We = (D, re = {}) => ee ? ee.surfaceSyncOutcome(D, re) : Promise.resolve({}), qe = () => null, ze = bn(w, {
  ...
  return Se = ni({
    ...
    parsePositiveInt: ke,
    isEditMode: y
  }), Se.bindChangeTracking(), X = si({
    documentIdInput: ne,
    titleInput: w.form.titleInput,
    participantsContainer: ae,
    fieldDefinitionsContainer: c,
    fieldRulesContainer: R,
    addFieldBtn: C,
    ...
    hasMeaningfulWizardProgress: jt,
    formatRelativeTime: Q,
    emitWizardTelemetry: (D, re) => L(D, re),
...
class nn {
  constructor(e) {
    this.initialized = !1, this.config = Si(e);
  }
  init() {
    this.initialized || (this.initialized = !0, yi(), hi(this.config));
  }
  destroy() {
    gi(), this.initialized = !1;
  }
}
function Ai(n) {
  const e = new nn(n);
...
function vi(n) {
  ...
  Tt(() => e.init()), typeof window < "u" && (window.esignAgreementFormController = e);
}
function bi(n) {
...
export {
  nn as AgreementFormController,
  vi as bootstrapAgreementForm,
  Ai as initAgreementForm
};
