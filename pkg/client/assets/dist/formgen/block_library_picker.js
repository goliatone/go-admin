...
function Q(t, r, n) {
  ...
  const v = G(t, n), L = document.createElement("span");
...
function X(t, r, n) {
  ...
  function E(a, c) {
    ...
    return a.status && a.status.toLowerCase() !== "active" && A.push(`(${a.status})`), N.textContent = A.join(" "), g.appendChild(m), g.appendChild(N), e.appendChild(s), e.appendChild(g), e;
  }
  function w(a = "") {
    b.innerHTML = "", u = [], d = -1;
...
async function O(t) {
  ...
  const { listBase: f, templatesBase: i } = V(o);
  if (!f || !i) {
  ...
  try {
    ...
    W(t, `Failed to load block definitions: ${s}`);
    return;
  }
  if (l.length > 0) {
    const e = /* @__PURE__ */ new Set();
    for (const s of l)
      for (const g of D(s)) e.add(g);
    u = u.filter((s) => {
      for (const g of D(s.slug))
        if (e.has(g)) return !0;
      return !1;
    });
  }
  ...
  async function c(e) {
    ...
    if (w instanceof HTMLSelectElement && Array.from(w.options).some((m) => m.value === e))
      ...
      if (N) {
        const A = Q(m, L, n.schemaVersionPattern);
