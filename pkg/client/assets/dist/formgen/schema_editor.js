...
(() => {
  const b = (t) => t.toLowerCase().trim().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "block", L = (t) => {
    ...
    for (const s of [
      ...
      const n = e.querySelector(s)?.value?.trim();
      if (n) return `${b(n)}@v1.0.0`;
    }
    return "block@v1.0.0";
  }, m = (t) => {
    const e = t.trim();
    if (!e) return { value: {} };
    let s = null;
    const n = A(e, null, { onError: (i) => {
      s = i;
    } });
    if (s) {
      const i = s instanceof Error ? s.message : "Invalid JSON", o = i.match(/position (\d+)/i) || i.match(/line (\d+)/i);
      return {
        value: null,
        error: i,
        errorLine: o ? parseInt(o[1], 10) : void 0
      };
    }
    return { value: n };
    ...
    let e = 0, s = 0, n = 0;
    const i = /* @__PURE__ */ new Set(), o = (a, u) => {
      if (!(!a || typeof a != "object")) {
        if (n = Math.max(n, u), a.type && i.add(String(a.type)), a.properties && typeof a.properties == "object") {
          const p = Object.keys(a.properties);
          e += p.length, p.forEach((r) => o(a.properties[r], u + 1));
        }
        if (Array.isArray(a.required) && (s += a.required.length), a.items && o(a.items, u + 1), a.allOf || a.anyOf || a.oneOf) {
          const p = a.allOf || a.anyOf || a.oneOf;
          Array.isArray(p) && p.forEach((r) => o(r, u + 1));
        }
      }
    };
    return o(t, 0), {
...
`), n = s.length, i = String(n).length;
    e.innerHTML = s.map((o, a) => `<span class="block text-right pr-2">${String(a + 1).padStart(i, " ")}</span>`).join("");
  }, O = (t, e) => {
  ...
  "enum": ["option1", "option2", "option3"]
    ...
    document.querySelectorAll("[data-schema-editor]").forEach((t) => {
      ...
      if (!e) return;
      const s = t.querySelector("[data-schema-format]"), n = t.querySelector("[data-schema-validate]"), i = t.querySelector("[data-schema-metadata]"), o = t.querySelector("[data-schema-copy]"), a = t.querySelector("[data-schema-line-numbers]"), u = t.querySelector("[data-schema-stats]");
      t.querySelectorAll("[data-schema-template]").forEach((r) => {
        const d = r.dataset.schemaTemplate;
        d && E[d] && r.addEventListener("click", () => {
          C(e, E[d]), l(t, `Inserted ${d} template`, "info");
        });
      }), a && (h(e, a), e.addEventListener("input", () => h(e, a)), e.addEventListener("scroll", () => O(e, a))), s?.addEventListener("click", () => {
        const r = m(e.value);
        if (r.error) {
          l(t, r.error, "error");
          return;
        ...
        const d = L(t), c = q(r.value, d);
        e.value = S(c), a && h(e, a), u && v(t, c), l(t, "Metadata inserted", "ok");
...
`, c - 1) + 1;
            f.substring(y, c).match(/^  /) && (e.value = f.substring(0, y) + f.substring(y + 2), e.selectionStart = e.selectionEnd = c - 2);
          } else
        ...
        if (d[r.key]) {
          ...
          if (c === g) {
            r.preventDefault();
            const f = r.key, y = d[f];
            e.value = e.value.substring(0, c) + f + y + e.value.substring(g), e.selectionStart = e.selectionEnd = c + 1, e.dispatchEvent(new Event("input", { bubbles: !0 }));
          }
        }
      });
      const p = m(e.value);
