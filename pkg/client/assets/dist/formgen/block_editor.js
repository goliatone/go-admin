...
function Z(t) {
  return ie(t.closest("[data-component-config]")?.getAttribute("data-component-config") ?? null);
}
function ee(t) {
  if (t == null) return;
...
function G(t, e) {
  ...
  const n = Ee(e), r = t.querySelector("[data-block-list]");
...
function Ie(t) {
  ...
  const l = e.list, w = e.output, O = () => {
    ...
    const d = o.map((f) => {
      ...
      x && (s._type = x);
      const i = f.dataset.blockSchema || s._schema;
      if (i) s._schema = i;
      else {
        const m = r.get(x);
    ...
    Array.from(l.querySelectorAll("[data-block-item]")).forEach((o, u) => {
      o.querySelectorAll("input, select, textarea").forEach((d) => {
        ...
        const f = d.getAttribute("data-block-field-name") || d.name;
        f && (d.hasAttribute("data-block-field-name") || d.setAttribute("data-block-field-name", f), d.name = ue(a, u, f));
      });
    });
  }, ae = () => {
  ...
  const oe = (o, u) => {
    ...
    const d = he(o, u, n.schemaVersionPattern), f = document.createElement("div");
    ...
    p.className = "block-schema-badge inline-flex items-center text-xs font-mono text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded dark:bg-blue-900/20 dark:text-blue-400", p.textContent = d, p.setAttribute("data-block-schema-badge", "true"), g.appendChild(x), g.appendChild(i), g.appendChild(m), g.appendChild(p);
    ...
    E.appendChild($), f.appendChild(s), f.appendChild(E), Be(f, o.type), qe(f, d), f.dataset.blockSchema = d, u && oe(f, u);
    const K = o.requiredFields || [];
    return K.length > 0 && ve(E, K), (o.collapsed ?? !1) && (E.classList.add("hidden"), f.dataset.blockCollapsed = "true", k.textContent = "Expand"), f;
  }, _ = (o, u) => {
    const d = r.get(o);
    if (!d) return;
    const f = Y(d, u);
  ...
  if (j && H && j.addEventListener("click", () => {
    ...
    if (d) {
      ...
      if (u.closest("[data-block-collapse]")) {
        const f = d.querySelector("[data-block-body]");
        if (f) {
      ...
      if (u.closest("[data-block-move-up]")) {
        ...
        f && l.insertBefore(d, f), S();
        return;
      }
      if (u.closest("[data-block-move-down]")) {
        const f = d.nextElementSibling;
        f && l.insertBefore(f, d), S();
        return;
      }
    }
    ...
    const o = n.dragFromHeader ?? !0, u = n.enableTouch ?? !0, d = n.enableAnimations ?? !0, f = n.enableCrossEditor ?? !1, s = {
      dragging: null,
      placeholder: null,
      touchStartY: 0,
      touchCurrentY: 0,
      scrollInterval: null,
      originalIndex: -1
    }, g = (i) => i.closest("[data-block-drag-handle]") ? !0 : o && i.closest("[data-block-header]") ? !i.closest("button, input, select, textarea") : !1, x = (i) => Array.from(l.querySelectorAll("[data-block-item]")).indexOf(i);
    if (l.addEventListener("dragstart", (i) => {
      ...
      if (!p) return;
      if (s.dragging = p, s.originalIndex = x(p), p.classList.add("block-item--dragging"), i.dataTransfer) {
        if (i.dataTransfer.effectAllowed = "move", i.dataTransfer.setData("text/plain", "block"), f) {
          const h = Se(p);
          i.dataTransfer.setData("application/x-block", JSON.stringify(h));
        }
        const k = p.cloneNode(!0);
        k.style.cssText = "position: absolute; top: -9999px; left: -9999px; opacity: 0.8; transform: rotate(2deg);", document.body.appendChild(k), i.dataTransfer.setDragImage(k, 20, 20), requestAnimationFrame(() => k.remove());
      }
      const y = T(l, p);
      ...
      const m = i.clientY, p = D(l, m, s.dragging || void 0);
      ...
      i.preventDefault(), I(l);
      const m = i.clientY, p = D(l, m, s.dragging || void 0);
      if (!s.dragging && f && i.dataTransfer) {
      ...
      if (s.dragging && (p && (p.position === "before" ? l.insertBefore(s.dragging, p.item) : l.insertBefore(s.dragging, p.item.nextSibling)), d && X(l, s.dragging), x(s.dragging) !== s.originalIndex)) {
      ...
      l.addEventListener("touchstart", (k) => {
        ...
        if (s.touchCurrentY = h.clientY, !p) {
          ...
          p = !0, m = i.cloneNode(!0), m.className = "block-item--touch-dragging", m.style.cssText = `
            ...
            transform: scale(1.02);
          `, document.body.appendChild(m), i.classList.add("block-item--placeholder"), s.dragging = i, s.originalIndex = x(i), B(`Dragging ${T(l, i).label}. Move finger to reposition.`);
        }
        k.preventDefault(), m && (m.style.top = `${h.clientY - m.offsetHeight / 2}px`);
        const E = D(l, h.clientY, i || void 0);
        E ? Q(l, E.item, E.position) : I(l), Le(l, s);
      }, { passive: !1 }), l.addEventListener("touchend", () => {
        if (s.scrollInterval && (clearInterval(s.scrollInterval), s.scrollInterval = null), I(l), m && (m.remove(), m = null), i && p) {
          i.classList.remove("block-item--placeholder");
          const k = D(l, s.touchCurrentY, i);
          if (k && (k.position === "before" ? l.insertBefore(i, k.item) : l.insertBefore(i, k.item.nextSibling)), d && X(l, i), x(i) !== s.originalIndex) {
            const h = T(l, i);
            B(`${h.label} moved to position ${h.position} of ${h.total}`);
          }
          S();
        }
        i = null, p = !1, s.dragging = null, s.originalIndex = -1;
      }), l.addEventListener("touchcancel", () => {
        s.scrollInterval && (clearInterval(s.scrollInterval), s.scrollInterval = null), I(l), m && (m.remove(), m = null), i && i.classList.remove("block-item--placeholder"), i = null, p = !1, s.dragging = null, s.originalIndex = -1;
      });
    }
  }
  if (e.addSelect) {
  ...
  t.addEventListener("keydown", (o) => {
    ...
    if (f)
      switch (o.key) {
        case "Delete":
        case "Backspace":
          o.shiftKey && (o.preventDefault(), d.remove(), S(), l.querySelector("[data-block-item] [data-block-header]")?.focus());
          break;
        case "ArrowUp":
          if (o.altKey && b) {
            o.preventDefault();
            ...
            if (g) {
              l.insertBefore(d, g), S(), d.querySelector("[data-block-header]")?.focus();
              const x = T(l, d);
        ...
        case "ArrowDown":
          if (o.altKey && b) {
            ...
            if (g) {
              l.insertBefore(g, d), S(), d.querySelector("[data-block-header]")?.focus();
              const x = T(l, d);
              B(`${x.label} moved to position ${x.position} of ${x.total}`);
        ...
        case "Enter":
        case " ":
          u.matches("[data-block-collapse]") || u.matches("[data-block-remove]") || u.matches("[data-block-move-up]") || u.matches("[data-block-move-down]") ? (o.preventDefault(), u.click()) : f && !u.matches("button") && (o.preventDefault(), d.querySelector("[data-block-collapse]")?.click());
          break;
        case "Escape":
  ...
  const P = n.showConflicts ?? !0;
  if (P && n.legacyBlocks && Array.isArray(n.legacyBlocks)) {
...
function we(t = document) {
  Array.from(t.querySelectorAll('[data-component="block"], [data-block-editor]')).filter((e) => e.dataset.blockLibraryPicker !== "true" && e.dataset.blockInit !== "manual").forEach((e) => Ie(e));
}
ce(() => we());
export {
  ...
  De as refreshBlockTemplateRegistry,
  $e as registerBlockTemplate
};
