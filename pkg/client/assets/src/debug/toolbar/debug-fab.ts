  ...
  private render(): void {
    ...
    this.shadow.innerHTML = `
      ...
      <div class="fab ${hiddenClass}" data-status="${this.connectionStatus}">
        ...
        <div class="fab-collapsed ${identity ? 'has-identity' : ''}" ${identity ? `style="--fab-environment:${identity.color}"` : ''}>
          <span class="fab-icon">
          ...
          ${identity ? `
            ...
            <span class="fab-identity-label" title="${escapeHTML(identity.label)}">${escapeHTML(identity.label)}</span>
          ` : ''}
        </div>
