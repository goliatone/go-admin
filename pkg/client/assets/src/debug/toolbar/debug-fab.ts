...
export class DebugFab extends HTMLElement {
  ...
  /** Accessible name: identity first, then live counters and link state. */
  private accessibleLabel(
    identity: ReturnType<typeof deploymentIndicator>,
    ...
    parts.push([
      plural(counts.requests, 'request', 'requests'),
      plural(counts.sql, 'query', 'queries'),
      plural(counts.logs, 'log', 'logs'),
    ].join(', '));
    if (counts.errors > 0) {
      parts.push(plural(counts.errors, 'error', 'errors'));
    }
    parts.push(`Debug stream ${this.connectionStatus}`);
    return parts.join('. ');
  }
  ...
  private updateCounters(): void {
    ...
    this.updateAccessibleLabel(counts);
  }
  ...
  /**
   * Counters and connection state change without a re-render, so the accessible
   * name is refreshed in place to stay truthful for assistive technology.
   */
  private updateAccessibleLabel(counts = getCounts(this.snapshot)): void {
    const fab = this.shadow.querySelector('.fab');
    if (!fab) {
      return;
    }
  ...
  private attachEventListeners(): void {
    ...
    // Keyboard activation: the FAB is an ARIA button, so Enter and Space must
    // behave like a click. Space is also swallowed to avoid scrolling the page.
