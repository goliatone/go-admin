...
test('Phase 0 contract: runtime imports shared agreement-form constants and normalization helpers', () => {
  ...
  assert.match(source, /from '\.\/agreement-form\/constants'/);
  assert.match(source, /from '\.\/agreement-form\/normalization'/);
  assert.match(source, /TOTAL_WIZARD_STEPS/);
  assert.match(source, /WIZARD_STEP\.PLACEMENT/);
...
test('Phase 1.3 contract: runtime uses computeEffectiveRulePages for rule summary', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // Must import computeEffectiveRulePages and formatEffectivePageRange
  assert.match(source, /computeEffectiveRulePages,/);
  assert.match(source, /formatEffectivePageRange,/);
  // Must call computeEffectiveRulePages in rule summary
  assert.match(source, /const effectiveResult = computeEffectiveRulePages\(/);
  // Must call formatEffectivePageRange for display text
  assert.match(source, /const effectiveRangeText = formatEffectivePageRange\(effectiveResult\)/);
  // Must handle empty result case with warning
  assert.match(source, /if \(effectiveResult\.isEmpty\) \{[\s\S]*?Warning: No initials fields will be generated/);
  // Must show effective range in summary
...
test('Phase 1.5-1.7 dist contract: built runtime reflects template/runtime agreements', () => {
  const distSource = fs.readFileSync(agreementRuntimeDistPath, 'utf8');
...
test('Phase 1.8 contract: runtime includes typeahead state management', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // TypeaheadState interface definition
  assert.match(source, /interface TypeaheadState \{/);
  ...
  assert.match(source, /const typeaheadState: TypeaheadState = \{/);
});

test('Phase 1.8 contract: runtime includes debounce utility', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // Debounce function must exist
...
test('Phase 1.8 contract: runtime includes loadRecentDocuments function', () => {
  ...
  // Must use sort parameters for recent documents
  assert.match(source, /sort: 'updated_at'/);
  assert.match(source, /sort_desc: 'true'/);
  // Must use per_page limit for recent documents
  assert.match(source, /per_page: String\(RECENT_DOCUMENTS_LIMIT\)/);
  // Must update typeaheadState.recentDocuments
...
test('Phase 1.8 contract: runtime includes typeahead dropdown control functions', () => {
  ...
  // Select document from typeahead function
  assert.match(source, /function selectDocumentFromTypeahead\(btn: HTMLElement\)/);
});
...
test('Phase 1.8 contract: runtime calls loadRecentDocuments on page load', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // Must call loadRecentDocuments after loadDocuments
  const loadDocumentsIndex = source.indexOf('loadDocuments();');
  const loadRecentIndex = source.indexOf('loadRecentDocuments();');
...
test('Phase 1.8 contract: runtime opens typeahead on change document button click', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // Change document button handler must focus search and open typeahead
...
test('Phase 1.8 contract: runtime closes typeahead on outside click', () => {
  ...
  assert.match(source, /document\.addEventListener\('click', \(e\) => \{[\s\S]*?closeTypeaheadDropdown\(\)/);
});
...
test('Phase 2 contract: runtime imports and initializes preview card', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // Must import preview card module
  assert.match(source, /import \{ createPreviewCard/);
  assert.match(source, /from '\.\/agreement-form\/preview-card'/);
  // Must import PREVIEW_CARD_VISIBLE_STEPS constant
  assert.match(source, /PREVIEW_CARD_VISIBLE_STEPS/);
  assert.match(source, /from '\.\/agreement-form\/constants'/);
  // Must create preview card instance
...
test('Phase 2 contract: runtime updates preview card on document selection', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // selectDocument must call previewCard.setDocument
  assert.match(source, /function selectDocument[\s\S]*?previewCard\.setDocument\(/);
  // selectDocumentFromTypeahead must call previewCard.setDocument
  assert.match(source, /function selectDocumentFromTypeahead[\s\S]*?previewCard\.setDocument\(/);
});

test('Phase 2 contract: runtime updates preview card visibility on step change', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // updateWizardUI must call previewCard.updateVisibility
...
test('Phase 3 contract: runtime imports and initializes link group state', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // Must import linked-placement module functions
  assert.match(source, /import \{[\s\S]*?createLinkGroupState/);
  assert.match(source, /from '\.\/agreement-form\/linked-placement'/);
  // Must initialize linkGroupState in placementState
...
test('Phase 3 contract: runtime creates link groups from rules', () => {
  const source = fs.readFileSync(agreementRuntimePath, 'utf8');
  // collectPlacementFieldDefinitions must create link groups from rules
  assert.match(source, /const fieldsByRuleId = new Map<string, string\[\]>\(\)/);
  // Must add link groups to state
