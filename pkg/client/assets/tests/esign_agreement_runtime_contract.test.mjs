...
test('Phase 5 contract: signer review uses shared PDF runtime and prefers preview_url', () => {
  ...
  assert.match(source, /function bootstrapEndpointPath\(\)/);
  assert.match(source, /\/bootstrap\/\$\{encodeURIComponent\(unifiedConfig\.token\)\}/);
  assert.match(source, /async function ensurePublicSignerSession\(\)/);
  assert.match(source, /publicSession\.bearerToken = String\(authPayload\.token \|\| ''\)\.trim\(\);/);
  assert.match(source, /headers\.set\('Authorization', `Bearer \$\{publicSession\.bearerToken\}`\);/);
  assert.match(source, /credentials:\s*'omit'/);
  assert.match(source, /function createAuthorizedAssetObjectUrl\(assetUrl\)/);
  assert.match(source, /URL\.createObjectURL\(blob\)/);
  assert.match(source, /replacePreviewObjectUrl\(documentUrl\)/);
  assert.doesNotMatch(source, /navigator\.sendBeacon/);
  assert.match(source, /function resolveBinaryAssetUrl\(assets\)/);
  assert.match(source, /function reviewBasePath\(\)/);
  assert.match(source, /function assetsContractPath\(\)/);
  assert.match(source, /function telemetryEndpointPath\(\)/);
