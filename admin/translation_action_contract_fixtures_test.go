...
func TestTranslationActionContractFixtures(t *testing.T) {
	files := []string{
	...
	for _, file := range files {
		t.Run(file, func(t *testing.T) {
			payload := readTranslationActionFixture(t, file)
...
func readTranslationActionFixture(t *testing.T, file string) map[string]any {
	t.Helper()
	path := filepath.Join("..", "pkg", "client", "assets", "tests", "fixtures", "translation_action_contracts", file)
