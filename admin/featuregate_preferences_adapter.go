...
func (a *PreferencesStoreAdapter) Load(ctx context.Context, ref state.Ref) (map[string]any, state.Meta, bool, error) {
	...
	snapshot, err := a.store.Resolve(ctx, PreferencesResolveInput{
		Scope:  prefScope,
...
func (a *PreferencesStoreAdapter) preferenceScope(scopeDef opts.Scope) (PreferenceLevel, PreferenceScope, error) {
	...
	case "tenant":
		...
		return PreferenceLevelTenant, PreferenceScope{TenantID: id}, nil
	case "org":
		id, err := extractScopeID(scopeDef, scope.MetadataOrgID)
		if err != nil {
			return "", PreferenceScope{}, err
		}
		return PreferenceLevelOrg, PreferenceScope{OrgID: id}, nil
	case "user":
		id, err := extractScopeID(scopeDef, scope.MetadataUserID)
		if err != nil {
			return "", PreferenceScope{}, err
		}
		return PreferenceLevelUser, PreferenceScope{UserID: id}, nil
	default:
		return "", PreferenceScope{}, ferrors.NewBadInput(ferrors.TextCodeScopeInvalid, fmt.Sprintf("optionsadapter: unsupported scope %q", scopeDef.Name), map[string]any{
			ferrors.MetaAdapter:   "options",
			ferrors.MetaStore:     "preferences",
