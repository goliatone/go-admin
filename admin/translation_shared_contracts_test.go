...
func TestTranslationSharedContractsSnapshot(t *testing.T) {
	payload := TranslationSharedContractsPayload()
	assertTranslationSharedContractsSnapshot(t, payload, "testdata/translation_shared_contracts_snapshot.json")
}

func TestTranslationSharedContractsStatusNormalization(t *testing.T) {
...
func TestTranslationSharedContractsDisabledReasonCodesAcrossActionPayloads(t *testing.T) {
...
func assertTranslationSharedContractsSnapshot(t *testing.T, payload map[string]any, snapshotPath string) {
	t.Helper()
	...
	if os.Getenv("UPDATE_SNAPSHOTS") == "1" {
		if writeErr := os.WriteFile(snapshotPath, append(data, '\n'), 0o644); writeErr != nil {
			t.Fatalf("write snapshot %q: %v", snapshotPath, writeErr)
		}
		return
	}
	...
	if got != expected {
		t.Fatalf("translation shared contracts snapshot mismatch\nexpected:\n%s\n\ngot:\n%s", expected, got)
	}
}
