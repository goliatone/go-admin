...
func TestTranslationQueueBindingDashboardAggregatesCardsTablesAndLinks(t *testing.T) {
	...
	if href := toString(queueLink["href"]); !strings.Contains(href, "translation_group_id=tg-page-1") {
		t.Fatalf("expected queue drilldown href to preserve translation_group_id, got %q", href)
	}
