...
type countCapableContentTypeSourceStub struct {
	siteAPIContentServiceStub
	rows  []map[string]any
	calls int
}

func (s *countCapableContentTypeSourceStub) ListContentTypeRecords(_ context.Context, _ CMSContentType, opts ListOptions) ([]map[string]any, int, error) {
	s.calls++
	rows := make([]map[string]any, 0, len(s.rows))
	for _, row := range s.rows {
		rows = append(rows, primitives.CloneAnyMap(row))
	}
	paged, total := applyListOptionsToRecordMaps(rows, opts, listRecordOptions{
...
func TestAdminContentReadServiceListForContentTypeUsesCountCapableSourceTotals(t *testing.T) {
	ctx := context.Background()
