...
func renderFamilyDetailAssignToUserForm(b *strings.Builder, data map[string]any, row map[string]any) {
	...
	renderFamilyDetailHiddenInput(b, "work_scope", firstNonEmpty(toString(payload["work_scope"]), toString(extractMap(row["locale_assignment"])["work_scope"])))
	renderFamilyDetailHiddenInput(b, "channel", toString(extractMap(data["meta"])["channel"]))
	b.WriteString(`<select class="block h-12 min-w-56 flex-1 rounded-lg border border-gray-300 bg-white px-3 text-sm shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" name="assignee_id" data-family-assignee-select="` + escapeFamilyFragmentAttr(key) + `" data-initial-assignee-id="` + escapeFamilyFragmentAttr(currentID) + `" data-formgen-managed="true" data-formgen-relationship="true" data-endpoint-url="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["endpoint_url"]), "/api/translations/options/assignees?per_page=200")) + `" data-endpoint-method="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["endpoint_method"]), "GET")) + `" data-endpoint-renderer="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["endpoint_renderer"]), "typeahead")) + `" data-endpoint-search-param="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["endpoint_search"]), "q")) + `" data-endpoint-hydrate-param="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["endpoint_hydrate"]), "assignee_id")) + `" data-endpoint-value-field="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["endpoint_value"]), "value")) + `" data-endpoint-label-field="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["endpoint_label"]), "label")) + `" data-endpoint-placeholder="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["placeholder"]), "Select assignee")) + `" data-endpoint-search-placeholder="` + escapeFamilyFragmentAttr(firstNonEmpty(toString(assignee["search_placeholder"]), "Search assignees")) + `" data-relationship-type="belongsTo" data-relationship-target="#/components/schemas/User" data-relationship-cardinality="one"`)
	if currentID != "" {
		b.WriteString(` data-relationship-current="` + escapeFamilyFragmentAttr(currentID) + `"`)
	}
	b.WriteString(` aria-label="Assignee">`)
