...
func TestWorkflowTransportApplyEventPersistsStateProjectsActivityAndReturnsEnvelope(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodPost, actionPath, strings.NewReader(`{"id":"`+id+`"}`))
...
func TestWorkflowTransportDetailIncludesSnapshotDiagnostics(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodGet, detailPath, nil)
	rr := httptest.NewRecorder()
	server.WrappedRouter().ServeHTTP(rr, req)
