...
func buildSearchPageViewProjection(
	...
	return searchPageViewProjection{
		...
		totalAccuracy: envelope.TotalAccuracy,
...
func (p searchPageViewProjection) apply(view router.ViewContext) {
	...
	view["search_variant"] = p.variant
	view["search_total_accuracy"] = p.totalAccuracy
	view["search_counts"] = p.counts
...
func (p searchPageViewProjection) nestedSearch() map[string]any {
	return map[string]any{
		...
		"state":          p.state,
		"variant":        p.variant,
		"total_accuracy": p.totalAccuracy,
		"counts":         p.counts,
	}
}
