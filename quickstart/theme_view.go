...
// WithThemeContext resolves the active theme (with optional query/header overrides) and adds it to the view context.
func WithThemeContext(ctx router.ViewContext, adm *admin.Admin, req router.Context) router.ViewContext {
	...
	baseCtx = admin.WithThemeSelectionFromRequest(baseCtx, req)
