...
- `WithViewAdmin(adm *admin.Admin) ViewEngineOption` - Inputs: Admin instance; outputs: binds Admin's structural-partial lookup to the exact normalized first-wins view stack during `NewViewEngine` construction.
...
- `NewAuthUIBrowserProtection(cfg admin.Config, opts ...AuthUIBrowserProtectionOption) (*AuthUIBrowserProtection, error)` - Inputs: admin config plus optional stable key, cookie, trusted secure-request resolver, and origin policy. Outputs: a reusable browser-bound CSRF contract for Auth UI pages and onboarding mutations.
...
- Login, registration, and password-reset forms use a browser-bound pre-auth cookie plus stateless CSRF signatures and same-origin checks. Create one `AuthUIBrowserProtection` and pass it through `WithAuthUIBrowserProtection`, `WithRegistrationUIBrowserProtection`, and `WithOnboardingBrowserProtection` so every page/mutation pair shares the same key and cookie contract.
- Multi-instance deployments must use a shared `cfg.PreviewSecret` or `WithAuthUIBrowserProtectionSecureKey(...)`. Use `WithAuthUIBrowserProtectionSecureRequestResolver(...)` when TLS terminates at a trusted proxy so the pre-auth cookie is marked Secure.
- On trusted HTTPS requests, the default cookie uses the `__Host-` prefix, `Path=/`, no `Domain`, `HttpOnly`, and `SameSite=Lax`; plain HTTP keeps a development-compatible host-only name.
...
- `RegisterOnboardingRoutes(r router.Router[T], cfg admin.Config, handlers OnboardingHandlers, opts ...OnboardingRouteOption) error` - Inputs: router/config/handlers; outputs: error (registers onboarding API routes; public registration and password-reset mutations require the browser-bound CSRF token/cookie/origin contract).
...
- Dashboard SSR: `DashboardTemplatesFS()` is the canonical client template set. Provide `WithDashboardTemplatesFS` for first-wins overlays; disabling packaged templates via `WithDashboardEmbeddedTemplates(false)` requires a complete compatible shell and fails explicitly for incomplete stacks.
...
- Typed authenticated pages: build `admin.AdminPageChrome` and call `admin.EnrichLayoutViewContextWithChrome`; keep arbitrary actions in `page_header_actions`. Import shared browser presentation from `@goliatone/go-admin-client/components.css`, then mount product CSS with `WithExtraAssetsFS` and link it from `head_extra`.
