...
- Use `RenderCachePolicy.RequireTagIndex: true` only with a shared non-memory backend for production tag invalidation. With this guard enabled, memory backends bypass caching, stores without `site.RenderCacheTagInvalidator` bypass caching, and tag attachment failures remove the just-written entry before it can be served.
