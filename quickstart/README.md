...
- `WithNavPlacements(ctx router.ViewContext, adm *admin.Admin, cfg admin.Config, placements PlacementConfig, placement PlacementKey, active string, reqCtx context.Context) router.ViewContext` - Inputs: same as `WithNav`, plus placement mapping. Outputs: placement-aware nav context for non-sidebar menus while preserving the same feature/session/theme enrichment.
- `ResolveDashboardArea(placements PlacementConfig, placement PlacementKey, fallback string) string` - Inputs: placement config, dashboard placement key, fallback area code. Outputs: resolved dashboard area code for quickstart-owned widget wiring.
- `DefaultDashboardArea(placement PlacementKey) string` - Inputs: dashboard placement key. Outputs: quickstart default dashboard area code for the placement.
