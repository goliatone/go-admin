...
- `WithThemeContext(ctx router.ViewContext, adm *admin.Admin, req router.Context) router.ViewContext` - Inputs: view context, admin, request. Outputs: context enriched with theme tokens/selection, typed `admin_partials`, bounded structural diagnostics, and query-string theme preview overrides.
